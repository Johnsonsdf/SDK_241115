 ...
 *      DEFINES
 *********************/
#ifndef LV_DISP_BUF_MAX_NUM
#define LV_DISP_BUF_MAX_NUM 4 /*Maximum buffer count for draw buffers, must not less than 2*/
#endif
...
typedef struct _lv_disp_draw_buf_t {
    ...
    void * bufs[LV_DISP_BUF_MAX_NUM]; /**< display buffers.*/
    uint8_t buf_cnt; /**< number of display buffers.*/
    ...
    volatile unsigned int flushing_idx; /**< flushing display buffer index, used to decide if any render buffer available*/
    volatile unsigned int rendering_idx; /**< rendering display buffer index.*/
...
typedef struct _lv_disp_drv_t {
    ...
    /**< 1: pause flushing the internal buffer (draw_buf) to the display*/
    uint32_t refr_paused : 1;
    ...
    /** OPTIONAL: called when start rendering an area part */
    void (*render_area_start_cb)(struct _lv_disp_drv_t * disp_drv, const lv_area_t * area);

...
static void set_px_cb_rgb_565(lv_disp_drv_t * disp_drv, uint8_t * buf, lv_coord_t buf_w, lv_coord_t x,
                                lv_coord_t y, lv_color_t color, lv_opa_t opa);
static void set_px_cb_argb_8565(lv_disp_drv_t * disp_drv, uint8_t * buf, lv_coord_t buf_w, lv_coord_t x,
                                lv_coord_t y, lv_color_t color, lv_opa_t opa);
#endif

#if LV_COLOR_DEPTH != 32
static void set_px_cb_argb_8888(lv_disp_drv_t * disp_drv, uint8_t * buf, lv_coord_t buf_w, lv_coord_t x,
                                lv_coord_t y, lv_color_t color, lv_opa_t opa);
static void set_px_cb_xrgb_8888(lv_disp_drv_t * disp_drv, uint8_t * buf, lv_coord_t buf_w, lv_coord_t x,
                                lv_coord_t y, lv_color_t color, lv_opa_t opa);
#endif

static void set_px_cb_rgb_888(lv_disp_drv_t * disp_drv, uint8_t * buf, lv_coord_t buf_w, lv_coord_t x,
                              lv_coord_t y, lv_color_t color, lv_opa_t opa);

static void set_px_cb_argb_6666(lv_disp_drv_t * disp_drv, uint8_t * buf, lv_coord_t buf_w, lv_coord_t x,
                                lv_coord_t y, lv_color_t color, lv_opa_t opa);

static void set_px_cb_argb_1555(lv_disp_drv_t * disp_drv, uint8_t * buf, lv_coord_t buf_w, lv_coord_t x,
                                lv_coord_t y, lv_color_t color, lv_opa_t opa);

static void set_px_cb_alpha1_le(lv_disp_drv_t * disp_drv, uint8_t * buf, lv_coord_t buf_w, lv_coord_t x, lv_coord_t y,
                                lv_color_t color, lv_opa_t opa);

static void set_px_cb_alpha4_le(lv_disp_drv_t * disp_drv, uint8_t * buf, lv_coord_t buf_w, lv_coord_t x, lv_coord_t y,
                                lv_color_t color, lv_opa_t opa);
...
void lv_disp_draw_buf_init(lv_disp_draw_buf_t * draw_buf, void * buf1, void * buf2, uint32_t size_in_px_cnt)
{
    ...
    draw_buf->bufs[0] = buf1;
    draw_buf->bufs[1] = buf2;
    draw_buf->buf_cnt = (buf2 != NULL) ? 2 : 1;
    draw_buf->buf_act = buf1;
    draw_buf->size    = size_in_px_cnt;

    draw_buf->flushing_idx = draw_buf->buf_cnt;
    draw_buf->rendering_idx = 0;
}

/**
 * Initialize a display buffer
 * @param draw_buf pointer `lv_disp_draw_buf_t` variable to initialize
 * @param bufs Array of buffers to be used by LVGL to draw the image.
 *             Always has to specified and can't be NULL.
 * @param buf_cnt Number of buffers. Must not be 0.
 *             (sending to the display) parallel.
 * @param size_in_px_cnt size of the each buffer in pixel count.
 */
void lv_disp_draw_buf_init2(lv_disp_draw_buf_t * draw_buf, void ** bufs, uint32_t buf_cnt, uint32_t size_in_px_cnt)
{
    lv_memset_00(draw_buf, sizeof(lv_disp_draw_buf_t));

    buf_cnt = LV_MIN(buf_cnt, LV_DISP_BUF_MAX_NUM);
    draw_buf->buf_cnt = buf_cnt;
    memcpy(draw_buf->bufs, bufs, buf_cnt * sizeof(void *));

    draw_buf->buf_act = draw_buf->bufs[0];
    draw_buf->size    = size_in_px_cnt;
...
void LV_ATTRIBUTE_FLUSH_READY lv_disp_flush_ready(lv_disp_drv_t * disp_drv)
{
    ...
    disp_drv->draw_buf->flushing_idx++;
    //disp_drv->draw_buf->flushing_last = 0; /*Do not clear because it may be called just before the user flush_cb()*/
}
...
/**
 * Tell if all the flushing is finished or not.
 * @param disp_drv pointer to display driver
 * @return true: all the flushing is finished; false: still some flushing is in-process
 */
LV_ATTRIBUTE_FLUSH_READY bool lv_disp_flush_is_finished(lv_disp_drv_t * disp_drv)
{
    return (disp_drv->draw_buf->rendering_idx + disp_drv->draw_buf->buf_cnt) ==
                disp_drv->draw_buf->flushing_idx;
}
...
void lv_disp_drv_use_generic_set_px_cb(lv_disp_drv_t * disp_drv, lv_img_cf_t cf)
{
    switch(cf) {
        ...
        case LV_IMG_CF_RGB_565:
            disp_drv->set_px_cb = set_px_cb_rgb_565;
            break;
        case LV_IMG_CF_ARGB_8565:
            disp_drv->set_px_cb = set_px_cb_argb_8565;
            break;
#endif
#if LV_COLOR_DEPTH != 32
        case LV_IMG_CF_ARGB_8888:
            disp_drv->set_px_cb = set_px_cb_argb_8888;
            break;
        case LV_IMG_CF_XRGB_8888:
            disp_drv->set_px_cb = set_px_cb_xrgb_8888;
            break;
#endif
        case LV_IMG_CF_RGB_888:
            disp_drv->set_px_cb = set_px_cb_rgb_888;
            break;
        case LV_IMG_CF_ARGB_6666:
            disp_drv->set_px_cb = set_px_cb_argb_6666;
            break;
        case LV_IMG_CF_ARGB_1555:
            disp_drv->set_px_cb = set_px_cb_argb_1555;
            break;
        case LV_IMG_CF_ALPHA_4BIT_LE:
            disp_drv->set_px_cb = set_px_cb_alpha4_le;
            break;
        case LV_IMG_CF_ALPHA_1BIT_LE:
            disp_drv->set_px_cb = set_px_cb_alpha1_le;
            break;
                                ...
                                lv_coord_t y, lv_color_t color, lv_opa_t opa)
{
    ...
    lv_color_mix_with_alpha(bg_color, 255, color, opa, &res_color, &res_opa);
                                ...
                                lv_coord_t y, lv_color_t color, lv_opa_t opa)
{
    ...
    d.header.h = 1;
    d.header.w = buf_w;
    d.header.cf = LV_IMG_CF_ARGB_8565;
                                ...
                                lv_color_t color, lv_opa_t opa)
{
    ...
    d.header.h = 1;
    d.header.w = buf_w;
    d.header.cf = LV_IMG_CF_ARGB_8888;
                                ...
                                lv_coord_t y, lv_color_t color, lv_opa_t opa)
{
    ...
    d.header.h = 1;
    d.header.w = buf_w;
    d.header.cf = LV_IMG_CF_ARGB_6666;
                                ...
                                lv_coord_t y, lv_color_t color, lv_opa_t opa)
{
    ...
    d.header.h = 1;
    d.header.w = buf_w;
    d.header.cf = LV_IMG_CF_ARGB_1555;
                                ...
                                lv_color_t color, lv_opa_t opa)
{
    ...
    d.data = buf;
    d.header.w = buf_w;
    d.header.cf = LV_IMG_CF_ALPHA_1BIT_LE;
                                ...
                                lv_color_t color, lv_opa_t opa)
{
    ...
    d.data = buf;
    d.header.w = buf_w;
    d.header.cf = LV_IMG_CF_ALPHA_4BIT_LE;

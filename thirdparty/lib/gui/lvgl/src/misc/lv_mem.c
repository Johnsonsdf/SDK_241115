...
#else
    void * alloc = LV_MEM_CUSTOM_ALLOC(size, func);
#endif
...
void * lv_mem_realloc_debug(void * data_p, size_t new_size, const char *func)
{
    ...
    if(data_p == &zero_mem) return lv_mem_alloc_debug(new_size, func);

#if LV_MEM_CUSTOM == 0
    void * new_p = lv_tlsf_realloc(tlsf, data_p, new_size);
#else
    void * new_p = LV_MEM_CUSTOM_REALLOC(data_p, new_size, func);
#endif
    if(new_p == NULL) {

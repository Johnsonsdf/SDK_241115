...
lv_res_t lv_snapshot_take_to_buf(lv_obj_t * obj, lv_img_cf_t cf, lv_img_dsc_t * dsc, void * buf, uint32_t buff_size)
{
    ...
    if(cf == LV_IMG_CF_TRUE_COLOR) {
        fake_disp.driver->clear_cb = obj_disp->driver->clear_cb;
        fake_disp.driver->clean_dcache_cb = obj_disp->driver->clean_dcache_cb;
        fake_disp.driver->draw_ctx_size = obj_disp->driver->draw_ctx_size;
        fake_disp.driver->draw_ctx_init = obj_disp->driver->draw_ctx_init;
        fake_disp.driver->draw_ctx_deinit = obj_disp->driver->draw_ctx_deinit;
    }
    else {
        fake_disp.driver->draw_ctx_size = sizeof(lv_draw_sw_ctx_t);
        fake_disp.driver->draw_ctx_init = lv_draw_sw_init_ctx;
        fake_disp.driver->draw_ctx_deinit = lv_draw_sw_deinit_ctx;
    }

    lv_draw_ctx_t * draw_ctx = lv_mem_alloc(fake_disp.driver->draw_ctx_size);
    ...
    if(fake_disp.driver->clear_cb) {
        fake_disp.driver->clear_cb(fake_disp.driver, buf, buff_size / sizeof(lv_color_t));
    }
    else {
        lv_memset_00(buf, buff_size);
    }

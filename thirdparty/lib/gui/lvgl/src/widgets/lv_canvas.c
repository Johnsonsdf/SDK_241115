...
void lv_canvas_fill_bg(lv_obj_t * canvas, lv_color_t color, lv_opa_t opa)
{
    ...
    else if((dsc->header.cf == LV_IMG_CF_TRUE_COLOR || dsc->header.cf == LV_IMG_CF_TRUE_COLOR_CHROMA_KEYED) && color.full == 0) {
        lv_memset((uint8_t *)dsc->data, 0, LV_IMG_BUF_SIZE_TRUE_COLOR(dsc->header.w, dsc->header.h));
    }
    else if((dsc->header.cf == LV_IMG_CF_TRUE_COLOR_ALPHA || dsc->header.cf >= LV_IMG_CF_ALPHA_2BIT) && color.full == 0 && opa == 0) {
        lv_memset((uint8_t *)dsc->data, 0, ((lv_img_cf_get_px_size(dsc->header.cf) * dsc->header.w + 7) / 8) * dsc->header.h);
    }
    else {
        uint32_t x;
        uint32_t y;
...
static void init_fake_disp(lv_obj_t * canvas, lv_disp_t * disp, lv_disp_drv_t * drv, lv_area_t * clip_area)
{
    ...
    if(dsc->header.cf == LV_IMG_CF_TRUE_COLOR) {
        lv_disp_t * obj_disp = lv_obj_get_disp(canvas);
        disp->driver->draw_ctx_size = obj_disp->driver->draw_ctx_size;
        disp->driver->draw_ctx_init = obj_disp->driver->draw_ctx_init;
        disp->driver->draw_ctx_deinit = obj_disp->driver->draw_ctx_deinit;
    }
    else {
        disp->driver->draw_ctx_size = sizeof(lv_draw_sw_ctx_t);
        disp->driver->draw_ctx_init = lv_draw_sw_init_ctx;
        disp->driver->draw_ctx_deinit = lv_draw_sw_deinit_ctx;
    }

    lv_draw_ctx_t * draw_ctx = lv_mem_alloc(disp->driver->draw_ctx_size);
    LV_ASSERT_MALLOC(draw_ctx);
    if(draw_ctx == NULL)  return;
    disp->driver->draw_ctx_init(disp->driver, draw_ctx);
...
{
    LV_UNUSED(canvas);
    lv_draw_wait_for_finish(disp->driver->draw_ctx);

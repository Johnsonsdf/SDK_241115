...
/* Consider each option owns half line space around it in click and move events */
#define LV_ROLLER_OPTION_HALF_SPACE 1
...
static const char * get_option_start(void * obj, lv_draw_label_dsc_t *label_dsc,
                                     lv_area_t *coords, const lv_area_t *clip);
...
void lv_roller_set_options(lv_obj_t * obj, const char * options, lv_roller_mode_t mode)
{
    ...
    if(roller->option_chr)
        lv_mem_free(roller->option_chr);

    roller->option_chr = lv_mem_alloc(roller->option_cnt * sizeof(void *));
    if(roller->option_chr) {
        const char *str = lv_label_get_text(label);

        roller->option_chr[0] = str;
        for(cnt = 1; *str != '\0'; str++) {
            if(*str == '\n') {
                roller->option_chr[cnt++] = str + 1;
            }
        }
    }
...
static void draw_main(lv_event_t * e)
{
    ...
    /*Post draw when the children are drawn*/
        ...
        if(area_ok) {
            lv_obj_t * label = get_label(obj);
            lv_roller_t * roller_data = (lv_roller_t *)obj;

            if(lv_label_get_recolor(label)) label_dsc.flag |= LV_TEXT_FLAG_RECOLOR;

            /*Get the size of the "selected text"*/
            if (roller_data->sel_size.x == 0) {
                lv_txt_get_size(&roller_data->sel_size, lv_label_get_text(label), label_dsc.font, label_dsc.letter_space, label_dsc.line_space,
                            lv_obj_get_width(obj), LV_TEXT_FLAG_EXPAND);
            }
            lv_point_t res_p = roller_data->sel_size;
            ...
            if (roller_data->option_chr) {
                const char *opt_start = get_option_start(obj, &label_dsc, &label_sel_area, &mask_sel);
                if(opt_start) {
                    lv_draw_label(draw_ctx, &label_dsc, &label_sel_area, opt_start, NULL);
                }
            } else {
...
static void draw_label(lv_event_t * e)
{
    ...
    if(_lv_area_intersect(&clip2, draw_ctx->clip_area, &clip2)) {
        lv_roller_t * roller_data = (lv_roller_t *)roller;
        const lv_area_t * clip_area_ori2 = draw_ctx->clip_area;
        draw_ctx->clip_area = &clip2;

        if (roller_data->option_chr) {
            lv_coord_t y1_ori = label_obj->coords.y1;
            const char *opt_start = get_option_start(roller, &label_draw_dsc, &label_obj->coords, &clip2);
            if(opt_start) {
                lv_draw_label(draw_ctx, &label_draw_dsc, &label_obj->coords, opt_start, NULL);
            }

            label_obj->coords.y1 = y1_ori;
        } else {
...
#if LV_ROLLER_OPTION_HALF_SPACE
        const lv_font_t * font = lv_obj_get_style_text_font(obj, LV_PART_MAIN);
        lv_coord_t line_space = lv_obj_get_style_text_line_space(obj, LV_PART_MAIN);
        lv_coord_t font_h              = lv_font_get_line_height(font);
        lv_coord_t label_unit = font_h + line_space;
        lv_coord_t label_ofs;

        if(roller->moved == 0) {
            lv_point_t p;
            lv_indev_get_point(indev, &p);
            label_ofs = p.y - label->coords.y1;
        }
        else {
            ...
            label_ofs = mid - label_y1;
        }

        label_ofs -= font_h + line_space / 2;
        if(label_ofs <= 0) {
            new_opt = 0;
        }
        else {
            new_opt = 1 + label_ofs / label_unit;
            if(new_opt >= roller->option_cnt) new_opt = roller->option_cnt - 1;
        }
...
#endif /* LV_ROLLER_OPTION_HALF_SPACE */
                                     ...
                                     lv_area_t *coords, const lv_area_t *clip)
{
    ...
    lv_coord_t line_h = font_h + label_dsc->line_space;
    lv_coord_t y_off = clip->y1 - coords->y1;
    lv_coord_t opt_idx = y_off / line_h;
    lv_coord_t opt_y_off = opt_idx * line_h;
    if (y_off - opt_y_off >= font_h) {
        opt_y_off += line_h;
        opt_idx++;
    }

    coords->y1 += opt_y_off;
    if(opt_idx >= 0 && opt_idx < roller_data->option_cnt) {
        return roller_data->option_chr[opt_idx];
    }

...
/* Sort and join the y-intersectant inv areas to avoid screen tearing */
#define LV_INV_AREAS_SORT_Y 1
...
#if LV_INV_AREAS_SORT_Y
static void _lv_inv_area_insert(lv_disp_t * disp, const lv_area_t * area_p, const lv_area_t *scr_area)
{
    int16_t join = -1;
    int16_t i, j;

    /* Find the area with the smallest y coordinate.
     * The areas are arranged by y coordinate from top to bottom.
     */
    for(i = 0; i < disp->inv_p; i++) {
        if(area_p->y1 <= disp->inv_areas[i].y1)
            break;
    }

    /* Join the y-coord intersectant areas */
    if(i > 0 && (area_p->y1 <= disp->inv_areas[i - 1].y2)) {
        join = i - 1;
    } else if (i < disp->inv_p && (area_p->y2 >= disp->inv_areas[i].y1)) {
        join = i;
    }

    if(join >= 0) {
        lv_area_t *area_joined = &disp->inv_areas[join];
        int16_t inv_p = disp->inv_p;

        _lv_area_join(area_joined, area_joined, area_p);

        for(i = join + 1; i < inv_p; i++) {
            if(area_joined->y2 < disp->inv_areas[i].y1) {
                break;
            }

            _lv_area_join(area_joined, area_joined, &disp->inv_areas[i]);
            disp->inv_p--;
        }

        if(inv_p != disp->inv_p) {
            for(j = i; j < inv_p; j++) {
                lv_area_copy(&disp->inv_areas[join + 1], &disp->inv_areas[j]);
                join++;
            }
        }
    } else {
        if(disp->inv_p < LV_INV_BUF_SIZE) {
            for(j = disp->inv_p; j > i; j--) {
                lv_area_copy(&disp->inv_areas[j], &disp->inv_areas[j - 1]);
            }

            disp->inv_p++;
            lv_area_copy(&disp->inv_areas[i], area_p);
...
#if 0
    LV_LOG_WARN("%s: area_p (%d %d %d %d)\n", __func__, area_p->x1, area_p->y1, area_p->x2, area_p->y2);
    for(i = 0; i < disp->inv_p; i++) {
        area_p = &disp->inv_areas[i];
        LV_LOG_WARN("[%d] (%d %d %d %d)\n", i, area_p->x1, area_p->y1, area_p->x2, area_p->y2);
    }
#endif
}
#endif /* LV_INV_AREAS_SORT_Y */
...
#if LV_INV_AREAS_SORT_Y
     _lv_inv_area_insert(disp, &com_area, &scr_area);
#else
...
void _lv_disp_refr_timer(lv_timer_t * tmr)
{
    ...
    /*Do nothing if refr paused*/
    if(disp_refr->driver->refr_paused) {
        REFR_TRACE("finished");
        return;
    }
    ...
    LV_STRACE_U32(LV_STRACE_ID_REFR_TASK, (uint32_t)disp_refr);
    LV_STRACE_U32(LV_STRACE_ID_UPDATE_LAYOUT, (uint32_t)disp_refr);
    ...
    LV_STRACE_CALL(LV_STRACE_ID_UPDATE_LAYOUT);
    ...
    if(disp_refr->inv_p != 0) {
        ...
        if(disp_refr->driver->direct_mode && disp_refr->driver->draw_buf->buf_cnt == 2) {
    ...
    LV_STRACE_CALL(LV_STRACE_ID_REFR_TASK);
...
static void refr_sync_areas(void)
{
    ...
    if(disp_refr->driver->draw_buf->buf_cnt != 2) return;
    ...
    void * buf_on_screen = disp_refr->driver->draw_buf->buf_act == disp_refr->driver->draw_buf->bufs[0]
                           ? disp_refr->driver->draw_buf->bufs[1]
                           : disp_refr->driver->draw_buf->bufs[0];
...
static void refr_area_part(lv_draw_ctx_t * draw_ctx)
{
    ...
    if(disp_refr->driver->render_area_start_cb) {
        disp_refr->driver->render_area_start_cb(disp_refr->driver, draw_ctx->clip_area);
    }
...
#else
    LV_STRACE_VOID(LV_STRACE_ID_WAIT);
    while(draw_buf->rendering_idx == draw_buf->flushing_idx) {
        if(disp_refr->driver->wait_cb) disp_refr->driver->wait_cb(disp_refr->driver);
    }
    LV_STRACE_CALL(LV_STRACE_ID_WAIT);
...
static void draw_buf_rotate(lv_area_t * area, lv_color_t * color_p)
{
    ...
    if(drv->rotated == LV_DISP_ROT_180) {
        ...
        while(row < area_h) {
            ...
            draw_buf->rendering_idx++;

            while(lv_disp_flush_is_finished(drv) == false) {
                if(drv->wait_cb) drv->wait_cb(drv);
            }

            /*Decrease rendering_idx since it draw_buf_flush() will increase it */
            if(row + height >= area_h) draw_buf->rendering_idx--;
...
static void draw_buf_flush(lv_disp_t * disp)
{
    ...
    /*Only flush the rendered content to the display for compability*/
    lv_draw_ctx_t * draw_ctx = disp->driver->draw_ctx;
    if(flushing_last && draw_ctx->wait_for_finish) draw_ctx->wait_for_finish(draw_ctx);

    if(disp->driver->flush_cb) {
        /*Rotate the buffer to the display's native orientation if necessary*/
        if(disp->driver->rotated != LV_DISP_ROT_NONE && disp->driver->sw_rotate) {
            /*Actions: must wait for finish for sw rotate */
            if(draw_ctx->wait_for_finish) draw_ctx->wait_for_finish(draw_ctx);
...
#else
    if(!disp->driver->direct_mode || flushing_last) {
        draw_buf->rendering_idx++;
        draw_buf->buf_act = draw_buf->bufs[draw_buf->rendering_idx % draw_buf->buf_cnt];
    }
#endif
}

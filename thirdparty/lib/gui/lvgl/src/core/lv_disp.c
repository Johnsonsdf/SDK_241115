...
static void set_x_zoom_anim(void * obj, int32_t v);
static void set_zoom_anim(void * obj, int32_t v);
...
void lv_scr_load_anim(lv_obj_t * new_scr, lv_scr_load_anim_t anim_type, uint32_t time, uint32_t delay, bool auto_del)
{
    ...
    if (anim_type >= LV_SCR_LOAD_ANIM_ZOOM_LEFT && anim_type <= LV_SCR_LOAD_ANIM_ZOOM_OUT) {
        lv_obj_set_style_transform_pivot_x(new_scr, lv_disp_get_hor_res(d) / 2, 0);
        lv_obj_set_style_transform_pivot_y(new_scr, lv_disp_get_ver_res(d) / 2, 0);
        lv_obj_set_style_transform_pivot_x(d->act_scr, lv_disp_get_hor_res(d) / 2, 0);
        lv_obj_set_style_transform_pivot_y(d->act_scr, lv_disp_get_ver_res(d) / 2, 0);
    }
    ...
    switch(anim_type) {
        ...
        case LV_SCR_LOAD_ANIM_OUT_BOTTOM:
            ...
            lv_anim_set_values(&a_old, 0, lv_disp_get_ver_res(d));
            break;
        case LV_SCR_LOAD_ANIM_ZOOM_LEFT:
            lv_anim_set_exec_cb(&a_new, set_x_zoom_anim);
            lv_anim_set_values(&a_new, lv_disp_get_hor_res(d), 0);

            lv_anim_set_exec_cb(&a_old, set_x_zoom_anim);
            lv_anim_set_values(&a_old, 0, -lv_disp_get_hor_res(d));
            break;
        case LV_SCR_LOAD_ANIM_ZOOM_RIGHT:
            lv_anim_set_exec_cb(&a_new, set_x_zoom_anim);
            lv_anim_set_values(&a_new, -lv_disp_get_hor_res(d), 0);
            ...
            lv_anim_set_values(&a_old, 0, lv_disp_get_hor_res(d));
            break;
        case LV_SCR_LOAD_ANIM_ZOOM_IN:
            lv_anim_set_exec_cb(&a_new, set_zoom_anim);
            lv_anim_set_values(&a_new, LV_IMG_ZOOM_NONE / 8, LV_IMG_ZOOM_NONE);
            break;
        case LV_SCR_LOAD_ANIM_ZOOM_OUT:
            lv_anim_set_exec_cb(&a_old, set_zoom_anim);
            lv_anim_set_values(&a_old, LV_IMG_ZOOM_NONE, LV_IMG_ZOOM_NONE / 8);
...
static void set_x_zoom_anim(void * obj, int32_t v)
{
    ...
    if (v < 0) {
        zoom = LV_SCR_LOAD_ANIM_ZOOM_MIN + (LV_IMG_ZOOM_NONE - LV_SCR_LOAD_ANIM_ZOOM_MIN) *
                (lv_disp_get_hor_res(d) + v) / lv_disp_get_hor_res(d);
    }
    else if (v > 0) {
        zoom = LV_SCR_LOAD_ANIM_ZOOM_MIN + (LV_IMG_ZOOM_NONE - LV_SCR_LOAD_ANIM_ZOOM_MIN) *
                (lv_disp_get_hor_res(d) - v) / lv_disp_get_hor_res(d);
...
static void set_zoom_anim(void * obj, int32_t v)
{
    /*Invalidate the whole display for zoom out*/
    if (v < LV_IMG_ZOOM_NONE) {
        lv_disp_t * d = lv_obj_get_disp(obj);

        lv_area_t scr_area;
        scr_area.x1 = 0;
        scr_area.y1 = 0;
        scr_area.x2 = lv_disp_get_hor_res(d) - 1;
        scr_area.y2 = lv_disp_get_ver_res(d) - 1;

        _lv_inv_area(d, &scr_area);
    }
...
static bool is_out_anim(lv_scr_load_anim_t anim_type)
{
    return anim_type == LV_SCR_LOAD_ANIM_FADE_OUT  ||
           ...
           anim_type == LV_SCR_LOAD_ANIM_ZOOM_OUT;
}

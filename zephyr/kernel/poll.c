...
static inline bool is_condition_met(struct k_poll_event *event, uint32_t *state)
{
	...
	case K_POLL_TYPE_MSGQ_DATA_AVAILABLE:
		...
		break;
	case K_POLL_TYPE_MBOX_DATA_AVAILABLE:
		if (k_mbox_get_pending_msg_cnt(event->mbox, _current) > 0) {
			*state = K_POLL_STATE_MBOX_DATA_AVAILABLE;
			return true;
		}
		break;
				 ...
				 struct z_poller *poller)
{
	...
	case K_POLL_TYPE_MSGQ_DATA_AVAILABLE:
		...
		break;
	case K_POLL_TYPE_MBOX_DATA_AVAILABLE:
		__ASSERT(event->mbox != NULL, "invalid mail box\n");
		add_event(&event->mbox->poll_events, event, poller);
		break;
	case K_POLL_TYPE_IGNORE:
		/* nothing to do */
		break;
				...
				int num_events, k_timeout_t timeout)
{
	...
	for (int i = 0; i < num_events; i++) {
		...
		case K_POLL_TYPE_MSGQ_DATA_AVAILABLE:
			...
			break;
		case K_POLL_TYPE_MBOX_DATA_AVAILABLE:
			Z_OOPS(Z_SYSCALL_OBJ(e->mbox, K_OBJ_MBOX));
			break;
		default:
			ret = -EINVAL;
			goto out_free;
...
void z_handle_obj_poll_thread_events(sys_dlist_t *events, uint32_t state, k_tid_t tid)
{
	struct k_poll_event *poll_event;

	poll_event = (struct k_poll_event *)sys_dlist_peek_head(events);
	while (poll_event != NULL) {
		if (poller_thread(poll_event->poller) == tid) {
			sys_dlist_remove(&poll_event->_node);
			(void) signal_poll_event(poll_event, state);
			break;
		}
		poll_event = (struct k_poll_event *)sys_dlist_peek_next(events, &poll_event->_node);
	}
}

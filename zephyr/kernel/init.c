...
#else
__in_section_unique(ram.noinit.stack) struct z_thread_stack_element __aligned(Z_KERNEL_STACK_OBJ_ALIGN) \
		  z_idle_stacks[CONFIG_MP_NUM_CPUS][CONFIG_IDLE_STACK_SIZE];
...
void z_bss_zero(void)
{
	...
	if (__sensor_bss_size > 0) {
		(void)memset(__sensor_bss_start, 0, (uint32_t)__sensor_bss_size);
	}
#endif
...
#endif
	if(!soc_pmu_lowpower_check()){
		z_sys_init_run_level(_SYS_INIT_LEVEL_POWER_MAX);
	}
...
FUNC_NORETURN void z_cstart(void)
{
	...
	stack_guard = (uintptr_t)0xdeadbeef;

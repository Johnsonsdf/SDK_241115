...
void arch_irq_unlock(unsigned int flags)
{
	uint32_t delta_cycles;

    if(flags == 0){
        delta_cycles = RUNNING_CYCLES(k_cycle_get_32(), disable_irq_timer);

        if(delta_cycles > CONFIG_DISABLE_IRQ_MAX_LATENCY_CYCLES){
		printk("\n\n!!! 0x%p disable irq %d(us) larger than %d(us)\n\n", __builtin_return_address(0),
				delta_cycles / CONFIG_HOSC_CLK_MHZ, CONFIG_DISABLE_IRQ_MAX_LATENCY_CYCLES / CONFIG_HOSC_CLK_MHZ);
		dump_stack();
        }
    }
    _arch_irq_unlock(flags);
}

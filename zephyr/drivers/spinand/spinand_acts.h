...
struct spinand_operation_api {
    ...
    uint32_t (*read_chipid)(struct spinand_info *sni);
    int (*read)(struct spinand_info *sni, uint32_t addr, void *buf, uint32_t len);
    int (*write)(struct spinand_info *sni, uint32_t addr, const void *buf, uint32_t len);
    int (*erase)(struct spinand_info *sni, uint32_t addr, uint32_t len);
    ...
    uint8_t (*spinand_set_feature) (struct spinand_info *sni, uint8_t feature, uint8_t setting);
    uint8_t (*spinand_get_version) (struct spinand_info *sni, char *version);	

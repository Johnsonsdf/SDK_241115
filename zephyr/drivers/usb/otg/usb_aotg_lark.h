...
/**
 * @file
 * @brief USB Actions OTG controller (lark) driver private definitions
 *
 * This file contains the Actions OTG USB controller (lark) driver private
 * definitions.
 */
...
/* FIFO Clock */
#define USB_MEM_CLK	CMU_MEMCLKSRC1
...
#define EP4OUT_FIFO_START	(NORMAL_FIFO_START + NORMAL_FIFO_BLOCK * 4)
#define EP4IN_FIFO_START	(NORMAL_FIFO_START + NORMAL_FIFO_BLOCK * 6)
...
static inline void usb_aotg_disable_specific(void)
{
	...
	/* Turn USB controller power off */
	usb_write8(USB_VDD, usb_read8(USB_VDD) & (~BIT(USB_VDD_EN)));
}
...
static inline int aotg_dc_fifo_enable(void)
{
	...
	usb_set_bit32(USB_MEM_CLK, USB_RAM_CLKEN_BIT);
...
static inline int aotg_dc_fifo_disable(void)
{
	...
	usb_clear_bit32(USB_MEM_CLK, USB_RAM_CLKEN_BIT);

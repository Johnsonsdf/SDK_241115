...
static void cw6305_close_vsys(void * arg)
{
	bool result;
	unsigned long addr;
	uint8_t value;
    ...
    LOG_INF("close ext charger VSYS...\n");
    bat_charge->ext_charger_debug_open = 1;
    addr = 0x7;
    value = 0xd9;
    result = _cw6305_write_bytes(addr, &value, 1);
...
static int cmd_enter_shipmode(const struct shell *shell,
			      size_t argc, char **argv)
{
	LOG_INF("shell cmd: enter shipmod\n");
	sys_pm_shimode_register_callback(cw6305_close_vsys, NULL);
	sys_pm_shipmode();    
    return 0;
}

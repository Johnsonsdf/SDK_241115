...
void bat_charge_status_proc(void)
{
    ...
    if ((bat_charge->bat_charge_current_state == BAT_CHG_STATE_CHARGE) && (bat_charge->bat_real_volt >= bat_charge->configs.cfg_charge.Charge_Stop_Voltage)) {
        //full
        bat_charge->bat_charge_current_state = BAT_CHG_STATE_FULL;
    }

    if ((bat_charge->bat_charge_current_state == BAT_CHG_STATE_FULL) && (bat_charge->bat_real_volt <= bat_charge->configs.cfg_charge.Recharge_Voltage)) {
        //recharge
        bat_charge->bat_charge_current_state = BAT_CHG_STATE_CHARGE;
        ext_charger_enable();
    }

...
#define BAT_CALIBRATION_VOL (4350)
...
#if (BAT_CALIBRATION_VOL == 4350)
#define CHARGE_FULL_STOP_VOL      4318      //CV-40mv
#endif
...
enum CFG_TYPE_CHARGE_VOLTAGE
{
    ...
    CHARGE_VOLTAGE_4_32_V = 0x26,    // 4.32v
...
#define BAT_CHARGE_CFG_DEFAULT { \
				...
				PRECHARGE_STOP_3_0_V, PRECHARGE_CURRENT_10_PERCENT, 4275, \
				60, 300, 420, 10, 300, \
...
#if (BAT_CALIBRATION_VOL == 4350)
#define BAT_CHARGE_CFG_DEFAULT { \
				BAT_BACK_CHARGE_MODE, CHARGE_CURRENT_250_MA, CHARGE_VOLTAGE_4_32_V, \
				CHARGE_STOP_CURRENT_10_PERCENT, CHARGE_FULL_STOP_VOL,\
				PRECHARGE_STOP_3_0_V, PRECHARGE_CURRENT_10_PERCENT, 4275, \
				60, 300, 420, 10, 300, \
				NO, NTC_THERMISTOR_10K, NTC_PULLUP_INTERNAL}
...
#define BAT_LEVEL_CFG_DEFAULT { \
				{3450, 3740, 3770, 3797, 3830, 3900, 3958, 4024, 4137, 4263, 4325}}
...
static int _cw6305_set_termination(uint8_t enable)
{
    int ret = 0;
    uint8_t reg_val;
...
static void _cw6305_irq_callback(const struct device *port, struct gpio_callback *cb, uint32_t pins)
{
    ...
    if (irq_timer == 0) {
	    irq_timer = k_cycle_get_32();
    } else {
        if (k_cyc_to_ms_floor32(k_cycle_get_32() - irq_timer) < 50) {
            //50ms debounce time.
            return;
        } else {
            irq_timer = 0;
...
static int _cw6305_init(void)
{
	...
	if(configs->cfg_charge.NTC_Func_Enable == NO) {
		_cw6305_set_irq_enable(MASK_CHG_OV|MASK_BAT_OT|MASK_BAT_UT, \
			MASK_CHG_DET|MASK_CHG_REMOVE|MASK_CHG_FINISH);
	} else {
		_cw6305_set_NTC();
		_cw6305_set_irq_enable(MASK_CHG_OV, \
			MASK_CHG_DET|MASK_CHG_REMOVE|MASK_CHG_FINISH|MASK_BAT_OT);
	}

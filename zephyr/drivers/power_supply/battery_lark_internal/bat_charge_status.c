...
bool bat_charge_check_stop_current(void)
{
	...
	charge_current_ma = bat_charge_get_current_ma(bat_charge->configs.cfg_charge.Charge_Current) * (p->percent) / 100;
	LOG_INF("check percent: %d%%, current: %dma", p->percent, charge_current_ma);
#endif
    ...
    if (bat_charge->configs.cfg_charge.Charge_Stop_Current ==
        ...
        if (p->percent < 20) {
			LOG_INF("stop by current less 20 percent!");
            return true;
        }
    } else if (bat_charge->configs.cfg_charge.Charge_Stop_Current ==
                CHARGE_STOP_CURRENT_5_PERCENT) {
        if (p->percent < 5) {
			LOG_INF("stop by current less 5 percent!");
            return true;
        }
    } else {
        if (adjust_current->is_valid &&
            p->percent < adjust_current->stop_current_percent) {
            LOG_INF("stop by current less mA: 0x%x -- %d", sys_read32(CHG_CTL_SVCC), adjust_current->stop_current_percent);
            return true;
        }
    }
#else
...
void bat_charge_state_charge(int bat_volt)
{
    ...
    if (bat_charge->state_timer_count < period) {
		...
		/* check ovp each 10s */
		if	((bat_charge->state_timer_count %
                (10000 / BAT_CHARGE_DRIVER_TIMER_MS)) == 0)	{
...
static void bat_charge_cfg_batadc(void)
{
	...
	//LOG_INF("pmuadc_ctl: 0x%x", sys_read32(PMUADC_CTL));
#endif	
}

static void bat_charge_restore_batadc(void)
{
#ifndef CONFIG_ADC_ACTS_ALWAYS_ON
	uint32_t reg = sys_read32(PMUADC_CTL);

	//int mask
	sys_write32(0, PMUADC_CTL+0x04);
...
void bat_charge_update_real_voltage(void)
{
	...
	if(bat_charge->bat_charge_state != BAT_CHG_STATE_PRECHARGE
		...
		bat_charge_cfg_batadc();
		
		bat_adc_value = bat_adc_get_sample_direct();
		if(bat_adc_value > 0) {

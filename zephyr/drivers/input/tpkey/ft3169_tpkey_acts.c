...
LOG_MODULE_REGISTER(tpkey, LOG_LEVEL_INF);
					...
					uint8_t num_msgs, bool is_err)
{
	...
	val.point.loc_x = ((msgs->buf[2] & 0x0F) << 8) + msgs->buf[3];
	val.point.loc_y = ((msgs->buf[4] & 0x0F) << 8) + msgs->buf[5];
	val.point.pessure_value = (((msgs->buf[2] >> 6) & 0x03) == 1) ? 0 : 1;
	val.point.gesture = 0;
	tpkey_put_point(&val, timestamp);

	LOG_DBG("pressed %d, loc (%d %d)\n",

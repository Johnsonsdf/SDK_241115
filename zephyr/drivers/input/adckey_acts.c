...
static void __adckey_acts_disable(const struct device *dev, int is_stop);
...
int adckey_pm_control(const struct device *dev, enum pm_device_action action)
{
    struct acts_adckey_data *adckey = dev->data;
    ...
    switch (action)
    {
        ...
        case PM_DEVICE_ACTION_SUSPEND:
            if(adckey->state)
            {
                LOG_ERR("adckey busy, cannot suspend");
                return -1;
            }
            break;
        default:

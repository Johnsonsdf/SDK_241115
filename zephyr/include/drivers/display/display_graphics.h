 ...
 *
 */
typedef enum display_blending_type {
	/* no blending */
...
typedef struct display_matrix {
	union {
		...
		int32_t m[2][3]; /*! The 2x3 matrix is in [row][column] order. */
	};
} display_matrix_t;
...
#define DISPLAY_COLOR_MAKE(red, green, blue, alpha) \
...
{
	return (display_color_t) DISPLAY_COLOR_MAKE(r, g, b, a);
}
...
int display_buffer_fill_image(const display_buffer_t *buffer, const void *data,
		uint32_t data_stride_bytes, int pad_byte);
...
/**
 * @brief Return pointer to the buffer at the given coordinates
 *
 * Only valid for buffers whose pixel format's bits per pixel not less than 8
 *
 * @param buffer pointer to structure display_buffer
 * @param x X coordinate
 * @param y Y coordinate
 *
 * @retval pointer to the buffer at the given coordinates
 */

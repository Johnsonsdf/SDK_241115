...
//read:
	...
	if (flash_read(spinand_disk, (uint64_t)start_sector<<9, buff, (uint64_t)sector_count << 9) != 0) {
		return -EIO;
	}
	return 0;
}
...
//write:
   // start_sector += boot_sector;
	if (flash_write(spinand_disk, (uint64_t)start_sector << 9, buff, (uint64_t)sector_count << 9) != 0) {
		return -EIO;
	}
...
int spinand_disk_ioctl(struct disk_info *disk, uint8_t cmd, void *buff)
{
	...
	case DISK_IOCTL_CTRL_SYNC:
        //flush data to spinand.
        flash_flush((struct device *)spinand_disk, true);
		break;
...
#endif
	if (parti != NULL) {
		if (partition_is_used_sector(parti)) {
			disk_spinand_mass.sector_offset = parti->offset;
			disk_spinand_mass.sector_cnt = (parti->size);
		} else {

 ...
 * @return device: storage device
 */
int coredump_region_init(const struct device *dev)
{
	struct device *storage_dev;
	const struct partition_entry *coredump_part;

	LOG_INF("init coredump config");
	...
	if (coredump_part) {
		...
		LOG_INF("coredump:0x%x,0x%x\n", backend_ctx.base_addr, backend_ctx.total_size);
	} else {
		LOG_ERR("coredump partition NOT find");
	}
...
static void coredump_flash_backend_start(void)
{
	...
	if(coredump_get_save_to_region_flag()){
		LOG_ERR("coredump already saved!");
		return;
	}
...
#define TMP_BUF_SIZE  512
	 ...
	 * being written.
	 */
	copy_sz = TMP_BUF_SIZE;
	while (remaining > 0) {
		if (remaining < TMP_BUF_SIZE) {
			copy_sz = remaining;
		}

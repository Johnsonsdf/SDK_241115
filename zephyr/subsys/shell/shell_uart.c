		 ...
		 const void *data, size_t length, size_t *cnt)
{
	...
	*cnt = length;
	...
	if (sh_uart->ctrl_blk->callback) {
		sh_uart->ctrl_blk->callback(data8, length, sh_uart->ctrl_blk->user_data);
	}
...
int shell_uart_register_output_callback(shell_uart_callback_t callback, void *user_data)
{
	const struct shell_uart *sh_uart = shell_uart.iface->ctx;

	sh_uart->ctrl_blk->callback = callback;
	sh_uart->ctrl_blk->user_data = user_data;

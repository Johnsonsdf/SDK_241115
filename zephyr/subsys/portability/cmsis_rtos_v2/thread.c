			 ...
			 const osThreadAttr_t *attr)
{
...
#endif
	void *stack_in_heap = NULL;
	void *tid_in_heap = NULL;
...
#else
	tid_in_heap = k_malloc(sizeof(struct cv2_thread));
	//printk("tid: %p %d\r\n", tid_in_heap, sizeof(struct cv2_thread));
	tid = (struct cv2_thread *)tid_in_heap;
	if (tid == NULL) {
		return NULL;
	}
#endif
	...
	if ((attr->stack_mem == NULL) && (attr->stack_size > 0)) {
		stack_in_heap = k_aligned_alloc(ARCH_STACK_PTR_ALIGN, attr->stack_size);
	...
	tid->tid_in_heap = tid_in_heap;
	tid->stack_in_heap = stack_in_heap;
...
__NO_RETURN void osThreadExit(void)
{
	...
	if (tid->stack_in_heap) {
		k_free(tid->stack_in_heap);
	}
	if (tid->tid_in_heap) {
		k_free(tid->tid_in_heap);
	}
	CODE_UNREACHABLE;
}

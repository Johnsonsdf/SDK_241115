 ...
 */
	...
	#else
		/* End of kernel image */
		static unsigned char __aligned(4) heap_base[CONFIG_NEWLIB_LIBC_MIN_REQUIRED_HEAP_SIZE];
	...
	/* End of the malloc arena is the end of physical memory */
		...
		#define MAX_HEAP_SIZE	(POINTER_TO_UINT(&_heap_sentry) - \
					 HEAP_BASE)
	#else
		/*#define MAX_HEAP_SIZE	(KB(CONFIG_PSRAM_SIZE) - (HEAP_BASE - \
					 CONFIG_PSRAM_BASE_ADDRESS)) */
		#define MAX_HEAP_SIZE	CONFIG_NEWLIB_LIBC_MIN_REQUIRED_HEAP_SIZE
	#endif /* CONFIG_XTENSA */
#endif

 ...
 * Refout:  True
 * Xorout:  0x0000
 * Alias:   CRC-CCITT,CRC-16/CCITT-TRUE,CRC-16/KERMIT,MSB first
 *****************************************************************************/
uint16_t crc16_ccitt(uint16_t seed, const unsigned char* ptr, unsigned int len)
{
    unsigned short current_crc = seed;

    while (len != 0)
    {
        current_crc = ((current_crc << 8) & 0xFFFF)
            ^ crc16_ccitt_table[((current_crc >> 8) & 0xff) ^ (*ptr)];

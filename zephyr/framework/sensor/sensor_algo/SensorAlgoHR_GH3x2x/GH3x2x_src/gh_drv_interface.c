...
/// 
GU8 g_uchGh3x2xChipOperateIndexRealtime = 0;   //0: chip0  1: chip1
GU8 g_uchGh3x2xLogicMasterChipIndex = 0;   //0: default master chip index    1: chip1(for mp test)


GU8 g_uchGhx2xChipSleepFlag = 0x03;  // bit0: 0: chip0 wake up  1: chip0 sleep   bit1: 0: chip1 wake up  1: chip1 sleep

GU8 g_uchChipDivZeroFlag= 0; // 1: divde 0 happen

/// i2c device addr
GU8 g_puchI2cDeviceIdList[2] = {GH3X2X_I2C_DEVICE_ID_BASE, GH3X2X_I2C_DEVICE_ID_BASE};
...
/**
 * @fn     static void GH3X2X_SpiReadRegs(GU16 usRegAddr, GU16 *pusRegValueBuffer, GU16 uchLen)
 *
 ...
 * @return  None
 */
static void GH3X2X_SpiReadRegs(GU16 usRegAddr, GU16 *pusRegValueBuffer, GU16 uchLen)
{
    GU8 uchBuffArr[SPI_READ_REG_BUFFER_LEN] = {0};
...
static void GH3X2X_SpiReadRegsHardwareCs(GU16 usRegAddr, GU16 *pusRegValueBuffer, GU16 uchLen)
{
    GU8 puchWriteBuf[3];
    puchWriteBuf[0] = 0xF0;
    puchWriteBuf[1] = usRegAddr>>8;
...
static void GH3X2X_I2cSendCmd(GU8 uchCmd)
{
    ...
    g_pI2cWriteFunc(g_puchI2cDeviceIdList[g_uchGh3x2xChipOperateIndexRealtime], uchBuffArr, I2C_CMD_BUFFER_LEN);
}
...
static void GH3X2X_I2cWriteReg(GU16 usRegAddr, GU16 usRegValue)
{
    ...
    g_pI2cWriteFunc(g_puchI2cDeviceIdList[g_uchGh3x2xChipOperateIndexRealtime], uchBuffArr, I2C_WRITE_REG_BUFFER_LEN);
}
...
static GU16 GH3X2X_I2cReadReg(GU16 usRegAddr)
{
    ...
    g_pI2cReadFunc(g_puchI2cDeviceIdList[g_uchGh3x2xChipOperateIndexRealtime], uchCmdBuffArr, I2C_READ_REG_ADDR_LEN, uchReadBuffArr, I2C_READ_REG_DATA_LEN);
    return GH3X2X_MAKEUP_WORD(uchReadBuffArr[INTERFACE_INDEX_0_BUFFER], uchReadBuffArr[INTERFACE_INDEX_1_BUFFER]);
}

/**
 * @fn     static void GH3X2X_I2cReadRegs(GU16 usRegAddr, GU16 *pusRegValueArr, GU16 uchLen)
 *
 ...
 * @return  None
 */
static void GH3X2X_I2cReadRegs(GU16 usRegAddr, GU16 *pusRegValueArr, GU16 uchLen)
{
    GU8 uchBuffArr[I2C_READ_REG_ADDR_LEN] = {0};

    uchBuffArr[INTERFACE_INDEX_0_BUFFER] = GH3X2X_GET_HIGH_BYTE_FROM_WORD(usRegAddr);
    uchBuffArr[INTERFACE_INDEX_1_BUFFER] = GH3X2X_GET_LOW_BYTE_FROM_WORD(usRegAddr);
    g_pI2cReadFunc(g_puchI2cDeviceIdList[g_uchGh3x2xChipOperateIndexRealtime], uchBuffArr, I2C_READ_REG_ADDR_LEN, \
...
static void GH3X2X_I2cReadFifo(GU8 *puchDataBuffer, GU16 usLen)
{
    ...
    g_pI2cReadFunc(g_puchI2cDeviceIdList[g_uchGh3x2xChipOperateIndexRealtime], uchCmdBuffArr, I2C_READ_REG_ADDR_LEN, puchDataBuffer, usLen);
}
...
void GH3X2X_SendCmd(GU8 uchCmd)
{
    ...
    {
        GH3X2X_DEBUG_LOG_PARAM("communicate interface register error!\r\n");
    }
}


void GH3X2X_TryWakeUp(void)
{
    GU8 uchSleepFlag;
    uchSleepFlag = (g_uchGhx2xChipSleepFlag&(1 << g_uchGh3x2xChipOperateIndexRealtime));
    if(uchSleepFlag)
    {
        g_uchGhx2xChipSleepFlag &= (~(1 << g_uchGh3x2xChipOperateIndexRealtime));  //clear sleep flag
 ...
 * @return  None
 */
void GH3X2X_ClearChipSleepFlag(GU8 uchChipIndex)
{
    g_uchGhx2xChipSleepFlag &= (~(1 << uchChipIndex)); 
}
 ...
 * @return  None
 */
void GH3X2X_SetChipSleepFlag(GU8 uchChipIndex)
{
    g_uchGhx2xChipSleepFlag |= ((1 << uchChipIndex)); 
}

#if GH3X2X_DIVIDE_ZERO_FLAG_UPLODE_EN
void GH3X2X_SetDivZeroFlag()
{
    g_uchChipDivZeroFlag = 1;
}

void GH3X2X_ClearDivZeroFlag()
{
    g_uchChipDivZeroFlag = 0;
    GOODIX_PLATFORM_INT_DIVIDE_ZERO_ENABLE_ENTITY();
}
#endif
                                    ...
                                    void (*pSpiCsCtrlFunc)(GU8 uchCsLevelHigh))
{
    ...
    {
        GH3X2X_DEBUG_LOG_PARAM("spi register param error!\r\n");
    }
    return chRet;
}
                                                        ...
                                                        GU8 uchReadBytesArr[], GU16 usMaxReadLen))
{
    ...
    {
        g_puchI2cDeviceIdList[0] = GH3X2X_I2C_DEVICE_ID_BASE | (uchI2cIdLowTwoBitsSelect << 1);
    ...
    {
        GH3X2X_DEBUG_LOG_PARAM("i2c register param error!\r\n");
    }
    return chRet;
}
...
{
    return g_uchGh3x2xChipOperateIndexRealtime;
}


/**
 * @fn     void GH3X2X_RegisterI2cDeviceChip1(GU8 uchI2cIdLowTwoBitsSelect)
 *
 * @brief  Register chip1 i2c device addr
 *
 * @attention   None
 * @param[in]   uchI2cIdLowTwoBitsSelect    i2c low two bits addr selected, ref EMGh3x2xI2cIdSelect
 * @param[out]  None
 ...
 * @return  None
 */
void GH3X2X_RegisterI2cDeviceChip1(GU8 uchI2cIdLowTwoBitsSelect)
{
    g_puchI2cDeviceIdList[1] = GH3X2X_I2C_DEVICE_ID_BASE | (uchI2cIdLowTwoBitsSelect << 1);
}

void GH3X2X_CascadeOperationMasterChip(void)
{
    g_uchGh3x2xChipOperateIndexRealtime = g_uchGh3x2xLogicMasterChipIndex;
}

void GH3X2X_CascadeOperationSlaverChip(void)
{
    g_uchGh3x2xChipOperateIndexRealtime = 1;
}

void GH3X2X_CascadeSetLogicMasterChip(GU8 uchPhysicsChipIndex)
{
    g_uchGh3x2xLogicMasterChipIndex = uchPhysicsChipIndex;

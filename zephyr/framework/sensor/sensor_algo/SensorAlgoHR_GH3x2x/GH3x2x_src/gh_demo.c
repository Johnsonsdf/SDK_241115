...
void Gh3x2xSetCurrentSlotEnReg(GU32 unFuncMode)
{
    ...
    GH3x2xNewAgcMeanInfoReset(uchSlotEnChange);
...
void Gh3x2xSampleRateUpdate(GU32 unFuncMode)
{
    ...
    for(GU8 uchSlotCnt = 0; uchSlotCnt < 8; uchSlotCnt ++)
    {
        usLastSampleRate[uchSlotCnt] = 32000/(GH3x2xReadReg(GH3X2X_SYS_SAMPLE_RATE_CTRL_REG_ADDR) + 1)/(GH3x2xGetAgcReg(GH3X2X_AGC_REG_SR_MULIPLIPER, uchSlotCnt) + 1);
    }

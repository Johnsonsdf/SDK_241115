...
GU8 hal_gh3x2x_spi_read(GU8 read_buffer[], GU16 length)
{
    ...
    GOODIX_PLATFORM_SPI_READ_ENTITY();
...
void hal_gh3x2x_spi_cs_ctrl(GU8 cs_pin_level)
{
    ...
    GOODIX_PLATFORM_SPI_CS_CTRL_ENTITY();
...
void hal_gh3x2x_reset_pin_ctrl(GU8 pin_level)
{
   ...
   GOODIX_PLATFORM_SPI_RESET_PIN_ENTITY();
}
...
/**************************** WARNNING END*****************************************************/
    ...
    GOODIX_PLATFORM_INT_GET_GS_DATA_ENTITY();
...
{
    sensor_udelay(usUsec);
    // GOODIX_PLATFORM_DELAY_US_ENTITY();
}
...
{
    sensor_mdelay(usMsec);
    // GOODIX_PLATFORM_DELAY_MS_ENTITY();
}
...
{
    GOODIX_PLATFORM_SERIAL_SEND_ENTITY();
}

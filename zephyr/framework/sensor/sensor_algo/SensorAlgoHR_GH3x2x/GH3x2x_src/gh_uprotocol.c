...
static void GH3X2X_UprotocolHandleLoadRegListCmd(GU8 *puchRespondBuffer, GU16 *pusRespondLen)
{
    ...
    GH3X2X_CHIP_WAKEUP_PROTOCOL();
    if (GH3X2X_LoadNewRegConfigArr(pstReglist, uchReglistCnt, 0) == GH3X2X_RET_OK)
    {
...
static void GH3X2X_UprotocolEcgCascadeCmd(GU8 *puchRespondBuffer, GU16 *pusRespondLen)
{
    ...
    /* respond packet format */
    //SET_VAL_VIA_PTR(pusRespondLen, GH3X2X_UprotocolPacketFormat(g_uchPacketCmd, puchRespondBuffer,g_pstGh3x2xProtocolData->puchPacketPayloadArr, UPROTOCOL_CONN_ECG_CLASSIFICATION_LEN));
}

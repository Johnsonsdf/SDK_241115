...
extern unsigned char SL_SC7A20_I2cAddr_Write(unsigned char iic_addr, unsigned char reg, unsigned char dat);
...
/***SL_SC7A20_I2cAddr_Write 函数中， iic_addr:Sensor's i2c address Reg：寄存器地址  dat:写入寄存器的值****************************/
/***SL_SC7A20_I2cAddr_Write 函数 是可以进行单次读或多次连续读取的函数*************************************************************/
...
/****sl_addr_auto=0: 地址不自增，仅对SC7A20E/H有效，应用于单地址读取FIFO数据***/
/****sl_addr_auto=1: 地址自增，仅对SC7A20E/H有效，应用于多地址读取FIFO数据*****/
/****SDO脚接地，推荐关闭SDO内部上拉电阻****************************************/
/****SPI通讯，推荐关闭SDA，SCL内部上拉电阻，SDO内部上拉电阻会自动关闭**********/
/****SC7A20E/SC7A20ET，推荐sl_addr_auto=0，单地址读取可以节约I2C通讯时的功耗***/
/****SC7A20H/SC7A20HT，推荐sl_addr_auto=0，单地址读取可以节约I2C通讯时的功耗***/
...
/***SC7A20H/SC7A20HT:  获取实时数据，且不对FIFO数据产生影响*****/	

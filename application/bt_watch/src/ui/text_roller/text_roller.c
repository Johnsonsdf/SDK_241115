...
static void text_roller_draw_label(lv_obj_t * obj,struct _lv_draw_ctx_t * draw_ctx, lv_draw_label_dsc_t * dsc, lv_area_t * change_area , lv_area_t * static_area ,lv_coord_t text_id);
...
void text_roller_set_visible_row_count(lv_obj_t *obj , uint8_t cnt)
{
	...
	roller->row_cnt = cnt;
	if(roller->text_area)
		roller->text_area = lv_mem_realloc(roller->text_area,sizeof(lv_area_t) * (roller->row_cnt + 1));
	else
		roller->text_area = lv_mem_alloc(sizeof(lv_area_t) * (roller->row_cnt + 1));
	lv_memset_00(roller->text_area,sizeof(lv_area_t) * (roller->row_cnt + 1));
	text_roller_set_show_area(obj);
}
...
void text_roller_set_str(lv_obj_t *obj , char **str, uint32_t str_byte,uint32_t str_num)
{
	...
	{
		if(roller->str)
			roller->str = lv_mem_realloc(roller->str,str_num * str_byte);
		else
			roller->str = lv_mem_alloc(str_num * str_byte);
...
{
	text_roller_t * roller = (text_roller_t *)obj;
	lv_anim_del(roller,text_roller_anim_cb);
}
	...
	case LV_EVENT_DRAW_MAIN:
		{
			...
			lv_draw_rect(draw_ctx, &sel_dsc, &roller->middle_area);
		}
		break;
	...
	case LV_EVENT_DRAW_POST:
		{
			...
			for(; text_area_id < roller->row_cnt + 1 ; text_area_id++)
			{
				...
				text_roller_draw_label(obj , draw_ctx , &label_draw_dsc , &at_area , up_down_area , text_id);
			}
			...
			for(uint8_t i = 0 ; i < 3 ; i++)
			{
				...
				text_roller_draw_label(obj , draw_ctx , &label_draw_dsc , &at_area , &roller->middle_area , text_id);
			}
		}
		break;
...
static void text_roller_draw_label(lv_obj_t * obj,struct _lv_draw_ctx_t * draw_ctx, lv_draw_label_dsc_t * dsc, lv_area_t * change_area , lv_area_t * static_area ,lv_coord_t text_id)
{
	...
	bool clip_ok = _lv_area_intersect(&clipped_area, change_area, static_area);
	if (!clip_ok)
		return;
	lv_area_t *label_area = (lv_area_t *)draw_ctx->clip_area;
	lv_area_t draw_area;
	clip_ok = _lv_area_intersect(&draw_area, &clipped_area, label_area);
	if (!clip_ok)
		return;
	...
	draw_ctx->clip_area = (const lv_area_t *)(&draw_area);
	text_id = text_id_transition(text_id , 0 , roller->str_num);
	if(roller->str_sign)
		lv_draw_label(draw_ctx, dsc , &clipped_area , roller->str + (text_id * roller->str_byte), NULL);
	else
	{
		...
		lv_draw_label(draw_ctx, dsc , &clipped_area ,str, NULL);
	}
	draw_ctx->clip_area = (const lv_area_t *)label_area;
	dsc->ofs_y = ofs_y;
}
...
static void text_roller_destructor(const lv_obj_class_t * class_p, lv_obj_t * obj)
{
	...
	text_roller_t * roller = (text_roller_t *)obj;
	if(roller->text_area)
		lv_mem_free(roller->text_area);
	if(roller->str)
		lv_mem_free(roller->str);
}

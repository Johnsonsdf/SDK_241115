...
void watch_clock_set_time(lv_obj_t * obj, uint8_t hour, uint8_t minute, uint16_t msec)
{
	...
	if (clock->type == ANALOG_CLOCK) {
		simple_img_set_angle(clock->time[CLOCK_HOUR], hour * 300 + minute * 5);
		if (clock->time[CLOCK_SEC]) {
			simple_img_set_angle(clock->time[CLOCK_MIN], minute * 60 + msec / 1000);
			simple_img_set_angle(clock->time[CLOCK_SEC], msec * 6 / 100);
		} else {
			simple_img_set_angle(clock->time[CLOCK_MIN], minute * 60);
		}
	} else {

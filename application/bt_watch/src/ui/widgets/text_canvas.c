...
static lv_img_cf_t get_text_font_img_cf(lv_obj_t * obj, const lv_font_t *font);
#if 0
...
void text_canvas_set_text_fmt(lv_obj_t * obj, const char * fmt, ...)
{
	...
	va_list args;
	va_start(args, fmt);
	canvas->text = _lv_txt_set_text_vfmt(fmt, args);
	va_end(args);
...
static void text_canvas_event(const lv_obj_class_t * class_p, lv_event_t * e)
{
	...
	if (code == LV_EVENT_STYLE_CHANGED) {
		if (canvas->text) {
			LV_LOG_WARN("style changed after text may reduce speed");
			text_canvas_refr_text(obj);
		}
...
static void text_canvas_refr_text(lv_obj_t *obj)
{
	...
	canvas->dsc.header.cf = get_text_font_img_cf(obj, draw_dsc.font);
...
#  endif /* CONFIG_DISPLAY_ENGINE_LARK */
...
#endif /* OPT_LV_TXT_GET_SIZE */
	{
		...
		bool exceeded = lvgl_txt_get_size(&size, canvas_text, draw_dsc.font, draw_dsc.letter_space,
		...
		if (canvas->long_mode == TEXT_CANVAS_LONG_DOT ||
			...
			if (exceeded && lvgl_txt_has_encoded_length(canvas_text, TEXT_CANVAS_DOT_NUM)) {
			...
			if (size.x > txt_w) {
				...
				* (In addition they will result misalignment is this case)*/
				...
				canvas->dsc_w = txt_w & ~canvas->dsc_w_mask;
			}
		...
		const lv_point_t * pos_p, uint32_t letter)
{
	...
	if (cf == LV_IMG_CF_ALPHA_4BIT_LE && bpp == 2) {
		static const lv_opa_t alpha2t4_opa_table[] = { 0, 5, 10, 15 };
	...
	} else if (cf == LV_IMG_CF_ALPHA_8BIT && bpp != 8) {
		...
		for (row = num_rows; row > 0; row--) {
			...
			for (col = num_cols; col > 0; col--) {
				...
				if (letter_px) {
					*draw_p_tmp = opa_table[letter_px];
				}

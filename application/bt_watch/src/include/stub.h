...
typedef struct
{
    uint8_t  magic;               ///< stub magic: 0xAE
    uint8_t  stub_id;             ///< stub connection identification: \ref STUB_CONNECTION_MODE.
    uint8_t  opcode;              ///< operation code
    uint8_t  reserved;
    uint32_t para;
    uint32_t payload_length;      ///< STUB data packet length, byte unit
}stub_cmd_packet_t;
...
typedef struct
{
    uint8_t  magic;               ///< stub magic: 0xAC
    uint8_t  reserved[2];
    uint8_t  status;              ///< stub response status, \ref in STUB_PROTOCOL_STATUS
...
/**
 *  \brief Change tx/rx max payload size for stub communication.
 *

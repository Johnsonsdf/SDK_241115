...
typedef struct
{
    ...
    uint16_t  payload_len;            ///< data length for payload only, not include packet head & crc16
    uint8_t   predefine_command;
    uint8_t   crc8_val;
}usp_packet_head_t;
...
typedef struct
{
    uint8_t   type;
    uint8_t   reserved[3];
    uint8_t   status;                 ///< respond status, \ref USP_PROTOCOL_STATUS
    uint8_t   crc8_val;
}usp_ack_packet_t;
...
typedef struct
{
    ...
    uint8_t    protocol_init_flag:1;
    uint8_t    connected:1;
    uint8_t    opened:1;
    uint8_t    reserved:5;
    // on some channel(such as USB, SPP), use transparent transfer
    uint8_t    transparent;
    uint8_t    transparent_bak;

    uint8_t    out_seq_number:4;
    uint8_t    in_seq_number:4;

    uint8_t    state;
    uint8_t    max_retry_count;
    uint16_t   rx_timeout;

    uint16_t   tx_max_payload_size;
    uint16_t   rx_max_payload_size;
...
typedef struct
{
    int (*read)(uint8_t *data, uint32_t length, uint32_t timeout_ms);
    int (*write)(uint8_t *data, uint32_t length, uint32_t timeout_ms);
    int (*ioctl)(uint32_t mode, void *para1, void *para2);
}usp_phy_interface_t;
 ...
 */
int USP_Protocol_Inquiry(usp_handle_t *usp_handle, uint8_t *payload, uint32_t payload_len, usp_packet_head_t *recv_head);
 ...
 *  \details
 */
int WriteUSPData(usp_handle_t *usp_handle, uint8_t *user_data, uint32_t size);
 ...
 *  \details
 */
int WriteUSPISOData(usp_handle_t *usp_handle, uint8_t *user_data, uint32_t size);
 ...
 *  \details
 */
int ReceiveUSPDataPacket(usp_handle_t *usp_handle, usp_packet_head_t *recv_head, uint8_t *payload, uint32_t size);
 ...
 *  \details
 */
int ReadUSPData(usp_handle_t *usp_handle, uint8_t *user_data, uint32_t size);
...
/**
 *  \brief Change tx/rx max payload size for communication.
 *
 *  \param [in] usp_handle  USP protocol handler.
 *  \param [in] tx_max      max payload size for device send to peer device.
 *  \param [in] rx_max      max payload size for device receive from peer device.
 *  \return     None.
 *
 *  \details
 */

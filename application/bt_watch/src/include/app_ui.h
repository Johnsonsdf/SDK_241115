...
#ifdef CONFIG_VG_LITE
#  define DEF_UI_SWITCH_EFFECT UI_SWITCH_EFFECT_PAGE
#else
#  define DEF_UI_SWITCH_EFFECT UI_SWITCH_EFFECT_SCALE
#endif

#define DEF_UI_SCROLL_EFFECT UI_SCROLL_EFFECT_NONE
...
static inline bool lvgl_click_decision(void)
{
	...
	if(indev)
	{
		if(LV_ABS(indev->proc.types.pointer.scroll_sum.x) <= DEF_CLICK_SCOPE
			&& LV_ABS(indev->proc.types.pointer.scroll_sum.y) <= DEF_CLICK_SCOPE)
		return true;
	}
	return false;
}

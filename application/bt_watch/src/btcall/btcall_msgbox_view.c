...
typedef struct incoming_view_data {
	...
	lv_img_dsc_t incoming_img_dsc_btn[NUM_INCOMING_BTNS];
	lv_img_dsc_t incoming_img_dsc_bmp[NUM_INCOMING_IMGS];
...
typedef struct ongoing_view_data {
	...
	lv_img_dsc_t ongoing_img_dsc_btn[NUM_ONGOING_BTNS];
	lv_img_dsc_t ongoing_img_dsc_bmp[NUM_ONGOING_IMGS];
...
typedef struct outgoing_view_data {
	...
	lv_img_dsc_t outgoing_img_dsc_btn[NUM_OUTGOING_BTNS];
	lv_img_dsc_t outgoing_img_dsc_bmp[NUM_OUTGOING_IMGS];
...
typedef struct thr_way_view_data {
	...
	lv_img_dsc_t thr_way_img_dsc_btn[NUM_3W_BTNS];
...
static void _display_incoming_label(const char *coming_str, uint8_t txt_num);
...
static void _display_ongoing_label(const char *calling_str, uint8_t txt_num);
...
static void _display_outgoing_label(const char *outgoing_str, uint8_t txt_num);
...
static void _display_3way_label(const char *thr_way_str, uint8_t txt_num);
...
static void _reset_btcall_label_style_array(lv_style_t *sty, uint32_t num)
{
	for (int i = num - 1; i >= 0; i--) {
...
static int _display_ongoing_view(btcall_view_data_t *data)
{
	...
	_create_btn_array(data->btcall_bg, ongo_view->ongoing_btn, ongo_view->ongoing_pt_btn,
			...
			_handle_ongoing_btn_event, ongo_view);
	...
	if (data->cur_phone_number) {
		_display_ongoing_label(data->cur_phone_number, TXT_ONGOING_NUM);
	} else {
		_display_ongoing_label("正在通话", TXT_ONGOING_NUM);
	}
	...
	if (data->view_state != BTCALL_3W_STATE) {
		...
		if (data->ongoing_start_time == 0) {
			_display_ongoing_label("00:00", TXT_ONGOING_TIME);
...
static void _exit_ongoing_view(btcall_view_data_t *data)
{
	...
	_reset_btcall_label_style_array(ongo_view->ongoing_style, NUM_ONGOING_TXTS);

	if (ongo_view->timer_ls) {
		lv_timer_del(ongo_view->timer_ls);
...
static void _display_ongoing_label(const char *calling_str, uint8_t txt_num)
{
	ongoing_view_data_t *ongo_view = &p_btcall_view_data->ongoing_view;

	if (!p_btcall_view_data || txt_num >= NUM_ONGOING_TXTS || !calling_str)
		return;
...
{
	char time_buff[12] = { 0 };
	uint32_t second  = (os_uptime_get() - p_btcall_view_data->ongoing_start_time) / 1000;
	...
	_display_ongoing_label(time_buff, TXT_ONGOING_TIME);
}

static void _handle_ongoing_btn_event(lv_event_t * e)
{
	lv_obj_t *obj = lv_event_get_target(e);
	ongoing_view_data_t *ongo_view = lv_event_get_user_data(e);
	...
	if (obj == ongo_view->ongoing_btn[BTN_ONGOING_HAND_UP]) {
		...
		p_btcall_view_data->cur_phone_number = p_btcall_view_data->phone_number;

		if (ret == BTSRV_HFP_STATE_CALL_MULTIPARTY) {
			_display_ongoing_label(p_btcall_view_data->cur_phone_number, TXT_ONGOING_NUM);
...
static int _display_3way_view(btcall_view_data_t *data)
{
	...
	/* display coming info */
	if (data->cur_phone_number) {
		_display_3way_label(data->cur_phone_number, TXT_3W_HOLD_NUM);
	} else {
		_display_3way_label("未知号码", TXT_3W_HOLD_NUM);
	}

	if (data->thr_way_phone_number) {
		_display_3way_label(data->thr_way_phone_number, TXT_3W_ANOTH_NUM);
	} else {
		_display_3way_label("三方号码", TXT_3W_ANOTH_NUM);
	}

	//_display_3way_label("结束并接听", TXT_3W_HUP_ANS);
	//_display_3way_label("保持并接听", TXT_3W_HOLD_ANS);
...
static void _exit_3way_view(btcall_view_data_t *data)
{
	...
	_reset_btcall_label_style_array(thr_way_view->thr_way_style, NUM_3W_TXTS);
...
static void _display_3way_label(const char *thr_way_str, uint8_t txt_num)
{
	thr_way_view_data_t *thr_way_view = &p_btcall_view_data->thr_way_view;

	if (!p_btcall_view_data || txt_num >= NUM_3W_TXTS || !thr_way_str)
		return;
...
static int _display_incoming_view(btcall_view_data_t *data)
{
	...
	_create_btn_array(data->btcall_bg, inc_view->incoming_btn, inc_view->incoming_pt_btn,
			...
			_handle_incoming_btn_event, inc_view);
	/* create coming label */
	...
	/* display coming info */
	_display_incoming_label("拒绝", TXT_REJECT);
	_display_incoming_label("接受", TXT_ACCEPT);

	if (data->cur_phone_number) {
		_display_incoming_label(data->cur_phone_number, TXT_CALL_NUM);
	} else {
		_display_incoming_label(/*"未知号码"*/ "", TXT_CALL_NUM);
	}
...
static void _exit_incoming_view(btcall_view_data_t *data)
{
	...
	_reset_btcall_label_style_array(inc_view->incoming_style, NUM_INCOMING_TXTS);
}

static void _display_incoming_label(const char *coming_str, uint8_t txt_num)
{
	incoming_view_data_t *inc_view = &p_btcall_view_data->incoming_view;

	if (!p_btcall_view_data || txt_num >= NUM_INCOMING_TXTS || !coming_str)
		return;
...
{
	lv_obj_t *obj = lv_event_get_target(e);
	incoming_view_data_t *inc_view = lv_event_get_user_data(e);

	if (!p_btcall_view_data)
		return;

	if (obj == inc_view->incoming_btn[BTN_ACCEPT]) {
		_btcall_view_apply_state_transition(p_btcall_view_data, BTCALL_ONGOING_STATE);
		btcall_accept_call();
...
static int _display_outgoing_view(btcall_view_data_t *data)
{
	...
	/* display coming info */
	if (data->cur_phone_number) {
		_display_outgoing_label(data->cur_phone_number, TXT_OUTGOING_NUM);
	} else {
		_display_outgoing_label(/*"未知号码"*/ "", TXT_OUTGOING_NUM);
	}
...
static void _exit_outgoing_view(btcall_view_data_t *data)
{
	...
	_reset_btcall_label_style_array(outg_view->outgoing_style, NUM_OUTGOING_TXTS);
}

static void _display_outgoing_label(const char *outgoing_str, uint8_t txt_num)
{
	outgoing_view_data_t *outg_view = &p_btcall_view_data->outgoing_view;

	if (!p_btcall_view_data || txt_num >= NUM_OUTGOING_TXTS || !outgoing_str)
		return;
...
static void _exit_siri_view(btcall_view_data_t *data)
{
	...
	if (siri_view->siri_anim) {
		lv_obj_del(siri_view->siri_anim);
	}

	if (siri_view->siri_txt) {
		lv_obj_del(siri_view->siri_txt);
	}
}
...
static void _btcall_view_layout(void)
{
	...
	if (p_btcall_view_data->calling_state == BTCALL_NULL_STATE) {
		...
		if (p_btcall_view_data->calling_pre_state == BTCALL_NULL_STATE) {
			...
			msgbox_cache_paint(BTCALL_MSGBOX_ID, 0);
		}
	}
...
void * btcall_msgbox_popup_cb(uint16_t msgbox_id, uint8_t msg_id, void *msg_data, void *user_data)
{
	btcall_view_data_t *data = user_data;
	int ret = 0;
	...
	if (msg_id == MSG_MSGBOX_POPUP) {
		...
		/* create bg image */
		data->btcall_bg = lv_obj_create((lv_obj_t *)msg_data);
		lv_obj_set_pos(data->btcall_bg, 0, 0);
		lv_obj_set_size(data->btcall_bg, LV_PCT(100), LV_PCT(100));
		lv_obj_set_style_bg_opa(data->btcall_bg, LV_OPA_COVER, LV_PART_MAIN);
		/*set current background clickable to cut out click event send to front bg*/
		lv_obj_add_flag(p_btcall_view_data->btcall_bg, LV_OBJ_FLAG_CLICKABLE);
	}

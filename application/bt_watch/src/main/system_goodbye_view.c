...
static int _goodbye_view_layout(view_data_t *view_data)
{
	...
	lv_anim_set_time(&anim, CONFIG_GOODBYE_ANIM_TIME);
...
int _goodbye_view_handler(uint16_t view_id, uint8_t msg_id, void *msg_data)
{
...
VIEW_DEFINE(goodbye_view, _goodbye_view_handler, NULL, NULL, GOODBYE_VIEW,

...
int USP_Protocol_Inquiry(usp_handle_t *usp_handle, uint8_t *payload, uint32_t length, usp_packet_head_t* recv_head)
{
    int ret = -1;
    ...
    else
    {
        ...
        if ((USP_PROTOCOL_OK == ret) && (NULL != recv_head))
        {
            memcpy(recv_head, &head, USP_PACKET_HEAD_SIZE);
        }

...
#ifdef CONFIG_UI_SWITCH_EFFECT
	ui_switch_effect_set_type(DEF_UI_SWITCH_EFFECT);
#endif
	SYS_LOG_INF("ok\n");
}
...
static int _create_progress_bar(ota_view_data_t *data, lv_obj_t *scr)
{
	...
	lv_style_set_pad_left(&data->style_slider_knob, 10);
	lv_style_set_pad_right(&data->style_slider_knob, 10);
	lv_style_set_pad_top(&data->style_slider_knob, 10);
	lv_style_set_pad_bottom(&data->style_slider_knob, 10);
	...
	lv_obj_clear_flag(data->obj_prg_bar, LV_OBJ_FLAG_CLICKABLE);/*remove click event*/
...
static int _ota_view_layout(view_data_t *view_data)
{
	...
	/* create ota bg image */
	data->obj_ota_bg = lv_img_create(scr);
	...
	lv_obj_clear_flag(data->obj_ota_bg, LV_OBJ_FLAG_SCROLLABLE | LV_OBJ_FLAG_GESTURE_BUBBLE);
...
static int _ota_view_handler(uint16_t view_id, uint8_t msg_id, void * msg_data)
{
...
VIEW_DEFINE(ota_view, _ota_view_handler, NULL, \

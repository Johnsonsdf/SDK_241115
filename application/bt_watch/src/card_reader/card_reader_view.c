...
typedef struct card_reader_view_data {
	...
	lv_img_dsc_t img_bmp[NUM_CARD_READER_IMGS];
...
static int _card_reader_view_handler(uint16_t view_id, uint8_t msg_id, void *msg_data)
{
...
VIEW_DEFINE(card_reader_view, _card_reader_view_handler, NULL, \

...
struct bt_player {
	...
	uint8_t avrcp_playing;
	uint8_t a2dp_playing;
...
static int bt_player_start_play(void)
{
	...
	media_player_fade_in(btplayer->player, 30);
	media_player_play(btplayer->player);
...
static void bt_player_stop_play(void)
{
	...
	media_player_fade_out(btplayer->player, 30);
...
void bt_player_bt_event_proc(struct app_msg *msg)
{
	...
	#endif
		if(!g_bt_player.a2dp_playing) {
			g_bt_player.a2dp_playing = true;
			if (g_bt_player.play_cb && g_bt_player.play_cb->set_status) {
				g_bt_player.play_cb->set_status(g_bt_player.a2dp_playing);
			}
		}
		break;
	}
	case BT_A2DP_STREAM_SUSPEND_EVENT: /* 7 */
	{
		...
		bt_player_stop_play();
		if(g_bt_player.a2dp_playing) {
			g_bt_player.a2dp_playing = false;
			if (g_bt_player.play_cb && g_bt_player.play_cb->set_status) {
				g_bt_player.play_cb->set_status(g_bt_player.a2dp_playing);
			}
		}
	...
	case BT_AVRCP_PLAYBACK_STATUS_CHANGED_EVENT: /* 31 */
	{
		...
		if (param == BT_STATUS_PAUSED) {
			g_bt_player.avrcp_playing = 0;
		}
		else if (param == BT_STATUS_PLAYING) {
			bt_manager_a2dp_check_state();
			g_bt_player.avrcp_playing = 1;
		}

		if (g_bt_player.play_cb && g_bt_player.play_cb->set_status) {
			g_bt_player.play_cb->set_status(g_bt_player.avrcp_playing);
		}
		break;
	}
...
void btmusic_view_input_event_proc(struct app_msg *msg)
{
	...
	case MSG_BT_PLAY_STOP_PLAYER: /* 208 */
	{
		if (g_bt_player.player) {
			bt_player_stop_play();
		}
...
{
	SYS_LOG_INF("start %p %d %d %d", g_bt_player.player, g_bt_player.user_playing,
		g_bt_player.a2dp_playing, g_bt_player.avrcp_playing);

	if (!g_bt_player.user_playing || !g_bt_player.a2dp_playing || !g_bt_player.avrcp_playing) {
		struct app_msg msg = {
			.type = MSG_INPUT_EVENT,
			.cmd = MSG_BT_PLAY_RESUME,
			.reserve = BTMUSIC_PLAYER,
		};
...
{
	return g_bt_player.avrcp_playing? true : false;
}

bool btmusic_get_a2dp_play_state(void)
{
	return g_bt_player.a2dp_playing ? true : false;
}

...
typedef struct digital1_clock_view_data {
	lv_img_dsc_t img_bg;
	lv_img_dsc_t electric_img[ARRAY_SIZE(electric_img_ids)];
	lv_img_dsc_t green_num1_img[ARRAY_SIZE(green_num1_ids)];
	lv_img_dsc_t green_num2_img[ARRAY_SIZE(green_num2_ids)];
	lv_img_dsc_t day_num_img[ARRAY_SIZE(day_num_ids)];
	lv_img_dsc_t week_img[ARRAY_SIZE(week_ids)];
	lv_img_dsc_t white_num_img[ARRAY_SIZE(white_num_ids)];
	lv_img_dsc_t min_num_img[ARRAY_SIZE(min_num_ids)];
...
static int _dynamic_load_pictures(lvgl_res_scene_t *scene,uint32_t id, const uint32_t *pic_ids,
		int images_num, lv_img_dsc_t *images,uint8_t *buf,int buf_cnt)
{
	int ret;
...
int _digital1_clock_view_handler(uint16_t view_id, uint8_t msg_id, void *msg_data)
{

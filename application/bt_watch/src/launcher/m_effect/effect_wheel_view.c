...
static void _effect_wheel_wheel_zoom(void * var, int32_t v)
{
	...
	uint32_t clock_z = data->clock_zoom * v / LV_IMG_ZOOM_NONE;
...
static int _effect_wheel_scene_load_resource(effect_wheel_wheel_scene_data_t *data)
{
	...
	data->clock_zoom = data->effect_wheel_imgs[0].header.w * LV_IMG_ZOOM_NONE / data->effect_wheel_imgs[1].header.w;
...
static int _effect_wheel_wheel_view_layout(view_data_t *view_data)
{
	...
	centre_x = all_x / 2;
	centre_y = all_y / 2;
	centre_z = centre_z * LV_IMG_ZOOM_NONE / data->clock_zoom;

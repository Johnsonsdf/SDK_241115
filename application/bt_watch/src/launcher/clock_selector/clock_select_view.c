...
typedef struct {
	...
	lv_img_dsc_t face_imgs[NUM_CLOCK_IDS];
	lv_coord_t move_x;
	lv_coord_t st_x;
	lv_coord_t end_x;
...
static void _clock_select_move_x(clock_select_scene_data_t * data,lv_coord_t id, lv_coord_t x ,lv_coord_t zoom ,lv_coord_t angle)
{
	...
	uint32_t zoom_w = data->picreg.height * zoom / LV_IMG_ZOOM_NONE;
...
static void _clock_select_move(clock_select_scene_data_t * data,lv_point_t point_v)
{
	...
	if(point_x <= 0 && point_x >= -(data->end_x - data->st_x) * MOVE_COEFFICIENT / 255)
	{
		...
		for(uint16_t i = 0;i<NUM_CLOCK_IDS;i++)
		{
			...
			zoom[i] = LV_IMG_ZOOM_NONE - LV_ABS(num_x[i]) * (LV_IMG_ZOOM_NONE - MIN_ZOOM_VALUE) / DEF_UI_VIEW_WIDTH;
		...
		lv_coord_t x_diff = ((w - LV_ABS(lv_trigo_cos(face_angle[id]/10)) * w / LV_TRIGO_SIN_MAX * zoom[id] / LV_IMG_ZOOM_NONE)>>1);
		...
		for(uint16_t i = 1;i<NUM_CLOCK_IDS;i++)
		{
			if(id - i >= 0)
			{
				lv_coord_t dx = ((w - LV_ABS(lv_trigo_cos(face_angle[id - i]/10)) * w / LV_TRIGO_SIN_MAX * zoom[id - i] / LV_IMG_ZOOM_NONE)>>1);
				x_diff_j += dx;
...
static void _clock_select_touch_event_cb(lv_event_t * e)
{
	...
	if (code == LV_EVENT_PRESSED) {
		...
		lv_anim_del(data,_clock_select_scroll_anim);
	}
		...
		if(residue_move != 0)
		{
			...
			lv_anim_set_time(&a, D_TIMER_SPEED);
...
int clock_selector_view_handler(uint16_t view_id, uint8_t msg_id, void * msg_data)
{
...
VIEW_DEFINE(clock_select3d, clock_selector_view_handler, NULL, NULL, CLOCK_SELECTOR_VIEW,

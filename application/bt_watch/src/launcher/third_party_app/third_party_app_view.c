...
#endif
	lvgx_t *lvgx = lvgx_init();
	lvgx_applet_list(scr);
	lvgx_setup_window(lvgx, scr);
	return 0;
}
...
static int _third_party_app_view_proc_key(lv_obj_t * scr, ui_key_msg_data_t * key_data)
{
...
int _third_party_app_view_handler(uint16_t view_id, view_data_t * view_data, uint8_t msg_id, void * msg_data)
{
	...
	case MSG_VIEW_KEY:
		return _third_party_app_view_proc_key(lv_disp_get_scr_act(view_data->display), msg_data);
	default:
		return 0;
	}
	return 0;

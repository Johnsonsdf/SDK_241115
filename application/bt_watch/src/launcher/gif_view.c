...
static void _gif_btn_evt_handler(lv_event_t *e)
{
	...
	if(gif_img_get_paused(gif_icon))
		gif_img_set_resume(gif_icon);
	else
		gif_img_set_pause(gif_icon);
	printf("_gif_btn_evt_handler %x\r\n",(uint32_t)gif_icon);
}
...
static int _gif_view_layout(view_data_t *view_data)
{
	...
	if(data->git_icon == NULL)
	{
		...
		gif_img_set_src(data->git_icon,(const void * )GIF_PATH);

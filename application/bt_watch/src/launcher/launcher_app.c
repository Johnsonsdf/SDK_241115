...
{
	launcher_app_t *app = launcher_app_get();
	printk("sensor_res_callback evt_id %d------>>>>>>>heart rate val:%f\n", evt_id, res->heart_rate);
...
static int _launcher_app_init(void)
{
	...
	app->scroll_effects_mode = (uint8_t)property_get_int(CFG_SCROLL_MODE, DEF_UI_SCROLL_EFFECT);
#endif
...
static void _launcher_key_event_handle(uint16_t view_id, uint32_t event)
{
	...
	if (event == (KEY_POWER | KEY_TYPE_DOUBLE_CLICK)) {
		printk("KEY_POWER | KEY_TYPE_DOUBLE_CLICK\n");
		return;
	}

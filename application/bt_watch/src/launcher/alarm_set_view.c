...
typedef struct alarm_main_view_data {
	...
	/* lvgl resource */
	lv_img_dsc_t img_dsc_def_main_btn[NUM_ALARM_MAIN_BTNS];
	lv_img_dsc_t img_dsc_sel_main_btn[NUM_ALARM_MAIN_BTNS];

	lv_img_dsc_t img_dsc_off_num[10];
	lv_img_dsc_t img_dsc_on_num[10];
...
typedef struct alarm_sub_view_data {
	...
	/* lvgl resource */
	lv_img_dsc_t img_dsc_sub_btn[NUM_ALARM_SUB_BTNS];
	lv_img_dsc_t img_dsc_ch_num[10];
	lv_img_dsc_t img_dsc_unch_num[10];
...
typedef struct alarm_set_view_data {
	...
	/* lvgl resource */
	lv_img_dsc_t img_dsc_set_bg[NUM_ALARM_SET_BG_IMGS];
...
static void _exit_alarm_main_view_btn(alarm_main_view_data_t *main_view)
{
	...
	for (int i = 0; i < NUM_ALARM_MAIN_BTNS; i++) {
		if (main_view->obj_main_btn[i])
			lv_obj_del(main_view->obj_main_btn[i]);
...
static void _display_alarm_time(alarm_set_view_data_t *data, lv_obj_t *par, uint16_t min, bool h_is_ch, bool m_is_ch)
{
	...
	if (h_is_ch) {
		if (sub_view->obj_sub_h_unch) {
			lv_obj_del(sub_view->obj_sub_h_unch);
		...
		if (sub_view->obj_sub_h_ch) {
			lv_obj_del(sub_view->obj_sub_h_ch);
	...
	if (m_is_ch) {
		if (sub_view->obj_sub_m_unch) {
			lv_obj_del(sub_view->obj_sub_m_unch);
		...
		if (sub_view->obj_sub_m_ch) {
			lv_obj_del(sub_view->obj_sub_m_ch);
...
static void _display_alarm_list_one(alarm_set_view_data_t *data, lv_obj_t *par, uint8_t index)
{
	...
	if (data->tm_state[index] == ALARM_STATE_OFF) {
		...
		if (main_view->obj_tm_hour_on[index]) {
			lv_obj_del(main_view->obj_tm_hour_on[index]);
			main_view->obj_tm_hour_on[index] = NULL;
		}
		if (main_view->obj_tm_hour_off[index])
			lv_obj_del(main_view->obj_tm_hour_off[index]);
		...
		if (main_view->obj_tm_min_on[index]) {
			lv_obj_del(main_view->obj_tm_min_on[index]);
			main_view->obj_tm_min_on[index] = NULL;
		}
		if (main_view->obj_tm_min_off[index])
			lv_obj_del(main_view->obj_tm_min_off[index]);
		...
		if (main_view->obj_main_btn[2 * index])
			lv_obj_del(main_view->obj_main_btn[2 * index]);
...
static int _alarm_set_view_layout(view_data_t *view_data)
{
	...
	/* create bg image */
	data->obj_alarm_bg = lv_img_create(scr);
	...
	/*set current background clickable to cut out click event send to front bg*/
	lv_obj_add_flag(data->obj_alarm_bg, LV_OBJ_FLAG_CLICKABLE);
	lv_obj_clear_flag(data->obj_alarm_bg, LV_OBJ_FLAG_SCROLLABLE | LV_OBJ_FLAG_GESTURE_BUBBLE);
...
static int _alarm_set_view_handler(uint16_t view_id, uint8_t msg_id, void * msg_data)
{
...
VIEW_DEFINE(alarm_set_view, _alarm_set_view_handler, NULL, \

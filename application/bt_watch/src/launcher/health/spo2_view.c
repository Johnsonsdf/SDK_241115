...
static int _spo2_view_layout_update(view_data_t *view_data, bool first_layout)
{
	...
	if(first_layout)
	{
		data->obj_bg = lv_img_create(scr);
		if (data->obj_bg == NULL) {
			goto fail_exit;
		}
	}

	lv_img_set_src(data->obj_bg, &data->img_bg);
...
int _spo2_view_handler(uint16_t view_id, uint8_t msg_id, void *msg_data)
{
...
VIEW_DEFINE(health_spo2, _spo2_view_handler, NULL,

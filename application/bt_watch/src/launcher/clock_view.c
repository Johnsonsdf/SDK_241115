...
typedef struct clock_view_data {
	...
	lv_img_dsc_t *img_stepcount; /* 0~9 */
	lv_img_dsc_t *img_heartrate; /* 0~9 */
	lv_img_dsc_t *img_calories;  /* 0~9 */
	lv_img_dsc_t *img_distance;  /* 0~9 . k m */
	lv_img_dsc_t *img_sleeptime; /* 0~9 h m */
	lv_img_dsc_t *img_battery;   /* 0~9 */
	lv_img_dsc_t *img_temperature; /* 0~9 */
	lv_img_dsc_t img_weather;
...
int _clock_view_select(uint16_t view_id, uint8_t msg_id, void *msg_data)
{
	view_data_t *view_data = msg_data;
	assert(view_id == CLOCK_VIEW);
	...
	case AOD_CLOCK_ID:
	#endif
		return _clock_view_handler(view_id , msg_id , msg_data);
	case DIGITAL1_CLOCK_ID:
		return _digital1_clock_view_handler(view_id , msg_id , msg_data);
	#ifdef CONFIG_VIDEO_APP
	case VIDEO_CLOCK_ID:
		return _video_clock_view_handler(view_id , msg_id , msg_data);
	#endif
	default:
		return -ENOMEM;
	}
...
VIEW_DEFINE(clock, _clock_view_select, NULL, NULL, CLOCK_VIEW,

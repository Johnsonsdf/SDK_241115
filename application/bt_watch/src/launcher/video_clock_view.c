...
typedef struct video_clock_view_data {
	lv_obj_t *obj_clock;
	lv_img_dsc_t video_dsc;
	
	lvgl_res_group_t grp_hour;
	lvgl_res_group_t grp_minute;
	lvgl_res_group_t grp_second;

	lv_img_dsc_t img_tm_hour;
	lv_img_dsc_t img_tm_min;
	lv_img_dsc_t img_tm_sec;
...
static void _video_img_update(video_clock_view_data_t *data)
{
	if (data) {
		...
		if (cur_video_data.decode_buf && cur_video_data.decode_buf_size) {
			data->video_dsc.header.cf = LV_IMG_CF_TRUE_COLOR;
...
int _video_clock_view_handler(uint16_t view_id, uint8_t msg_id, void *msg_data)
{

...
static void _show_time_cb(lv_timer_t *timer)
{
	...
	if (data->cube_imgs[CUBE_MAP_FRONT].header.cf != LV_IMG_CF_TRUE_COLOR)
		lv_disp_drv_use_generic_set_px_cb(&driver, data->front_img_ori.header.cf);
	...
	uint8_t px_size = lv_img_cf_get_px_size(data->front_img_ori.header.cf) / 8;
	const uint8_t * src = data->front_img_ori.data +
			(data->tm_area.y1 * img_dsc->header.w + data->tm_area.x1) * px_size;
	uint8_t * dst_ori = (uint8_t *)img_dsc->data +
			(data->tm_area.y1 * img_dsc->header.w + data->tm_area.x1) * px_size;
	uint8_t * dst = dst_ori;
	int copy_len = lv_area_get_width(&data->tm_area) * px_size;
	int stride = img_dsc->header.w * px_size;
	...
	char buffer[16];
	_update_time(data);
	snprintf(buffer, sizeof(buffer), "%02d:%02d:%02d", data->time.tm_hour, data->time.tm_min, data->time.tm_sec);

#ifdef CONFIG_LVGL_USE_FREETYPE_FONT
	lvgl_freetype_force_bitmap(&data->font, 1);
#endif
...
#ifdef CONFIG_LVGL_USE_FREETYPE_FONT
	lvgl_freetype_force_bitmap(&data->font, 0);
#endif

	/* clean dcache */
	mem_dcache_clean(dst_ori, lv_area_get_height(&data->tm_area) * img_dsc->header.w * px_size);

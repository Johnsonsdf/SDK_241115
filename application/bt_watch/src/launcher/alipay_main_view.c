...
typedef struct alipay_main_view_data_s {
	lv_obj_t* bind_title;
	...
	int bind_status;
	int status_elaps;
} alipay_main_view_data_t;

static int32_t alipay_main_preload_inited = 0;
...
static int _load_txt_resource(alipay_main_view_data_t *data)
{
	// load scene
	int ret = lvgl_res_load_scene(SCENE_ALIPAY_MAIN_VIEW, &data->res_scene, DEF_STY_FILE, DEF_RES_FILE, DEF_STR_FILE);
	if (ret < 0) {
		SYS_LOG_ERR("SCENE_ALIPAY_MAIN_VIEW not found");
		return -ENOENT;
	}	
...
static void _unload_txt_resource(alipay_main_view_data_t *data)
{
	lvgl_res_unload_strings(data->res_txt, NUM_TXTS);
	lvgl_res_unload_scene(&data->res_scene);
}

static void _alipay_event_handler(lv_event_t * e)
{
	if(lvgl_click_decision())
	{
		go_to_m_effect();
		lv_obj_remove_event_cb(lv_event_get_user_data(e),_alipay_event_handler);
	}
}
...
static void _create_ui_bind(view_data_t *view_data)
{
	...
	alipay_main_view_data_t *data = view_data->user_data;
	...
	// create qrcode
	data->bind_qrcode = lv_qrcode_create(scr, 220, lv_color_black(), lv_color_white());
...
static void _delete_ui_bind(view_data_t *view_data)
{
	...
	if (data->bind_qrcode) {
		lv_qrcode_delete(data->bind_qrcode);
		data->bind_qrcode = NULL;
	}
	if (data->bind_title) {
		lv_obj_del(data->bind_title);
		data->bind_title = NULL;
	}
	if (data->bind_info) {
		lv_obj_del(data->bind_info);
		data->bind_info = NULL;
	}
}
...
static void _create_ui_pay(view_data_t *view_data)
{
	...
	if (is_barcode) {
		// create barcode
		data->pay_barcode = lv_barcode_create(scr, 270, 100, lv_color_black(), lv_color_white());
		if (data->pay_barcode) {
			lv_barcode_update(data->pay_barcode, str_buf, buf_len, barcodegen_128C);
			lv_obj_center(data->pay_barcode);
			lv_obj_set_style_border_color(data->pay_barcode, lv_color_white(), 0);
			lv_obj_set_style_border_width(data->pay_barcode, 8, 0);
		}
	} else {
		// create qrcode
		data->pay_qrcode = lv_qrcode_create(scr, 230, lv_color_black(), lv_color_white());
		if (data->pay_qrcode) {
...
static void _delete_ui_pay(view_data_t *view_data)
{
	...
	if (data->pay_title) {
		lv_obj_del(data->pay_title);
		data->pay_title = NULL;
	}
	if (data->pay_qrcode) {
		lv_qrcode_delete(data->pay_qrcode);
		data->pay_qrcode = NULL;
	}
	if (data->pay_barcode) {
		lv_barcode_delete(data->pay_barcode);
		data->pay_barcode = NULL;
	}
	if (data->pay_code) {
		lv_obj_del(data->pay_code);
		data->pay_code = NULL;
	}
	if (data->pay_btn) {
		lv_obj_del(data->pay_btn);
		data->pay_btn = NULL;
	}
}
...
static int _alipay_main_view_preload(view_data_t *view_data, bool update)
{
	if (alipay_main_preload_inited == 0) {
		lvgl_res_preload_scene_compact_default_init(SCENE_ALIPAY_MAIN_VIEW, NULL, 0,
			NULL, DEF_STY_FILE, DEF_RES_FILE, DEF_STR_FILE);
		alipay_main_preload_inited = 1;
	}

	return lvgl_res_preload_scene_compact_default(SCENE_ALIPAY_MAIN_VIEW, ALIPAY_MAIN_VIEW, update, 0);
}

static int _alipay_main_view_layout_update(view_data_t *view_data, bool first_layout)
{
	alipay_main_view_data_t *data = view_data->user_data;
	...
	if(first_layout) {
		...
		lv_obj_t *scr = lv_disp_get_scr_act(view_data->display);
		lv_obj_remove_event_cb(scr,_alipay_event_handler);
		lv_obj_add_flag(scr,LV_OBJ_FLAG_CLICKABLE);
		lv_obj_add_event_cb(scr, _alipay_event_handler, LV_EVENT_SHORT_CLICKED, scr);
	}
...
static int _alipay_main_view_layout(view_data_t *view_data)
{
	int ret;

	ret = _alipay_main_view_layout_update(view_data, true);
	if (ret < 0) {
		return ret;
	}
...
static int _alipay_main_view_delete(view_data_t *view_data)
{
	const alipay_view_presenter_t *presenter = view_get_presenter(view_data);
	alipay_main_view_data_t *data = view_data->user_data;
	...
	if (data) {
		...
		lvgl_res_preload_cancel_scene(SCENE_ALIPAY_MAIN_VIEW);
	}

	lvgl_res_unload_scene_compact(SCENE_ALIPAY_MAIN_VIEW);

	return 0;
}

static int _alipay_main_view_updated(view_data_t* view_data)
{
	return _alipay_main_view_layout_update(view_data, false);
}

static int _alipay_main_view_focus_changed(view_data_t *view_data, bool focused)
{
	const alipay_view_presenter_t *presenter = view_get_presenter(view_data);
	alipay_main_view_data_t *data = view_data->user_data;

	if (focused) {
		if(data && !lvgl_res_scene_is_loaded(SCENE_ALIPAY_MAIN_VIEW)) {
			_alipay_main_view_preload(view_data, true);
		}
	} else {
		if(data) {
			_unload_txt_resource(data);
...
int _alipay_main_view_handler(uint16_t view_id, uint8_t msg_id, void *msg_data)
{
	view_data_t *view_data = msg_data;

	assert(view_id == ALIPAY_MAIN_VIEW);

	switch (msg_id) {
	case MSG_VIEW_PRELOAD:
		return _alipay_main_view_preload(view_data, false);
	case MSG_VIEW_LAYOUT:
		return _alipay_main_view_layout(view_data);
	case MSG_VIEW_DELETE:
		return _alipay_main_view_delete(view_data);
	case MSG_VIEW_FOCUS:
		return _alipay_main_view_focus_changed(view_data, true);
	case MSG_VIEW_DEFOCUS:
		return _alipay_main_view_focus_changed(view_data, false);
	case MSG_VIEW_UPDATE:
		return _alipay_main_view_updated(view_data);
	default:
		return 0;
	}
}

VIEW_DEFINE(alipay_main_view, _alipay_main_view_handler, NULL,
		NULL, ALIPAY_MAIN_VIEW, NORMAL_ORDER, UI_VIEW_LVGL, DEF_UI_VIEW_WIDTH, DEF_UI_VIEW_HEIGHT);

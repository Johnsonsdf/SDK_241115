...
#define D_ICON_ZOOM	LV_IMG_ZOOM_NONE + 128
...
static void _turntable_view_update_icon(applist_ui_data_t * data, uint8_t idx, const lv_img_dsc_t * src);
...
static void _turntable_icon_change(bool save,applist_ui_data_t *data,int32_t angle)
{
	...
	for(uint32_t i=0;i<data->turntable_view.icon_num;i++)
	{
		...
		{
			lv_img_dsc_t *icon_img = (lv_img_dsc_t *)simple_img_get_src(icon_data->obj_icon);
			lv_img_dsc_t *middle_img = (lv_img_dsc_t *)simple_img_get_src(data->turntable_view.middle_icon);
...
static void _turntable_scroll_event_cb(lv_event_t * e)
{
	...
	case LV_EVENT_PRESSED:
		...
		lv_anim_del(data,_turntable_scroll_anim);
		break;
...
static int _turntable_view_create(lv_obj_t * scr)
{
	...
	simple_img_set_zoom(data->turntable_view.middle_icon,D_ICON_ZOOM);

...
#define D_ICON_MIN_ZOOM (LV_IMG_ZOOM_NONE>>1)
#define D_ICON_MAX_ZOOM	(LV_IMG_ZOOM_NONE + 100)
#define D_ICON_ZOOM_SIDE  (((82*D_ICON_MAX_ZOOM)/LV_IMG_ZOOM_NONE)>>1)
...
static void _wonhot_view_update_icon(applist_ui_data_t * data, uint8_t idx, const lv_img_dsc_t * src);
...
static void _wonhot_icon_x_y(applist_ui_data_t *data,int32_t radius)
{
	...
	for(uint32_t i=0;i<data->wonhot_view.icon_num;i++)
	{
		...
		if(at_radius >= data->wonhot_view.max_radius)
			...
			at_radius += data->wonhot_view.max_radius;
		if(at_radius == 0 || at_radius >= (3 * (D_ICON_RADIUS>>1)))
		{
			lv_obj_add_flag(icon_data->obj_icon,LV_OBJ_FLAG_HIDDEN);
			simple_img_set_zoom(icon_data->obj_icon,LV_IMG_ZOOM_NONE);
		}
		else
		{
			int32_t x = lv_trigo_sin(icon_data->angle) * at_radius / LV_TRIGO_SIN_MAX;
			x += data->icon[0].header.w>>1;
			x = (DEF_UI_WIDTH>>1) - x;
			int32_t y = lv_trigo_cos(icon_data->angle) * at_radius / LV_TRIGO_SIN_MAX;
			y += data->icon[0].header.h>>1;
			y = (DEF_UI_HEIGHT>>1) - y;
			lv_obj_set_pos(icon_data->obj_icon, x , y);
			int32_t icon_zoom =  (D_ICON_MAX_ZOOM - D_ICON_MIN_ZOOM) * at_radius / (3 * (D_ICON_RADIUS>>1)) + D_ICON_MIN_ZOOM;
			simple_img_set_zoom(icon_data->obj_icon,icon_zoom);
			lv_obj_clear_flag(icon_data->obj_icon,LV_OBJ_FLAG_HIDDEN);
		}
	}
}
...
static void _wonhot_scroll_event_cb(lv_event_t * e)
{
	...
	switch (code)
	{
		case LV_EVENT_PRESSED:
			...
			lv_anim_del(data,_wonhot_scroll_anim);
		break;
		...
		case LV_EVENT_RELEASED:
			{
				...
				if(code == LV_EVENT_RELEASED)
				{
					...
					if(radius != data->wonhot_view.move_radius && diff != 0)
					{
						...
						if(r_time)
						{
							...
							if(auto_time != 0 && dx != at_radius)
							{
								...
								lv_anim_set_values(&a, at_radius,dx);
					...
					if(ret)
					{
						if(at_radius != data->wonhot_view.move_radius)
						{
							int32_t dx = _wonhot_icon_radius_calibration(at_radius);
							lv_anim_t a;
							lv_anim_init(&a);
							lv_anim_set_var(&a, data);
							lv_anim_set_time(&a, D_ICON_SPEED);
							lv_anim_set_values(&a, at_radius,dx);
							lv_anim_set_exec_cb(&a, _wonhot_scroll_anim);
							lv_anim_set_path_cb(&a, lv_anim_path_ease_out);
							lv_anim_set_deleted_cb(&a, _wonhot_deleted_anim);
...
static int _wonhot_view_create(lv_obj_t * scr)
{
	...
	_wonhot_icon_x_y(data,scrl_y);
	data->wonhot_view.move_radius = scrl_y;
	return 0;
}

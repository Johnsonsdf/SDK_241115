...
#define D_ICON_ZOOM_MIN (LV_IMG_ZOOM_NONE>>1)	//min zoom
#define D_ICON_ZOOM_MAX (LV_IMG_ZOOM_NONE + 125)	//middle max zoom
#define D_SIDE_ICON_ZOOM_MAX (LV_IMG_ZOOM_NONE + 25)	//two flanks max zoom
...
static void _waterfall_view_update_icon(applist_ui_data_t * data, uint8_t idx, const lv_img_dsc_t * src);
...
static void _waterfall_img_zoom_up(applist_ui_data_t *data,lv_coord_t scroll_y)
{
	...
	for (lv_coord_t i = 0; i < data->waterfall_view.icon_num; i++)
	{
		...
		if(icon_y >= zoom_min_y && icon_y <= zoom_max_y)
		{
			...
			simple_img_set_zoom(icon_obj[i],v_zoom);
		}
		else
			simple_img_set_zoom(icon_obj[i],LV_IMG_ZOOM_NONE);
	}
}

...
#define ICON_ZOOM_MAX  (LV_IMG_ZOOM_NONE * 3 / 2)

/* Minimum zoom factor */
#define ICON_ZOOM_MIN  (LV_IMG_ZOOM_NONE / 6)
...
static void _cellular_view_update_icon(applist_ui_data_t * data, uint8_t idx, const lv_img_dsc_t * src);
...
static int _celluar_view_init_coords(lv_obj_t * scr)
{
	applist_ui_data_t *data = lv_obj_get_user_data(scr);
	applist_cellular_view_t *cellular = &data->cellular_view;

	/* check square icon */
	assert(data->icon[0].header.w == data->icon[0].header.h);

	cellular->pivots = lv_mem_alloc(sizeof(lv_point_t) * NUM_ITEMS * (TEST_DOUBLE_ICONS + 1));
	...
	cellular->radius = cellular->radius * ICON_ZOOM_MAX / (LV_IMG_ZOOM_NONE * 2);
	...
	lv_point_t bg_size = {
		lv_obj_get_content_width(data->cont),
		lv_obj_get_content_height(data->cont),
	};
	lv_point_t bg_pivot = { bg_size.x / 2, bg_size.y / 2 };

	for (int i = 0; i < NUM_ITEMS * (TEST_DOUBLE_ICONS + 1); i++, hexagon_idx++) {
...
static void _celluar_view_move(applist_ui_data_t *data)
{
	...
	for (int i = lv_obj_get_child_cnt(data->cont) - 1; i >= 0; i--) {
		...
		uint16_t zoom = ICON_ZOOM_MAX * radius / cellular->radius;
		if (zoom < ICON_ZOOM_MIN) {
			lvgl_obj_set_hidden(obj_icon, true);
			continue;
		}

		simple_img_set_zoom(obj_icon, zoom);
...
static void _cellular_press_event_cb(lv_event_t * e)
{
	...
	if (code == LV_EVENT_PRESSED) {
		...
		lv_anim_del(data, _cellular_scroll_anim);
	...
	if (code == LV_EVENT_PRESSING) {
		...
		if (cellular->scrolling == false) {
			...
			if (LV_ABS(vect.x) < indev->driver->scroll_limit &&
				LV_ABS(vect.y) < indev->driver->scroll_limit) {
				return;
			}
...
static int _cellular_view_create(lv_obj_t * scr)
{
	...
	lv_obj_clear_flag(data->cont, LV_OBJ_FLAG_SCROLLABLE);
	...
	if (_celluar_view_init_coords(scr))
		return -EINVAL;

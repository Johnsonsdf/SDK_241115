...
typedef struct compass_view_data {
	...
	/* lvgl resource */
	lv_img_dsc_t img_main;
#ifdef PIC_MAIN_ROT180
	lv_img_dsc_t img_main_rot180;
#endif
	lv_img_dsc_t img_decor;
	lv_img_dsc_t img_char[NUM_COMPASS_CHARS];
} compass_view_data_t;
...
static int _load_pictures(lvgl_res_scene_t *scene, uint32_t group_id,
		const uint32_t *pic_ids, int num_ids, lv_img_dsc_t *images)
{
...
int _compass_view_handler(uint16_t view_id, uint8_t msg_id, void * msg_data)
{
...
VIEW_DEFINE(compass, _compass_view_handler, NULL,

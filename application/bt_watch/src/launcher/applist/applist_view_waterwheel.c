...
#define BTN_ZOOM_MIN  (LV_IMG_ZOOM_NONE / 2)
...
static void _waterwheel_view_update_icon(applist_ui_data_t * data, uint8_t idx, const lv_img_dsc_t * src);
...
static void _waterwheel_scroll_event_cb(lv_event_t * e)
{
	...
	for (int i = lv_obj_get_child_cnt(cont_bg) - 1; i >= 0; i--) {
		...
		uint16_t zoom = LV_IMG_ZOOM_NONE - (LV_IMG_ZOOM_NONE - BTN_ZOOM_MIN) * LV_ABS(offset_y) / half_y;
		lv_coord_t btn_w = cont_w * zoom / LV_IMG_ZOOM_NONE;
		lv_coord_t btn_h = waterwheel_view->btn_h * zoom / LV_IMG_ZOOM_NONE;
		...
		lv_coord_t icon_w = data->icon[0].header.w * zoom / LV_IMG_ZOOM_NONE;
		lv_coord_t icon_space = waterwheel_view->icon_space * zoom / LV_IMG_ZOOM_NONE;
		lv_coord_t icon_ofs = icon_space + (icon_w - (lv_coord_t)data->icon[0].header.w) / 2;
		lv_coord_t text_space = BTN_COL_SPACE * zoom / LV_IMG_ZOOM_NONE;
		...
		if (obj_text) {
			const lv_img_dsc_t *text_src = simple_img_get_src(obj_text);
			lv_coord_t text_w = text_src->header.w * zoom / LV_IMG_ZOOM_NONE;

			lv_obj_set_x(obj_text, text_ofs + (text_w - (lv_coord_t)text_src->header.w) / 2);
			simple_img_set_zoom(obj_text, zoom);
		}
#else
...
static int _waterwheel_view_create(lv_obj_t * scr)
{
	...
	for (int i = 0; i < NUM_ITEMS; i++) {
		lv_obj_t *btn = lv_btn_create(cont_bg);
	...
	lv_event_send(data->cont, LV_EVENT_SCROLL, NULL);

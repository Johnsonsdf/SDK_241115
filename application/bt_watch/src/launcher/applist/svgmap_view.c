...
#define ZOOM_MIN (LV_IMG_ZOOM_NONE / 2)
...
static void _timer_cb(lv_timer_t *timer)
{
	...
	zoom = ZOOM_MIN + data->angle * (LV_IMG_ZOOM_NONE - ZOOM_MIN) / 3600;
	zoom = data->zoom_base * zoom / LV_IMG_ZOOM_NONE;

	svg_img_set_zoom(data->svg_obj, zoom);
}
...
static int _svgmap_view_layout(view_data_t *view_data)
{
	...
	data->zoom_base = (img_size <= scr_size) ? LV_IMG_ZOOM_NONE :
			(LV_IMG_ZOOM_NONE * scr_size / img_size);

	svg_img_set_zoom(data->svg_obj, data->zoom_base);
	svg_img_set_angle(data->svg_obj, data->angle);

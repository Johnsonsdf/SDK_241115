...
static void _grid_view_update_icon(applist_ui_data_t * data, uint8_t idx, const lv_img_dsc_t * src);
...
static int _grid_view_create(lv_obj_t * scr)
{
	...
	grid_view->col_dsc = lv_mem_alloc(sizeof(lv_coord_t) * (NUM_GRID_COLS + 1));
	grid_view->row_dsc = lv_mem_alloc(sizeof(lv_coord_t) * (NUM_GRID_ROWS + 1));
	if (grid_view->col_dsc == NULL || grid_view->row_dsc == NULL) {
		lv_mem_free(grid_view->col_dsc);
		lv_mem_free(grid_view->row_dsc);
		return -ENOMEM;
	}

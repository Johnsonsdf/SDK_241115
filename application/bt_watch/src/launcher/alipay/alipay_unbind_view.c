...
typedef struct alipay_unbind_view_data_s {
	lv_obj_t *unbind_obj;
	lv_obj_t *unbind_id;
...
} alipay_unbind_view_data_t;

static int32_t alipay_unbind_preload_inited = 0;
...
static void _create_ui_unbind(view_data_t *view_data)
{
	...
	alipay_unbind_view_data_t *data = view_data->user_data;
	...
	// create unbind btn
	data->unbind_btn = lv_btn_create(data->unbind_obj);
...
static void _delete_ui_unbind(view_data_t *view_data)
{
	...
	if (data->unbind_id) {
		lv_obj_del(data->unbind_id);
		data->unbind_id = NULL;
	}
	if (data->unbind_name) {
		lv_obj_del(data->unbind_name);
		data->unbind_name = NULL;
	}
	if (data->unbind_lbl) {
		lv_obj_del(data->unbind_lbl);
		data->unbind_lbl = NULL;
	}
	if (data->unbind_btn) {
		lv_obj_del(data->unbind_btn);
		data->unbind_btn = NULL;
	}
	if (data->unbind_obj) {
		lv_obj_del(data->unbind_obj);
...
static int _alipay_unbind_view_preload(view_data_t *view_data, bool update)
{
	if (alipay_unbind_preload_inited == 0) {
		lvgl_res_preload_scene_compact_default_init(SCENE_ALIPAY_UNBIND_VIEW, NULL, 0,
			NULL, DEF_STY_FILE, DEF_RES_FILE, DEF_STR_FILE);
		alipay_unbind_preload_inited = 1;
	}

	return lvgl_res_preload_scene_compact_default(SCENE_ALIPAY_UNBIND_VIEW, ALIPAY_UNBIND_VIEW, update, 0);
}

static int _alipay_unbind_view_layout_update(view_data_t *view_data, bool first_layout)
{
	alipay_unbind_view_data_t *data = view_data->user_data;
	int idx, ret;
	...
	if(first_layout) {
		// load scene
		ret = lvgl_res_load_scene(SCENE_ALIPAY_UNBIND_VIEW, &data->res_scene, DEF_STY_FILE, DEF_RES_FILE, DEF_STR_FILE);
		if (ret < 0) {
			SYS_LOG_ERR("SCENE_ALIPAY_UNBIND_VIEW not found");
			return -ENOENT;
		}
...
static int _alipay_unbind_view_layout(view_data_t *view_data)
{
	int ret;

	ret = _alipay_unbind_view_layout_update(view_data, true);
	if (ret < 0) {
		return ret;
	}
...
static int _alipay_unbind_view_delete(view_data_t *view_data)
{
	alipay_unbind_view_data_t *data = view_data->user_data;
	int idx;
	...
	if (data) {
		...
		lvgl_res_preload_cancel_scene(SCENE_ALIPAY_UNBIND_VIEW);
	}
	
	lvgl_res_unload_scene_compact(SCENE_ALIPAY_UNBIND_VIEW);

	return 0;
}

static int _alipay_unbind_view_updated(view_data_t* view_data)
{
	return _alipay_unbind_view_layout_update(view_data, false);
}

static int _alipay_unbind_view_focus_changed(view_data_t *view_data, bool focused)
{
	alipay_unbind_view_data_t *data = view_data->user_data;

	if (focused) {
		if(data && !lvgl_res_scene_is_loaded(SCENE_ALIPAY_UNBIND_VIEW)) {
			_alipay_unbind_view_preload(view_data, true);
		}
	} else {
		//if(data) {
...
int _alipay_unbind_view_handler(uint16_t view_id, uint8_t msg_id, void *msg_data)
{
	view_data_t *view_data = msg_data;

	assert(view_id == ALIPAY_UNBIND_VIEW);

	switch (msg_id) {
	case MSG_VIEW_PRELOAD:
		return _alipay_unbind_view_preload(view_data, false);
	case MSG_VIEW_LAYOUT:
		return _alipay_unbind_view_layout(view_data);
	case MSG_VIEW_DELETE:
		return _alipay_unbind_view_delete(view_data);
	case MSG_VIEW_FOCUS:
		return _alipay_unbind_view_focus_changed(view_data, true);
	case MSG_VIEW_DEFOCUS:
		return _alipay_unbind_view_focus_changed(view_data, false);
	case MSG_VIEW_UPDATE:
		return _alipay_unbind_view_updated(view_data);
	default:
		return 0;
	}
}

VIEW_DEFINE(alipay_unbind_view, _alipay_unbind_view_handler, NULL,
		NULL, ALIPAY_UNBIND_VIEW, NORMAL_ORDER, UI_VIEW_LVGL, DEF_UI_VIEW_WIDTH, DEF_UI_VIEW_HEIGHT);

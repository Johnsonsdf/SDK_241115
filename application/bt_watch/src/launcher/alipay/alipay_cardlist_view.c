	...
	if(cur_cardlist_view_status == STATE_LOAD_OK) {
		if(card_num == 0) {
			...
			if(data->auto_refresh_cardlist_timer) {
				lv_timer_del(data->auto_refresh_cardlist_timer);
...
static void _create_ui_cardlist(view_data_t *view_data, int cardlist_status)
{
	...
	case STATE_LOAD_IDLE:
		...
		lv_obj_clear_flag(tip_label, LV_OBJ_FLAG_CLICKABLE);
		...
		data->exit_btn = lv_btn_create(data->cont);
		...
		lv_obj_clear_flag(finished_label, LV_OBJ_FLAG_CLICKABLE);
	...
	case STATE_LOAD_OK:
		...
		if(card_num == 0) {
			...
			lv_obj_t *qrcode = lv_qrcode_create(data->cont, 220, lv_color_black(), lv_color_white());
			...
			for(i=0; i<card_num+1; i++) {
				lv_obj_t* btn = lv_btn_create(cardlist_area);
				if(i < card_num) {
	...
	case STATE_LOAD_FAIL:
		...
		lv_obj_add_style(tip_label, &data->sty_lbl, LV_PART_MAIN);
		// create btn
		retry_btn = lv_btn_create(data->cont);
		...
		lv_obj_clear_flag(retry_btn_label, LV_OBJ_FLAG_CLICKABLE);
...
int _alipay_cardlist_view_handler(uint16_t view_id, uint8_t msg_id, void *msg_data)
{
...
VIEW_DEFINE(alipay_cardlist_view, _alipay_cardlist_view_handler, NULL,

...
static void _create_ui_card(view_data_t *view_data, int card_status)
{
	...
	case STATE_LOAD_OK:
		...
		// create qrcode
		data->transit_qrcode = lv_qrcode_create(data->cont, 220, lv_color_black(), lv_color_white());
		...
		// create btn
		data->transit_btn = lv_btn_create(data->cont);
...
int _alipay_transitcode_view_handler(uint16_t view_id, uint8_t msg_id, void *msg_data)
{
...
VIEW_DEFINE(alipay_transitcode_view, _alipay_transitcode_view_handler, NULL,

...
typedef struct alipay_bind_view_data_s {
	lv_obj_t* bind_title;
	...
	int bind_status;
	int status_elaps;
} alipay_bind_view_data_t;

static int32_t alipay_bind_preload_inited = 0;
...
static void _create_ui_bind(view_data_t *view_data)
{
	...
	alipay_bind_view_data_t *data = view_data->user_data;
...
static int _alipay_bind_view_preload(view_data_t *view_data, bool update)
{
	if (alipay_bind_preload_inited == 0) {
		lvgl_res_preload_scene_compact_default_init(SCENE_ALIPAY_BIND_VIEW, NULL, 0,
			NULL, DEF_STY_FILE, DEF_RES_FILE, DEF_STR_FILE);
		alipay_bind_preload_inited = 1;
	}

	return lvgl_res_preload_scene_compact_default(SCENE_ALIPAY_BIND_VIEW, ALIPAY_BIND_VIEW, update, 0);
}

static int _alipay_bind_view_layout_update(view_data_t *view_data, bool first_layout)
{
	alipay_bind_view_data_t *data = view_data->user_data;
	int ret;
	...
	if(first_layout) {
		// load scene
		ret = lvgl_res_load_scene(SCENE_ALIPAY_BIND_VIEW, &data->res_scene, DEF_STY_FILE, DEF_RES_FILE, DEF_STR_FILE);
		if (ret < 0) {
			SYS_LOG_ERR("SCENE_ALIPAY_BIND_VIEW not found");
			return -ENOENT;
		}
...
static int _alipay_bind_view_layout(view_data_t *view_data)
{
	int ret;

	ret = _alipay_bind_view_layout_update(view_data, true);
	if (ret < 0) {
		return ret;
	}
...
static int _alipay_bind_view_delete(view_data_t *view_data)
{
	const alipay_view_presenter_t *presenter = view_get_presenter(view_data);
	alipay_bind_view_data_t *data = view_data->user_data;
	...
	if (data) {
		...
		lvgl_res_preload_cancel_scene(SCENE_ALIPAY_BIND_VIEW);
	}

	lvgl_res_unload_scene_compact(SCENE_ALIPAY_BIND_VIEW);

	return 0;
}

static int _alipay_bind_view_updated(view_data_t* view_data)
{
	return _alipay_bind_view_layout_update(view_data, false);
}

static int _alipay_bind_view_focus_changed(view_data_t *view_data, bool focused)
{
	const alipay_view_presenter_t *presenter = view_get_presenter(view_data);
	alipay_bind_view_data_t *data = view_data->user_data;

	if (focused) {
		if(data && !lvgl_res_scene_is_loaded(SCENE_ALIPAY_BIND_VIEW)) {
			_alipay_bind_view_preload(view_data, true);
		}
	} else {
		//if(data) {
...
int _alipay_bind_view_handler(uint16_t view_id, uint8_t msg_id, void *msg_data)
{
	view_data_t *view_data = msg_data;

	assert(view_id == ALIPAY_BIND_VIEW);

	switch (msg_id) {
	case MSG_VIEW_PRELOAD:
		return _alipay_bind_view_preload(view_data, false);
	case MSG_VIEW_LAYOUT:
		return _alipay_bind_view_layout(view_data);
	case MSG_VIEW_DELETE:
		return _alipay_bind_view_delete(view_data);
	case MSG_VIEW_FOCUS:
		return _alipay_bind_view_focus_changed(view_data, true);
	case MSG_VIEW_DEFOCUS:
		return _alipay_bind_view_focus_changed(view_data, false);
	case MSG_VIEW_UPDATE:
		return _alipay_bind_view_updated(view_data);
	default:
		return 0;
	}
}

VIEW_DEFINE(alipay_bind_view, _alipay_bind_view_handler, NULL,
		NULL, ALIPAY_BIND_VIEW, NORMAL_ORDER, UI_VIEW_LVGL, DEF_UI_VIEW_WIDTH, DEF_UI_VIEW_HEIGHT);

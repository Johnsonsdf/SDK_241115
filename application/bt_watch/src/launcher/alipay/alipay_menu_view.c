...
static void _create_ui_menu(view_data_t *view_data)
{
	...
	for(i=0; i<NUM_BTNS; i++) {
		data->pay_btn[i] = lv_btn_create(scr);
		...
		lv_obj_t *obj_icon = lv_img_create(data->pay_btn[i]);
		lv_img_set_src(obj_icon, &data->icon[i]);
...
int _alipay_menu_view_handler(uint16_t view_id, uint8_t msg_id, void *msg_data)
{
...
VIEW_DEFINE(alipay_menu_view, _alipay_menu_view_handler, NULL,

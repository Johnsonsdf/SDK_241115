...
typedef struct {
	...
	/* lvgl resource */
	lv_img_dsc_t img_icons[NUM_APP_TYPES];
...
static void _message_list_btn_event_handler(lv_event_t * e)
{
	...
	os_strace_u32x2(SYS_TRACE_ID_GUI_EVENT, index, LV_EVENT_SHORT_CLICKED);
...
static int _show_message_list(view_data_t *view_data)
{
	...
	for (int i = 0; i < ARRAY_SIZE(messages); i++) {
		lv_obj_t *btn = lv_btn_create(data->list);
...
static void _message_content_event_handler(lv_event_t * e)
{
	...
	if (data->box && dir == LV_DIR_RIGHT) {
		os_strace_u32x2(SYS_TRACE_ID_GUI_EVENT, (uint32_t)data->box, LV_EVENT_GESTURE);

		lv_indev_wait_release(indev);
		lv_obj_del(data->box);
		data->box = NULL;
		lv_obj_clear_flag(data->list, LV_OBJ_FLAG_HIDDEN);
...
int _message_view_handler(uint16_t view_id, uint8_t msg_id, void *msg_data)
{
...
VIEW_DEFINE(message, _message_view_handler, NULL,

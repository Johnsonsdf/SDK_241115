...
typedef struct sys_view_data {
	...
	lv_img_dsc_t img_dsc_bat[NUM_BATS];
	lv_style_t style_bg;
...
static void _update_btn_status(sys_view_data_t *data)
{
	...
	if (data->btn_sta[BTN_BLUETOOTH]) {
		lv_imgbtn_set_state(data->btn[BTN_BLUETOOTH], LV_IMGBTN_STATE_CHECKED_RELEASED);
	} else {
		lv_imgbtn_set_state(data->btn[BTN_BLUETOOTH], LV_IMGBTN_STATE_RELEASED);
	}
}
...
static void _load_btn_status(sys_view_data_t *data)
{
	...
	for (i = 0; i < NUM_BTNS; i++) {
		if (data->btn_sta[i]) {
			lv_imgbtn_set_state(data->btn[i], LV_IMGBTN_STATE_CHECKED_RELEASED);
		} else {
			lv_imgbtn_set_state(data->btn[i], LV_IMGBTN_STATE_RELEASED);
		}
	}
}
...
static int _main_view_layout_update(view_data_t *view_data, bool first_layout)
{
	...
	if(first_layout)
	{
		...
		lv_obj_clear_flag(data->btn[BTN_BLUETOOTH], LV_OBJ_FLAG_CLICKABLE);
		lv_obj_clear_flag(data->btn[BTN_SETTING], LV_OBJ_FLAG_CHECKABLE);
...
static int _main_view_paint(view_data_t *view_data)
{
	...
	if (data) {
		...
		lv_img_set_src(data->img[IMG_BAT], &data->img_dsc_bat[data->bat_val / (100/4)]);
...
int _main_view_handler(uint16_t view_id, uint8_t msg_id, void * msg_data)
{
...
VIEW_DEFINE(main, _main_view_handler, NULL, \

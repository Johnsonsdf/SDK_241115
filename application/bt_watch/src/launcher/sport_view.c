...
static int _sport_view_layout_update(view_data_t *view_data, bool first_layout)
{
	...
	if (first_layout) {
		...
		lv_imgbtn_set_src(data->btn, LV_IMGBTN_STATE_RELEASED, NULL, &data->img_dsc_bmp[BMP_BTN], NULL);
		lv_imgbtn_set_src(data->btn, LV_IMGBTN_STATE_PRESSED, NULL, &data->img_dsc_bmp[BMP_BTN_H], NULL);
		lv_imgbtn_set_src(data->btn, LV_IMGBTN_STATE_CHECKED_RELEASED, NULL, &data->img_dsc_bmp[BMP_BTN_H], NULL);
		lv_imgbtn_set_src(data->btn, LV_IMGBTN_STATE_CHECKED_PRESSED, NULL, &data->img_dsc_bmp[BMP_BTN], NULL);
...
static int _sport_view_paint(view_data_t *view_data)
{
	...
	if (data) {
		...
		if (lv_obj_has_state(data->btn, LV_STATE_CHECKED)) {
			lv_obj_clear_state(data->btn, LV_STATE_CHECKED);
		} else {
...
static int _sport_view_updated(view_data_t* view_data)
{
	...
	ret = _sport_view_layout_update(view_data, false);
	return ret;
...
int _sport_view_handler(uint16_t view_id, uint8_t msg_id, void * msg_data)
{
...
VIEW_DEFINE(sport_view, _sport_view_handler, NULL, \

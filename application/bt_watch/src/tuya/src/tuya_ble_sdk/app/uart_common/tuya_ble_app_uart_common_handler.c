...
void tuya_ble_uart_common_process(uint8_t *p_in_data,uint16_t in_len)
{
    ...
    case TUYA_BLE_UART_COMMON_QUERY_MCU_VERSION:
        ...
        TUYA_BLE_LOG_INFO("reveived mcu_firmware_version : 0x%04x mcu_hardware_version : 0x%04x",mcu_firmware_version,mcu_hardware_version);
    ...
    case TUYA_BLE_UART_COMMON_MCU_OTA_END:
        ...
        if(mcu_ota_flag!=0)
        {
            ...
            else
            {
                ota_data_response_buffer[6] = 0;
                mcu_ota_flag = 0;
                TUYA_BLE_LOG_INFO("mcu ota test succeed.");
            }
            ota_data_response_len = 7;
        }
        else

...
#define TUYA_BLE_VOS_TOKEN_DATA_MAX_SIZE  (0x400)


tuya_ble_vos_token_data_t current_token;
...
{
    //current_speech_state = SpeechState_IDLE;
    TUYA_BLE_LOG_INFO("");
...
static void tuya_vos_token_init(void)
{
    ...
    if(p_buffer)
    {
        ...
        if((current_token.token_length==0)||(current_token.token_length>(TUYA_BLE_VOS_TOKEN_DATA_MAX_SIZE-18)))
        {
            TUYA_BLE_LOG_ERROR("tuya_vos_token_init->token length error, length = %d",current_token.token_length);
		...
		TUYA_BLE_LOG_INFO("current_token->is_token_valid = %d",current_token.is_token_valid);
...
//    err_code = drv_mic_init();
//    TUYA_BLE_LOG_DEBUG("drv_mic_init error code = %d",err_code);
//    APP_ERROR_CHECK(err_code);

    return 0;//drv_mic_start();
...
tuya_ble_speech_state_t tuya_ble_vos_speech_state_get(void)
{
...
static void tuya_ble_start_speech_handler(uint32_t evt_id,void *data)
{
    ...
    sn = 0;
    TUYA_BLE_LOG_INFO("[VoS] Push to talk!");
...
static void tuya_vos_codec_data_handler(void)
{
    ...
    tuya_ble_device_exit_critical();
	TUYA_BLE_LOG_INFO("flag %d, streamlen = %d",audio_data_send_flag, ui32StreamLen);
    ...
    if((ui32StreamLen >= TUYA_BLE_VOS_DATA_BUFFER_SIZE)&&(audio_data_send_flag==0))
    {
        ...
        //tuya_ble_common_uart_send_data(ble_buffer+7,CODEC_OUT_RING_BUFF_SIZE*6);
        tuya_ble_commData_send(TUYA_BLE_FRM_VOS_STREAMING,0,ble_buffer,7+TUYA_BLE_VOS_DATA_BUFFER_SIZE,ENCRYPTION_MODE_NONE);
        sn++;
        //tuya_vos_audio_flush_ring_buffer();
    ...
    if (vos_sys_info.PushTalkFlag || vos_sys_info.ProvideSpeechFlag)
    {
        ...
        if (vos_sys_info.StreamingTimeOutCount < AUDIO_KWD_TIMEOUT_S * AM_SPP_SAMPLE_FREQ)
        {
            ...
            if(vos_sys_info.StreamingTimeOutCount == 0)
            {
                ...
                //tuya_vos_audio_buffer_rewind();  // in conversation mode, dial back only for 100ms...
...
tuya_ble_status_t tuya_vos_encoded_audio_data_send(uint8_t * p_data,uint32_t size)
{
	...
	// SYS_LOG_ERR("PushTalkFlag %d ProvideSpeechFlag %d",vos_sys_info.PushTalkFlag, vos_sys_info.ProvideSpeechFlag);
    if (vos_sys_info.PushTalkFlag || vos_sys_info.ProvideSpeechFlag)
    {
...
static void tuya_ble_send_audio_data(uint16_t send_sn)
{
	...
	while (1)
	{
	    ...
	    if(ui32StreamLen >= TUYA_BLE_VOS_DATA_BUFFER_SIZE)
	    {
	        ...
	        //TUYA_BLE_LOG_HEXDUMP_DEBUG("[Vos] opus encode data :",buffer,CODEC_OUT_RING_BUFF_SIZE+1);
	    ...
	    else
	    {
	        ...
	        {
	        	//TUYA_BLE_LOG_INFO("flag %d",audio_data_send_flag);
	            audio_data_send_flag = 0;
	        }
			break;
	    }
...
static void tuya_ble_handle_speech_cmd(uint8_t *p_recv_data,uint32_t recv_data_len)
{
    ...
    case TUYA_BLE_VOS_CMD_START_SPEECH:
        if(is_ack)
        {
            ...
            {
                dialog_id_temp = (p_recv_data[3]<<24) + (p_recv_data[4]<<16) + (p_recv_data[5]<<8) + p_recv_data[6];
                status = p_recv_data[7];
                TUYA_BLE_LOG_DEBUG("received start speech cmd ack dialog id = %d, status = %d.",dialog_id_temp,status);
                ...
                if((dialog_id_temp!=dialog_id)||(status!=0))
                {
                    ...
                    audio_process_flag = 0;
                    tuya_ble_device_exit_critical();
                    event.avs_cmd_event_data.res_data.status = 1;
                }
                else
                {
    ...
    case TUYA_BLE_VOS_CMD_ENDPOINT_SPEECH:
        ...
        else
        {
            ...
            dialog_id_temp = (p_recv_data[3]<<24) + (p_recv_data[4]<<16) + (p_recv_data[5]<<8) + p_recv_data[6];
            if ((dialog_id_temp==dialog_id) && (0 == tuya_restart_va_status_get()))
            {
            ...
            {
                TUYA_BLE_LOG_WARNING("Received Endpoint Speech Cmd with invaild id,dialog id = %d.",dialog_id_temp);
            }
    ...
    case TUYA_BLE_VOS_CMD_STOP_SPEECH:  //  stop speech cmd
        ...
        {
            dialog_id_temp = (p_recv_data[4]<<24) + (p_recv_data[5]<<16) + (p_recv_data[6]<<8) + p_recv_data[7];
...
static uint32_t vos_data_unpack(frm_trsmitr_proc_s *p_trsmitr_proc, tuya_ble_common_recv_packet *p_recv_packet, uint8_t *buf,uint32_t len)
{
    ...
    if(FRM_PKG_FIRST == p_trsmitr_proc->pkg_desc)
    {
        ...
        if((p_recv_packet->recv_len_max > AUDIO_RESULT_DATA_MAX_SIZE)||(p_recv_packet->recv_len_max==0))
        {
            p_recv_packet->recv_len_max = 0;
            p_recv_packet->recv_len = 0;
...
static void tuya_ble_vos_handle_common_speech_response_data(uint8_t *p_recv_data,uint32_t recv_data_len)
{
    ...
    if ((p_data_buffer==NULL) && (0 != data_length))
    {
...
static void tuya_ble_vos_handle_weather_response_data(uint8_t *p_recv_data,uint32_t recv_data_len)
{
    ...
    else
    {
        ...
        index_src++;
        if(((index_dest+weather_data->main_title_length)<=data_buffer_size)&&((index_src+weather_data->main_title_length)<=data_length))
        {
        ...
        {
            TUYA_BLE_LOG_ERROR("[Vos] Received weather main_title_length error,main_title_length = %d",weather_data->main_title_length);
        ...
        if(((index_dest+weather_data->sub_title_length)<=data_buffer_size)&&((index_src+weather_data->sub_title_length)<=data_length))
        {
        ...
        {
            TUYA_BLE_LOG_ERROR("[Vos] Received weather sub_title_length error,sub_title_length = %d",weather_data->sub_title_length);
            tuya_ble_free(p_data_buffer);
            return;
        }
        ...
        {
            TUYA_BLE_LOG_ERROR("[Vos] Received weather days error,weather days = %d",weather_data->weather_days);
            tuya_ble_free(p_data_buffer);
            return;
        }

        if((weather_data->temperature_unit != TUYA_TEMPERATURE_UNIT_CELSIUS)&&(weather_data->temperature_unit != TUYA_TEMPERATURE_UNIT_FAHRENHEIT))
        {
            TUYA_BLE_LOG_ERROR("[Vos] Received weather temperature unit error,unit = %d",weather_data->temperature_unit);
            tuya_ble_free(p_data_buffer);
            return;
        }

        weather_data->query_day_weather_data.length = (p_recv_data[index_src]<<8)+p_recv_data[index_src+1];
        index_src += 2;
        weather_data->query_day_weather_data.flag = p_recv_data[index_src];
        index_src++;

        weather_data->query_day_weather_data.weather_type = (p_recv_data[index_src]<<8)+p_recv_data[index_src+1];
        index_src += 2;
        weather_data->query_day_weather_data.temperature_current = (p_recv_data[index_src]<<8)+p_recv_data[index_src+1];
        index_src += 2;
        weather_data->query_day_weather_data.temperature_min = (p_recv_data[index_src]<<8)+p_recv_data[index_src+1];
        index_src += 2;
        weather_data->query_day_weather_data.temperature_max = (p_recv_data[index_src]<<8)+p_recv_data[index_src+1];
        index_src += 2;

        weather_data->query_day_weather_data.tts_length = p_recv_data[index_src];
        index_src++;
        if(((index_dest+weather_data->query_day_weather_data.tts_length)<=data_buffer_size)&&((index_src+weather_data->query_day_weather_data.tts_length)<=data_length))
        {
            memcpy(p_data_buffer+index_dest,p_recv_data+index_src,weather_data->query_day_weather_data.tts_length);
        ...
        {
            TUYA_BLE_LOG_ERROR("[Vos] Received weather data tts string length error,tts_length = %d",weather_data->query_day_weather_data.tts_length);
            tuya_ble_free(p_data_buffer);
            return;
        }
        ...
        index_src++;
        if(((index_dest+weather_data->query_day_weather_data.weather_description_length)<=data_buffer_size)&&((index_src+weather_data->query_day_weather_data.weather_description_length)<=data_length))
        {
            memcpy(p_data_buffer+index_dest,p_recv_data+index_src,weather_data->query_day_weather_data.weather_description_length);
        ...
        {
            TUYA_BLE_LOG_ERROR("[Vos] Received query day weather data weather_description_length error,weather_description_length = %d",weather_data->query_day_weather_data.weather_description_length);
            tuya_ble_free(p_data_buffer);
            return;
        }
        ...
        for(i=0; i<days; i++)
        {
            ...
            weather_data->daily_weather_data[i].weather_type = (p_recv_data[index_src]<<8)+p_recv_data[index_src+1];
            index_src += 2;
            weather_data->daily_weather_data[i].temperature_min = (p_recv_data[index_src]<<8)+p_recv_data[index_src+1];
            index_src += 2;
            weather_data->daily_weather_data[i].temperature_max = (p_recv_data[index_src]<<8)+p_recv_data[index_src+1];
            index_src += 2;

            weather_data->daily_weather_data[i].date_length = p_recv_data[index_src];
            index_src += 1;

            if(((index_dest+weather_data->daily_weather_data[i].date_length)<=data_buffer_size)&&((index_src+weather_data->daily_weather_data[i].date_length)<=data_length))
            {
                memcpy(p_data_buffer+index_dest,p_recv_data+index_src,weather_data->daily_weather_data[i].date_length);
            ...
            {
                TUYA_BLE_LOG_ERROR("[Vos] Received weather daily data length error,date_length = %d",weather_data->daily_weather_data[i].date_length);
                tuya_ble_free(p_data_buffer);
                return;
            }
            ...
            if(((index_dest+weather_data->daily_weather_data[i].day_length)<=data_buffer_size)&&((index_src+weather_data->daily_weather_data[i].day_length)<=data_length))
            {
                memcpy(p_data_buffer+index_dest,p_recv_data+index_src,weather_data->daily_weather_data[i].day_length);
            ...
            {
                TUYA_BLE_LOG_ERROR("[Vos] Received weather daily data length error,day_length = %d",weather_data->daily_weather_data[i].day_length);
                tuya_ble_free(p_data_buffer);
                return;
            }
            ...
            index_src++;
            if(((index_dest+weather_data->daily_weather_data[i].weather_description_length)<=data_buffer_size)&&((index_src+weather_data->daily_weather_data[i].weather_description_length)<=data_length))
            {
                memcpy(p_data_buffer+index_dest,p_recv_data+index_src,weather_data->daily_weather_data[i].weather_description_length);
            ...
            {
                TUYA_BLE_LOG_ERROR("[Vos] Received weather daily data weather_description_length error,weather_description_length = %d",weather_data->daily_weather_data[i].weather_description_length);
                tuya_ble_free(p_data_buffer);
                return;
            }
        ...
        /*
                TUYA_BLE_LOG_HEXDUMP_DEBUG("[Vos]Weather data -> MainTitle :",weather_data->p_main_title,weather_data->main_title_length);
                TUYA_BLE_LOG_HEXDUMP_DEBUG("[Vos]Weather data -> subTitle :",weather_data->p_sub_title,weather_data->sub_title_length);
                TUYA_BLE_LOG_DEBUG("[Vos]Weather data -> Temperature unit : %d ",weather_data->temperature_unit);
                TUYA_BLE_LOG_DEBUG("[Vos]Weather data -> Weather Days : %d ",weather_data->weather_days);
                TUYA_BLE_LOG_DEBUG("[Vos]Weather data -> query_day_weather_data : length = %d , flag = %d , weather type = %d, temperature current = %d , temperature min = %d \
        		, temperature max = %d .",weather_data->query_day_weather_data.length,weather_data->query_day_weather_data.flag,weather_data->query_day_weather_data.weather_type,
                                   weather_data->query_day_weather_data.temperature_current,weather_data->query_day_weather_data.temperature_min,weather_data->query_day_weather_data.temperature_max);

                for(i=0; i<days; i++)
                {
                    TUYA_BLE_LOG_DEBUG("[Vos]Weather data -> daily_weather_data[%d] : weather type = %d, temperature min = %d ,temperature max = %d .",i,
                                       weather_data->daily_weather_data[i].weather_type,weather_data->daily_weather_data[i].temperature_min,weather_data->daily_weather_data[i].temperature_max);
                    TUYA_BLE_LOG_HEXDUMP_DEBUG("Date :",weather_data->daily_weather_data[i].p_date,weather_data->daily_weather_data[i].date_length);
                    TUYA_BLE_LOG_HEXDUMP_DEBUG("Day : ",weather_data->daily_weather_data[i].p_day,weather_data->daily_weather_data[i].day_length);
                }
        */
...
static void tuya_ble_vos_handle_list_response_data(uint8_t *p_recv_data,uint32_t recv_data_len)
{
    ...
    else
    {
        ...
        if(((index_dest+list_data->main_title_length)<=data_buffer_size)&&((index_src+list_data->main_title_length)<=data_length))
        {
        ...
        {
            TUYA_BLE_LOG_ERROR("[Vos] Received List data main_title_length error,main_title_length = %d",list_data->main_title_length);
            tuya_ble_free(p_data_buffer);
            return;
        }
        ...
        index_src++;
        if(((index_dest+list_data->sub_title_length)<=data_buffer_size)&&((index_src+list_data->sub_title_length)<=data_length))
        {
        ...
        {
            TUYA_BLE_LOG_ERROR("[Vos] Received List data sub_title_length error,sub_title_length = %d",list_data->sub_title_length);
            tuya_ble_free(p_data_buffer);
            return;
        }
        ...
        for(i=0; i<list_data->list_number; i++)
        {
            ...
            index_src += 1;
            if(((index_dest+list_data->list_content_data[i].left_text_length)<=data_buffer_size)&&((index_src+list_data->list_content_data[i].left_text_length)<=data_length))
            {
                memcpy(p_data_buffer+index_dest,p_recv_data+index_src,list_data->list_content_data[i].left_text_length);
            ...
            {
                TUYA_BLE_LOG_ERROR("[Vos] Received List data list_content_data left text length error,list_content_data[%d].left_text_length = %d",i,list_data->list_content_data[i].left_text_length);
                tuya_ble_free(p_data_buffer);
                return;
            }
            ...
            index_src += 1;
            if(((index_dest+list_data->list_content_data[i].right_text_length)<=data_buffer_size)&&((index_src+list_data->list_content_data[i].right_text_length)<=data_length))
            {
                memcpy(p_data_buffer+index_dest,p_recv_data+index_src,list_data->list_content_data[i].right_text_length);
            ...
            {
                TUYA_BLE_LOG_ERROR("[Vos] Received List data list_content_data right text length error,list_content_data[%d].right_text_length = %d",i,list_data->list_content_data[i].right_text_length);
                tuya_ble_free(p_data_buffer);
                return;
            }
        }
        /*
                TUYA_BLE_LOG_HEXDUMP_DEBUG("[Vos]List data -> MainTitle :",list_data->p_main_title,list_data->main_title_length);
                TUYA_BLE_LOG_HEXDUMP_DEBUG("[Vos]List data -> subTitle :",list_data->p_sub_title,list_data->sub_title_length);
                TUYA_BLE_LOG_DEBUG("[Vos]List data -> List number : %d ",list_data->list_number);

                for(i=0; i<list_data->list_number; i++)
                {
                    TUYA_BLE_LOG_DEBUG("[Vos]List data -> List content data[%d] : ",i);
                    TUYA_BLE_LOG_HEXDUMP_DEBUG("Left text :",list_data->list_content_data[i].p_left_text,list_data->list_content_data[i].left_text_length);
                    TUYA_BLE_LOG_HEXDUMP_DEBUG("Right text : ",list_data->list_content_data[i].p_right_text,list_data->list_content_data[i].right_text_length);
                }
        */
...
static void tuya_ble_handle_speech_response(uint8_t *p_recv_data,uint32_t recv_data_len)
{
	...
	if (!token_trsmitr_proc.subpkg)
		token_trsmitr_proc.subpkg = (u8_t *)tuya_ble_malloc(SNGL_PKG_TRSFR_LMT);

	err_code = vos_data_unpack(&token_trsmitr_proc,&token_recv_packet,p_recv_data,recv_data_len);
	tuya_ble_free(token_trsmitr_proc.subpkg);
	token_trsmitr_proc.subpkg = NULL;
    ...
    if(err_code == 0) //分包数据未发送完就停止发送了如何处理？什么时候释放内存？
    {
        type = audio_recv_packet.recv_data[1];

        data_length = (audio_recv_packet.recv_data[2]<<8) + audio_recv_packet.recv_data[3];

        TUYA_BLE_LOG_DEBUG("[Vos] Received vos response data type = %d, data length = %d ",type,data_length);
...
static void tuya_ble_vos_token_send(void)
{
	...
	if (!token_trsmitr_proc_send.subpkg)
		token_trsmitr_proc_send.subpkg = (u8_t *)tuya_ble_malloc(SNGL_PKG_TRSFR_LMT);

    ret = trsmitr_send_pkg_encode_with_packet_length(&token_trsmitr_proc_send,AUDIO_RESULT_DATA_MAX_PACKET_SIZE,TUYA_BLE_PROTOCOL_VERSION_HIGN,
    ...
    if (MTP_OK != ret && MTP_TRSMITR_CONTINUE != ret)
    {
        ...
        token_send_buffer_length = 0;
		tuya_ble_free(token_trsmitr_proc_send.subpkg);
		token_trsmitr_proc_send.subpkg = NULL;
        return;
    }
    ...
    TUYA_BLE_LOG_DEBUG("current send token packet number = %d, packet length = %d, total length = %d",current_send_token_packet_number,token_trsmitr_proc_send.subpkg_len,
                       token_trsmitr_proc_send.total);
    tuya_ble_commData_send(TUYA_BLE_FRM_VOS_TOKEN_SEND,0,p_buf,send_len,ENCRYPTION_MODE_SESSION_KEY);
...
static void tuya_ble_handle_vos_token_reading(uint8_t *p_recv_data,uint32_t recv_data_len)
{
    ...
    tuya_ble_commData_send(TUYA_BLE_FRM_VOS_TOKEN_READING_RESP,0,buffer,18,ENCRYPTION_MODE_SESSION_KEY);
    ...
    if((p_recv_data[1]==1)&&(current_token.is_token_valid)) // token data request
    {
        ...
        if(p_token_send_buffer)
        {
            tuya_ble_nv_read(TUYA_NV_VOS_TOKEN_START_ADDR,p_token_send_buffer,current_token.token_length+20);
...
static void tuya_ble_handle_vos_token_write(uint8_t *p_recv_data,uint32_t recv_data_len)
{
    ...
    if(err_code)
    {
        tuya_ble_commData_send(TUYA_BLE_FRM_VOS_TOKEN_WRITE_RESP,0,buffer,13,ENCRYPTION_MODE_SESSION_KEY);
    }
    else //分包数据未发送完就停止发送了如何处理？什么时候释放内存？断开连接时
    {
        version = token_recv_packet.recv_data[0];
        if(version==0)
        {
            current_token.token_length = (token_recv_packet.recv_data[17]<<8) + token_recv_packet.recv_data[18];
            ...
            if(tuya_ble_nv_erase(TUYA_NV_VOS_TOKEN_START_ADDR,TUYA_NV_VOS_TOKEN_AREA_SIZE)==TUYA_BLE_SUCCESS)
            {
                if(tuya_ble_nv_write(TUYA_NV_VOS_TOKEN_START_ADDR,token_recv_packet.recv_data,TUYA_NV_VOS_TOKEN_AREA_SIZE/*token_recv_packet.recv_len_max+(4-(token_recv_packet.recv_len_max&0x0003))*/)==TUYA_BLE_SUCCESS)
                {
                    current_token.is_token_valid = 1;

					tuya_start_tone_play(TUYA_ALEXA_AUTH_PASS);
					// 这里给bt manager发消息，播放提示音ful_state_bluetooth_connected_16KHZ_蓝牙连接or授权成功
...
static void tuya_ble_handle_vos_common_set(uint8_t *p_recv_data,uint32_t recv_data_len)
{
    ...
    case 1:
        if((p_recv_data[2]==3)&&((p_recv_data[3]==1)||(p_recv_data[3]==2)))
        {
            ...
            event.avs_notifications_data.cmd = (p_recv_data[3]==1) ? TUYA_AVS_NOTIFICATIONS_INDICATOR_SET : TUYA_AVS_NOTIFICATIONS_INDICATOR_CLEAR;
...
static void tuya_ble_handle_vos_timer_setting(uint8_t *p_recv_data,uint32_t recv_data_len)
{
    ...
    if((p_recv_data[2]!=TUYA_VOS_TIMER_TIMER)&&(p_recv_data[2]!=TUYA_VOS_TIMER_ALARM)&&(p_recv_data[2]!=TUYA_VOS_TIMER_REMINDER)&&(p_recv_data[2]!=0xFF))
    {
        return;
    }
    ...
    else
    {
        if(p_recv_data[1]==0)//set cmd
        {
            ...
            p_timer_set_data->time = (p_recv_data[19]<<24) + (p_recv_data[20]<<16) + (p_recv_data[21]<<8) + p_recv_data[22];
            p_timer_set_data->loop_count = p_recv_data[23];
            p_timer_set_data->loop_count_pause_time = (p_recv_data[24]<<24) + (p_recv_data[25]<<16) + (p_recv_data[26]<<8) + p_recv_data[27];
    ...
    memcpy(buffer,p_recv_data,19);
    buffer[19] = 0;

    tuya_ble_commData_send(TUYA_BLE_FRM_VOS_TIMER_SETTING_RESP,0,buffer,20,ENCRYPTION_MODE_SESSION_KEY);

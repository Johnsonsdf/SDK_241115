...
void tuya_ble_test_bulk_data_generation(void)
{
    ...
    while(write_addr<TEST_BULK_DATA_TYPE1_END_ADDR)
    {
        if(write_addr%TUYA_NV_ERASE_MIN_SIZE==0)
        {
            ...
            {
                TUYA_APP_SYS_LOG_ERR("test_bulk_data_generation ERROR!");
                bulk_data_total_length = 0;
                bulk_data_total_crc32 = 0;
                return;
            }
        ...
        bulk_data_total_crc32 = tuya_ble_crc32_compute(TLD_Example_Data, sizeof(TLD_Example_Data), &bulk_data_total_crc32);
        dp_time += 60;

        tuya_ble_device_delay_ms(10);
    }

    TUYA_APP_SYS_LOG_INF("test_bulk_data_generation OK, TOTAL LENGTH = %d , TOTAL CRC32 = 0x%08x,",bulk_data_total_length,bulk_data_total_crc32);
...
void tuya_ble_app_bulk_data_handler(tuya_ble_bulk_data_request_t *p_data)
{
    ...
    case TUYA_BLE_BULK_DATA_EVT_READ_INFO:
	    ...
	    if(bulk_data_read_block_size==0)
		{
            ...
            TUYA_APP_SYS_LOG_ERR("Error, Invalid bulk data block size");
        ...
        else if(p_data->bulk_type!=TEST_BULK_DATA_TYPE)     //Our test has only one type of bulk data.
        {
            ...
            TUYA_APP_SYS_LOG_ERR("Invalid bulk type = %d ",p_data->bulk_type);
			current_read_status = TUYA_BLE_BULK_DATA_READ_STATUS_NONE;
        }
        else
        {
			...
			bulk_data_total_blocks = (bulk_data_total_length + bulk_data_read_block_size - 1)/bulk_data_read_block_size;
			
			TUYA_APP_SYS_LOG_INF("Received TUYA_BLE_BULK_DATA_EVT_READ_INFO event, current bulk data TOTAL LENGTH = %d , TOTAL CRC32 = 0x%08x, BLOCK SIZE = %d , BLOCK NUMBERS = %d",
    ...
    case TUYA_BLE_BULK_DATA_EVT_READ_BLOCK:
	    ...
	    if((current_read_status != TUYA_BLE_BULK_DATA_READ_STATUS_READ_INFO)&&(current_read_status != TUYA_BLE_BULK_DATA_READ_STATUS_READ_DATA))
		{
			res_data.bulk_type = p_data->bulk_type;
            res_data.params.block_res_data.status = 3;  //Invalid status.
            TUYA_APP_SYS_LOG_ERR("Invalid status ,current status = %d ",current_read_status);
			current_read_status = TUYA_BLE_BULK_DATA_READ_STATUS_NONE;
		}
    	else if(p_data->bulk_type != current_bulk_type_being_read)     //The bulk data type to be read is not the type specified by the 'TUYA_BLE_BULK_DATA_EVT_READ_INFO' event.
        {
            res_data.bulk_type = p_data->bulk_type;
        ...
        else
        {
            ...
            if(current_block_number > (bulk_data_total_blocks-1))
            {
                ...
                TUYA_APP_SYS_LOG_ERR("Invalid bulk block number,received block number = %d total blocks = %d",current_block_number,bulk_data_total_blocks);
            }
            else
            {
                res_data.params.bulk_info_res_data.status = 0;
                ...
                current_block_size = (current_block_number < (bulk_data_total_blocks - 1)) ?
                                     bulk_data_read_block_size : (bulk_data_total_length - current_block_number * bulk_data_read_block_size); //The data length of the last block may be less than TEST_BULK_DATA_TYPE0_BLOCK_SIZE.
                ...
                TUYA_APP_SYS_LOG_INF("Read block data : block number = %d, block size = %d ,block data crc16 = 0x%04x",
    ...
    case TUYA_BLE_BULK_DATA_EVT_SEND_DATA:
        ...
        {
            TUYA_APP_SYS_LOG_ERR("Invalid bulk type = %d ,not start send bulk data.",p_data->bulk_type);
			current_read_status = TUYA_BLE_BULK_DATA_READ_STATUS_NONE;
        }
        else
        {
            ...
            {
                TUYA_APP_SYS_LOG_ERR("The current_block_total_packets is 0,not start send bulk data.");
                valid = 0;
            }
            else
            {
                ...
                if(p_buf)
                {
					...
					TUYA_APP_SYS_LOG_INF("Current block data crc for send : 0x%04x",crc16_temp);
                ...
                {
                    TUYA_APP_SYS_LOG_ERR("tuya_ble_app_bulk_data_handler malloc failed ,Send termination.");
                    valid = 0;
                }
    ...
    case TUYA_BLE_BULK_DATA_EVT_ERASE:
        ...
        else if(p_data->bulk_type != current_bulk_type_being_read)     ////The bulk data type to be erased is not the type specified by the 'TUYA_BLE_BULK_DATA_EVT_READ_INFO' event.
        {
            res_data.bulk_type = p_data->bulk_type;
        ...
        else
        {
			...
			TUYA_APP_SYS_LOG_INF("received erase cmd.");
    ...
    if(valid)
    {
        ...
        {
        	if (res_data.params.send_res_data.p_current_block_data)
            	tuya_ble_free(res_data.params.send_res_data.p_current_block_data);

			TUYA_APP_SYS_LOG_INF("Send block data : block number = %d, block size = %d",

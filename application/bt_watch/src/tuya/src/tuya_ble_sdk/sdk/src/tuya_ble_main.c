...
tuya_ble_parameters_settings_t *tuya_ble_current_para = NULL;
...
void tuya_ble_connect_monitor_timer_stop(void)
{
    ...
    {
        TUYA_BLE_LOG_ERROR("tuya_ble_xtimer_connect_monitor stop failed");
    }
...
// add by zdx
//extern void tuya_ble_event_process_new(tuya_ble_evt_param_t *tuya_ble_evt);
int tuya_ble_app_handle(void* msg_data, int msg_size)
{
	tuya_ble_evt_param_t *tuya_ble_evt = (tuya_ble_evt_param_t *)msg_data;
	printf("tuya_ble_app_handle: event: %d, size %d\n", tuya_ble_evt->hdr.event, msg_size);
	//dump_memory();
    ...
    switch(tuya_ble_evt->hdr.event)
    {
        ...
        if (tuya_ble_evt->connect_change_evt == TUYA_BLE_DISCONNECTED)
		{
			printf("tuya_ble_app_handle: disconnected\n");
		}
        break;
...
#if TUYA_BLE_USE_OS
    for(i= 0; i<TUYA_BLE_MAX_CALLBACKS; i++)
    {
...
#if TUYA_BLE_SELF_BUILT_TASK    
	printf("tuya_ble_event_send: event %d evt->hdr.event_handler 0x%p\n", evt->hdr.event,evt->hdr.event_handler);
    if(tuya_ble_os_msg_queue_send(tuya_ble_queue_handle, evt, sizeof(tuya_ble_evt_param_t)))
    {
        return 0;
    }
    else
...
#ifdef TUYA_BLE_SELF_BUILT_TASK
    if(tuya_ble_os_msg_queue_send(tuya_ble_queue_handle, &event, sizeof(tuya_ble_evt_param_t)))
    {
        return 0;
    }
    else
...
{
    printf("tuya_ble_inter_event_response:%d", param->evt);
...
uint8_t tuya_ble_cb_event_send(tuya_ble_cb_evt_param_t *evt)
{
    ...
    if(m_cb_queue_table[0])
    {
		...
		printf("tuya_ble_cb_event_send: event %d\n", evt->evt);
        
        if(tuya_ble_os_msg_queue_send((void *)m_cb_queue_table[0], evt, sizeof(tuya_ble_cb_evt_param_t)))
        {
            fun = (tuya_ble_callback_t)m_cb_queue_table[0];
            fun(evt);
    ...
    if(m_cb_table[0])
    {
        fun = (tuya_ble_callback_t)m_cb_table[0];
        fun(evt);
...
void tuya_ble_adv_change(void)
{
	...
	print_hex("adv_data_const", adv_data_const, TUYA_BLE_ADV_DATA_LEN_MAX);
...
void tuya_ble_adv_change(void)
{
	...
	print_hex("[adv]", &adv_data[12], tuya_ble_current_para->pid_len);

...
bool buffer_value_is_all_x(uint8_t *buffer,uint16_t len,uint8_t value)
{
    bool ret = true;
	uint16_t i = 0;
    for(i = 0; i<len; i++)
...
uint32_t tuya_ble_storage_load_settings(void)
{
    ...
    if(auth_settings_flag==1)
    {
        ...
        TUYA_BLE_LOG_INFO("%d %d ", settings_valid, settings_backup_valid);
    ...
    if(sys_settings_flag==1)
    {
        ...
        //print_hex("FLASH_ADDR:\n", (uint8_t *)&p_storage_settings_sys->flash_settings_sys, sizeof(tuya_ble_sys_settings_t));
        ...
        else
        {
            ...
            TUYA_BLE_LOG_ERROR("settings_unvalid!!!");
        }

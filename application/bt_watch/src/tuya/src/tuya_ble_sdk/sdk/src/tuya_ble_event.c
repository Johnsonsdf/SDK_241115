...
static tuya_ble_evt_param_t *tuya_ble_evt = NULL;
void tuya_sched_execute(void)
{
    tuya_ble_evt_param_t *evt;

	if (!tuya_ble_evt) {
    	tuya_ble_evt = (tuya_ble_evt_param_t *)tuya_ble_malloc(sizeof(tuya_ble_evt_param_t)); // zdx
	}

    evt = tuya_ble_evt;

	printf("%s tuya_ble_evt_param_t size=%d\n", __func__, sizeof(tuya_ble_evt_param_t));
    ...
    while (m_queue_start_index != end_ix) //(!TUYA_BLE_SCHED_QUEUE_EMPTY()) 
    {
        ...
        //TUYA_BLE_LOG_INFO("TUYA_RECEIVE_EVT-0x%04x,start index-0x%04x,end index-0x%04x\n",evt->hdr.event,m_queue_start_index,m_queue_end_index);

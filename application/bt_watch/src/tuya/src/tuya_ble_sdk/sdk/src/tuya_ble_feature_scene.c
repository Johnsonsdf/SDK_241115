...
tuya_ble_status_t tuya_ble_feature_scene_request(tuya_ble_request_scene_t *req_data)
{
	...
	event.hdr.event = TUYA_BLE_EVT_SCENE_DATA_REQ;
	event.scene_req_data.p_data 	= ble_evt_buffer;
	event.scene_req_data.data_len	= ble_evt_buffer_len;

	if (tuya_ble_event_send(&event) != 0)
	{
		TUYA_BLE_LOG_ERROR("tuya_event_send scene req error");
...
void tuya_ble_handle_scene_request_evt(tuya_ble_evt_param_t *evt)
{
    ...
    tuya_ble_commData_send(FRM_IOT_DATA_REQUEST, 0, evt->scene_req_data.p_data, evt->scene_req_data.data_len, encry_mode);

    if (evt->scene_req_data.p_data)
    {
        tuya_ble_free(evt->scene_req_data.p_data);
    }
}

...
uint32_t tuya_ble_iot_channel_subpackage_maxsize_get(void)
{
	...
	TUYA_BLE_LOG_INFO("iot channel subpackage maxsize =[%d]", subpackage_size);
...
static uint32_t tuya_ble_iot_data_unpack(uint8_t *buf, uint32_t len)
{
    ...
    {
        offset = 0;
        TUYA_BLE_LOG_INFO("iot_data_unpack[%d]", iot_recv_packet.recv_len);
        return 0;
    }
...
void tuya_ble_handle_iot_data_received(uint8_t *recv_data, uint16_t recv_len)
{
	...
	if (!iot_trsmitr_proc.subpkg)
		iot_trsmitr_proc.subpkg = (u8_t *)tuya_ble_malloc(SNGL_PKG_TRSFR_LMT);
	/* received iot data unpack */
	uint32_t unpack_ret = tuya_ble_iot_data_unpack(recv_data+13, data_len);
	tuya_ble_free(iot_trsmitr_proc.subpkg);
	iot_trsmitr_proc.subpkg = NULL;
	...
	tuya_ble_commData_send(FRM_IOT_DATA_RECEIVED_RESP, ack_sn, p_buf, buf_len, ENCRYPTION_MODE_SESSION_KEY); 

...
tuya_ble_status_t tuya_ble_callback_queue_register(void *cb_queue)
{
	...
	printf("cb_queue 0x%x  0x%x\n", cb_queue,tuya_ble_callback_queue_register);
...
{
    tuya_ble_evt_param_t *event = (tuya_ble_evt_param_t *)tuya_ble_malloc(sizeof(tuya_ble_evt_param_t));
    ...
    {
        ble_evt_buffer =(uint8_t *)tuya_ble_malloc(event->mtu_data.len);
        if(ble_evt_buffer == NULL)
        {
        	tuya_ble_free((u8_t *)event);
            return TUYA_BLE_ERR_NO_MEM;
        }
        memcpy(ble_evt_buffer, p_data,event->mtu_data.len);
		event->mtu_data.p_data = ble_evt_buffer;
    }
...
static uint8_t *UART_RX_Buffer;//[UART_RX_BUFFER_MAX]; zdx
...
tuya_ble_status_t tuya_ble_common_uart_send_full_instruction_received(uint8_t *p_data,uint16_t len)
{
    ...
    else
    {
        ...
        if(tuya_ble_event_send(&event)!=0)
        {
			...
			tuya_ble_free(uart_evt_buffer);            ret = TUYA_BLE_ERR_BUSY;
        }
    }
	//if (uart_evt_buffer)
		//tuya_ble_free(uart_evt_buffer);
...
#endif
extern void sys_ble_advertise(struct thread_timer *ttimer, void *expiry_fn_arg);
...
tuya_ble_status_t tuya_ble_sdk_init(tuya_ble_device_param_t * param_data)
{
	...
	// add by zdx
	UART_RX_Buffer = tuya_ble_malloc(UART_RX_BUFFER_MAX);
#if (TUYA_BLE_SECURE_CONNECTION_TYPE == TUYA_BLE_SECURE_CONNECTION_WITH_AUTH_KEY_ADVANCED_ENCRYPTION) && \
    ...
    {
        TUYA_BLE_LOG_ERROR("tuya_ble_sdk_init param_data->adv_local_name_len error %d.",param_data->adv_local_name_len);
        return TUYA_BLE_ERR_INVALID_PARAM;
    }

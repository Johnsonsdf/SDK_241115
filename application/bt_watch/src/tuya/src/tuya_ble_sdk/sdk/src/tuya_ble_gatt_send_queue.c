...
static tuya_ble_gatt_send_data_t *send_buf;//[TUYA_BLE_GATT_SEND_DATA_QUEUE_SIZE];  //zdx
...
{
	// add by zdx
	send_buf = (tuya_ble_gatt_send_data_t *)tuya_ble_malloc(TUYA_BLE_GATT_SEND_DATA_QUEUE_SIZE * sizeof(tuya_ble_gatt_send_data_t));
	gatt_queue_flag = 0;
...
static void tuya_ble_gatt_send_queue_free(void)
{
	 ...
	 TUYA_BLE_LOG_INFO("tuya_ble_gatt_send_queue_free execute.");
}
...
void tuya_ble_gatt_send_data_handle(void *evt)
{
	...
	while (tuya_ble_queue_get(&gatt_send_queue, &data) == TUYA_BLE_SUCCESS) 
	{
		...
		else
		{
			...
			TUYA_BLE_LOG_INFO("event.hdr.event %d, event.hdr.event_handler 0x%p.",event.hdr.event, event.hdr.event_handler);

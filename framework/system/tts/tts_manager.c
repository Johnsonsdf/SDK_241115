...
static io_stream_t _tts_create_stream(uint8_t *filename, bool tele_num)
{
	struct tts_manager_ctx_t *tts_ctx = _tts_get_ctx();
...
int tts_manager_play_process(void)
{
    ...
    if(tts_item->tts_mode & TTS_MODE_TELE_NUM) {
        ...
        tts_ctx->tts_stream = _tts_create_stream(tele_name, true);
    } else {
        tts_ctx->tts_stream = _tts_create_stream(tts_item->tts_file_name, false);
    }
...
int tip_manager_play(uint8_t *tip_name, uint64_t bt_clk)
{
	...
	tts_ctx->tip_stream = _tts_create_stream(tip_name, false);
	if (!tts_ctx->tip_stream) {
        ret = -1;
		goto exit;
	}

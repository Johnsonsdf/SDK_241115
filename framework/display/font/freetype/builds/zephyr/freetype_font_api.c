...
internal_ft_face_t* _ft_get_internal_face(const char* file_path)
{
	...
	if(common_faces[face_idx].face != NULL)
	{
		...
		if(freetype_font_use_svg_path())
		{
			...
			freetype_glyphshape_t* shapes = (freetype_glyphshape_t*)cache->data;
			if(cache->inited)
			{
				if(cache->glyph_index)
				{
					...
					{
						if(shapes[i].vertices)
						{
							freetype_font_shape_cache_free(shapes[i].vertices);
							shape_cache_debug_size -= shapes[i].num_vertices*sizeof(freetype_vertex_t);
						}
					}				
...
//		SYS_LOG_INF("current %d, current id %d\n", cache->current, cache->glyph_index[cache->current]);
	...
	else
	{
		...
		{
			freetype_glyphshape_t* shape = (freetype_glyphshape_t*)data;
			if(!shape)
			{
			...
			if(shape->vertices)
			{
				SYS_LOG_DBG("fttest free vertices %p, index %d, max %d, total %d, glyf_id %d\n", shape->vertices, cache->current, cache->cached_max, cache->cached_total, cache->glyph_index[cache->current]);
				freetype_font_shape_cache_free(shape->vertices);
				shape->vertices = NULL;
				shape_cache_debug_size -= shape->num_vertices*sizeof(freetype_vertex_t);
			}
		}
...
freetype_font_t* freetype_font_open(const char* file_path, uint32_t font_size)
{
	...
	else
	{
		if(freetype_font_get_memory_face_enabled())
		{
			...
			if(ft_font->internal_face->face == NULL)
			{
				...
				if(ret)
				{
					SYS_LOG_ERR("FT_New_Memory_Face faild %d\n", ret);
					return NULL;
				}
			}
	...
	if(copy_slot < 0)
	{
		...
		{
			//use svg path, no scale
			if(ft_font->internal_face->current_size != font_size)
			{
				error = FT_Set_Pixel_Sizes(ft_font->internal_face->face, 0, font_size);
				if ( error ) {
					SYS_LOG_ERR("Error in FT_Set_Char_Size: %d\n", error);
	...
	if(ft_font != NULL)
	{
		...
		if(opend_font[i].internal_face != NULL)
		{
			...
			else
			{
				...
				if(!freetype_font_use_svg_path())
				{
					FT_Done_Size(opend_font[i].size);	
				}
				*/
...
void _read_bitmap_in_bpp(uint8_t* data, uint8_t* gray_data, int pitch, int rows, int dst_bpp)
{
	...
	//int width = (pitch + ppb - 1)/ppb;
	//uint32_t gray_off = 0;
	uint8_t tail_byte;

	//SYS_LOG_INF("pitch %d, rows %d, width %d\n", pitch, rows, width);
	for(i=0,j=0;i<pitch*rows-ppb+1;i+=ppb,j++)
	{
		if(dst_bpp == 2)
		{
			data[j] = (gray_data[i]&0xc0) | ((gray_data[i+1]&0xc0)>>2) 
					| ((gray_data[i+2]&0xc0)>>4) | ((gray_data[i+3]&0xc0)>>6);
		}
		else if(dst_bpp == 4)
		{
			data[j] = (gray_data[i]&0xf0) | ((gray_data[i+1]&0xf0)>>4);
		}
	}
	tail_byte = 0;

	for(k=i,h=0;k<pitch*rows;k++,h++)
	{
		if(dst_bpp == 2)
		{
			tail_byte = tail_byte | ((gray_data[k]&0xc0)>>(2*h));
		}
		else if(dst_bpp == 4)
		{
			tail_byte = tail_byte | ((gray_data[k]&0xf0)>>(4*h));
		}
	}
	data[j] = tail_byte;
}
...
int freetype_font_load_glyph_shape(freetype_font_t* font, freetype_cache_t *cache,
		uint32_t unicode, freetype_glyphshape_t **pshape)
{
	uint32_t glyf_id;
	uint8_t* data;
	int error;
	...
	//freetype_glyphshape_t* shape = (freetype_glyphshape_t*)_ft_get_glyph_shape_cache(cache, glyf_id, &shape_index);
	SYS_LOG_DBG("glyf_id %d, vertices num %d\n", glyf_id, shape->num_vertices);
	if(shape->num_vertices == 0)
	{
		...
		void* vertices_data = freetype_font_shape_cache_malloc(s_ft_shape.num_vertices*sizeof(freetype_vertex_t));
		if(!vertices_data)
		{
			SYS_LOG_INF("shape cache debug 1: current size %d, peak size %d, malloc %d\n", 
					shape_cache_debug_size, shape_cache_peak_size, s_ft_shape.num_vertices*sizeof(freetype_vertex_t));
			return -1;
		}			
		shape_cache_debug_size += s_ft_shape.num_vertices*sizeof(freetype_vertex_t);
		if(shape_cache_debug_size > shape_cache_peak_size)
		{
			shape_cache_peak_size = shape_cache_debug_size;
		}
		...
		shape->vertices = (freetype_vertex_t*)vertices_data;
		shape->current_size = font->font_size;
	...
	SYS_LOG_DBG("shape->num_vertices %d\n", shape->num_vertices);
	return 0;
}
...
bbox_metrics_t* freetype_font_get_glyph_dsc(freetype_font_t* font, freetype_cache_t *cache, uint32_t unicode)
{
	...
	//					face->glyph->bitmap.rows, cache->unit_size, font->bpp);
		...
		else
		{
			...
			//freetype_glyphshape_t* shape = (freetype_glyphshape_t*)_ft_get_glyph_shape_cache(cache, glyf_id, &shape_cache_index);
			freetype_glyphshape_t* shape = (freetype_glyphshape_t*)data;
			...
			SYS_LOG_DBG("22 unicode 0x%x, glyf_id %d, cache_index %d, num_vertices %d\n", 
				unicode, glyf_id, cache_index, shape->num_vertices);

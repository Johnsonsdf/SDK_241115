...
#else
#undef FT_THROW
#define FT_THROW( e ) (ErrRaster_ ## e)
#endif
...
#define SCALED( x )  ( (x) * ( 1L << shift ) - delta )
                            ...
                            int  continued )
  {
    ...
    {
      error = Smooth_Err_Memory_Overflow;
...
extern unsigned char* gray_pool_buffer;
  static int
  gray_convert_glyph( RAS_ARG )
  {
    const TCoord  yMin = ras.min_ey;
    const TCoord  yMax = ras.max_ey;
    TCell*    buffer = (TCell*)gray_pool_buffer;
    ...
    if(buffer == NULL)
    {
       //printk("invalid gray pool buffer in gray_convert_glyph()");
       return -1;
    }
                      ...
                      const FT_Raster_Params*  params )
  {
    ...
    if ( !( params->flags & FT_RASTER_FLAG_AA ) )
      return Smooth_Err_Invalid_Mode;

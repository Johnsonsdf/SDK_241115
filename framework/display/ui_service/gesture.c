...
/* gesture moving snap to edge anim enable flag */
#define USE_GESTURE_MOVE_SNAP_ANIM     0
...
#if USE_GESTURE_MOVE_SNAP_ANIM == 0 || USE_GESTURE_LONGVIEW_SNAP_ANIM == 0
...
#if USE_GESTURE_MOVE_SNAP_ANIM == 0
...
#if USE_GESTURE_MOVE_SNAP_ANIM == 0
	if (view_has_move_attribute(runtime->view_id))
#else
	if (runtime->view_id == VIEW_INVALID_ID || view_has_move_attribute(runtime->view_id))
#endif
	{	/* move */

 ...
 *
 *  Unregister a Service Record. Applications can make use of
 *  macros such as BT_SDP_DECLARE_SERVICE, BT_SDP_LIST,
 *  BT_SDP_SERVICE_ID, BT_SDP_SERVICE_NAME, etc.
 *  A service declaration must start with BT_SDP_NEW_SERVICE.
 *
...
/** @brief Sdp service list exist or not.
 *
 *  Get status of sdp service register.
 *
 *  @param service Service record declared using BT_SDP_DECLARE_SERVICE.
 *
 * @return ture is exist, false is not exist.
 */
...
/** @brief sdp profile list register/unregister
 */
...
#define BT_SDP_ARRAY_8_CONST(...) ((const uint8_t[]) {__VA_ARGS__})
#define BT_SDP_ARRAY_16_CONST(...) ((const uint16_t[]) {__VA_ARGS__})
#define BT_SDP_ARRAY_32_CONST(...) ((const uint32_t[]) {__VA_ARGS__})

...
/* Actions add start */
bool bt_vs_evt_handle(struct net_buf_simple *buf);
...
#define BT_HCI_OP_WRITE_SYNC_FLOW_CTRL          BT_OP(BT_OGF_BASEBAND, 0x002f)
struct bt_hci_write_sync_flow_ctrl {
	uint8_t enable;
} __packed;
...
struct bt_hci_cp_write_current_iac_lap {
	...
	uint8_t iac_lap[3];	/* Current only support 1 iac */
} __packed;

#define BT_HCI_OP_WRITE_INQUIRY_SCAN_TYPE       BT_OP(BT_OGF_BASEBAND, 0x0043)
//#define BT_HCI_OP_WRITE_INQUIRY_MODE            BT_OP(BT_OGF_BASEBAND, 0x0045)		/* Wait todo */

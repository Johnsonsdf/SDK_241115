...
#include <acts_bluetooth/hci_err.h>
...
#if defined(CONFIG_BT_SMP)							/* For build lib, always keep member */
...
//#endif /* defined(CONFIG_BT_SMP) || defined(CONFIG_BT_BREDR) */
	 ...
	 */
	void (*role_change)(struct bt_conn *conn, uint8_t role);
	 ...
	 */
	void (*mode_change)(struct bt_conn *conn, uint8_t mode, uint16_t interval);
	 ...
	 */
	void (*rx_sco_data)(struct bt_conn *conn, uint8_t *data, uint8_t len, uint8_t pkg_flag);
	 ...
	 *	@param len Data length.
	 */
	void (*rx_connectionless_data)(struct bt_conn *conn, uint8_t *data, uint16_t len);
 ...
 */
#define BT_BR_CONN_PARAM_INIT(role_switch, create_key) \
{ \
	.allow_role_switch = (role_switch), \
	.create_linkkey = (create_key), \
}
...
/** Helper to declare BR/EDR connection parameters inline
  ...
  * @param role_switch True if role switch is allowed
  */
#define BT_BR_CONN_PARAM(role_switch, create_key) \
	((struct bt_br_conn_param[]) { \
		BT_BR_CONN_PARAM_INIT(role_switch, create_key) \
	 })

/** Default BR/EDR connection parameters:
 *    Role switch allowed
 *    Create linkkey when remote without linkkey
 */
#define BT_BR_CONN_PARAM_DEFAULT BT_BR_CONN_PARAM(true, true)

/** BR/EDR connection parameters not active create linkkey:
 *    Role switch allowed
 *    Not active create linkkey when remote without linkkey
 */
#define BT_BR_CONN_PARAM_NAC_LINKKEY BT_BR_CONN_PARAM(true, false)
...
typedef void (*bt_tx_pending_cb)(struct bt_conn *conn, uint8_t pkts);

/** @brief Register conn tx pending callback
 *
 *  @param conn Connection object.

...
/**
 * @brief get ble advertise status
 *
 * get adv status.
 *
 * @return Zero on success or (negative) error code otherwise.
 ...
 */
typedef void bt_br_remote_name_cb_t(bt_addr_t *addr, uint8_t *name);
...
/** @brief BR enable pincoce
 *
 * @param type: 0:disable, 1:enable
 *
 *  @return Zero on success or error code otherwise, positive in case
 *  of protocol error or negative (POSIX) in case of stack internal error
 */
...
struct br_linkkey_info {
	bt_addr_t addr;
	uint8_t linkkey[16];
};

/** @typedef bt_br_resolve_addr_t
 *  @brief Callback type for resolve br address.
 *
 *  @param Resolve br addr.
 */
typedef void bt_br_resolve_addr_t(bt_addr_t *addr);

/** @brief Register resolve br address callback function
 *
 * @param cb callback function for resolve br address.
 *
 * @return Zero if done successfully, other indicator failed.
 */
int bt_br_reg_resolve_cb(bt_br_resolve_addr_t cb);
...
int bt_vs_set_tws_int_clock_new(struct bt_conn *conn, uint8_t index, uint8_t mode, uint32_t clk,
	uint16_t slot, uint32_t per_clk, uint16_t per_slot);

...
static void connect_to(bt_addr_le_t *addr)
{
	int err;
	struct bt_conn *conn;
		      ...
		      struct net_buf_simple *buf)
{
	...
	if (info->adv_type == BT_GAP_ADV_TYPE_ADV_DIRECT_IND) {
		connect_to((bt_addr_le_t *)info->addr);
	}
}
...
static void connected(struct bt_conn *conn, uint8_t err)
{
	...
	if (type == BT_CONN_TYPE_BR || type == BT_CONN_TYPE_SCO) {
		return;
	}
...
{
	char addr[BT_ADDR_LE_STR_LEN];
	uint8_t type = bt_conn_get_type(conn);
...
void bt_le_init(const struct shell *shell)
{
	ctx_shell = shell;
...
static int cmd_conn_update(const struct shell *shell, size_t argc, char *argv[])
{
	...
	param.interval_min = strtoul(argv[1], NULL, 10);
	param.interval_max = strtoul(argv[2], NULL, 10);
	param.latency = strtoul(argv[3], NULL, 10);
	param.timeout = strtoul(argv[4], NULL, 10);
...
#define BLE_TEST_SEND_SIZE sizeof(send_buf)

static bool test_flag;
static uint8_t ble_notify_enable = 0;

static void speed_ccc_cfg_changed(const struct bt_gatt_attr *attr, uint16_t value)
{
	printk("speed ccc: %d\n", value);
	ble_notify_enable = (uint8_t)value;
}

static int speed_rx_data(uint8_t *buf, uint16_t len)
{
...
BT_GATT_SERVICE_DEFINE(
	...
	BT_GATT_PRIMARY_SERVICE(SPEED_BLE_SERVICE_UUID), /* 0 */

	BT_GATT_CHARACTERISTIC(SPEED_BLE_WRITE_UUID,  /* 1-2 */
        BT_GATT_CHRC_WRITE,
        BT_GATT_PERM_WRITE,
        NULL, speed_write_cb, NULL),

	BT_GATT_CHARACTERISTIC(SPEED_BLE_READ_UUID,  /* 3-4 */
        BT_GATT_CHRC_NOTIFY,
        BT_GATT_PERM_READ | BT_GATT_PERM_WRITE,
        NULL, NULL, NULL),		/* or BT_GATT_CHRC_INDICATE */
	BT_GATT_CCC(speed_ccc_cfg_changed, BT_GATT_PERM_READ | BT_GATT_PERM_WRITE),
);

static int speed_notify(struct bt_conn *conn)
{
    int err;

    err = bt_gatt_notify(conn, &speed_svc.attrs[4], send_buf, BLE_TEST_SEND_SIZE);
	if (err < 0) {
        printk("send notify failed(err %d)\n", err);
	}

    return err;
}

static void speed_handler(struct k_timer *timer)
{
	test_flag = false;
}

K_TIMER_DEFINE(speed_timer, speed_handler, NULL);

static int cmd_speed_test(const struct shell *shell, size_t argc, char *argv[])
{
	int count = 0;
	int time = strtoul(argv[1], NULL, 10);

	k_timer_start(&speed_timer, K_SECONDS(time), K_FOREVER);
	test_flag = true;

	while (test_flag) {
		speed_notify(default_conn);
		k_sleep(K_MSEC(20));
		count++;
	}

	shell_print(shell, "trans %dK data(%d) in %us", 
		(BLE_TEST_SEND_SIZE+11)*count/1024, count, time);
	return 0;
}
/* Actions add end */
...
SHELL_STATIC_SUBCMD_SET_CREATE(bt_cmds,
	...
	SHELL_CMD_ARG(speed-test, NULL, "timeout", cmd_speed_test, 2, 0),

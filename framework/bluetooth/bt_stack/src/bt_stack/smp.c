...
/* SMP Wait TODO: derive the LE LTK from the BR/EDR Link Key and derive the BR/EDR Link Key from the LE LTK ;
 * 1. After LE disconnect, need clear bt_keys, or clear bt_keys, when each time alloc;
 * 2. Encryption Change event, Encryption_Enabled, controler not return 0x02 for BR/EDR secure;
 * 3. Open LE_DEFAULT_STAIC_RANDOM_ADDR need update mac for encryption;
 * 4. Derive the BR/EDR Link Key from the LE LTK, BR/EDR link key need to store;
 * 5. As Central role wait to debug.
*/
...
#if defined(CONFIG_BT_PRIVACY) || defined(CONFIG_BT_STATIC_RANDOM_ADDR)
...
#if defined(CONFIG_BT_BREDR_DISABLE) || !defined(CONFIG_BT_BR_ACTS)
#define LINK_DIST 0
#else
...
static struct bt_smp bt_smp_pool[CONFIG_BT_MAX_CONN - CONFIG_BT_MAX_BR_CONN];
...
/* Actions add start */
static bool pairing_wait;
static struct bt_smp_pairing smp_pairing_req_data;
static struct bt_smp *smp_context;
static struct bt_keys_link_key br_link_key;

static void smp_clear_derive_br_linkkey(void)
{
	acts_bt_keys_link_key_clear(&br_link_key);
}

struct bt_keys_link_key *smp_get_derive_br_linkkey(void)
{
	return &br_link_key;
}
/* Actions add end */
...
static uint8_t get_io_capa(void)
{
	...
	if (le_auth->passkey_display && le_auth->passkey_entry &&
	    (le_auth->passkey_confirm || !sc_supported)) {
		return BT_SMP_IO_KEYBOARD_DISPLAY;
	}

	/* DisplayYesNo is useful only for LE SC */
	if (sc_supported && le_auth->passkey_display &&
	    le_auth->passkey_confirm) {
		return BT_SMP_IO_DISPLAY_YESNO;
	}

	if (le_auth->passkey_entry) {
...
#if defined(CONFIG_BT_PRIVACY) || defined(CONFIG_BT_STATIC_RANDOM_ADDR) || defined(CONFIG_BT_SIGNING) || \
...
static void smp_check_complete(struct bt_conn *conn, uint8_t dist_complete)
{
	...
	if (conn->type == BT_CONN_TYPE_LE) {
		...
		BT_ASSERT(chan);
		if (!chan) {
			BT_ERR("smp chan get fail, crash!");
			return;
		}
...
static void sc_derive_link_key(struct bt_smp *smp)
{
	...
	BT_DBG("LTK to BR linkkey");

	/* TODO handle errors? */

	/* Wait TODO:  derive the BR/EDR Link Key from the LE LTK
	 * At this point remote device identity is known so we can use
	 * destination address here
	 */
	link_key = smp_get_derive_br_linkkey();
	smp_clear_derive_br_linkkey();
	...
	if (br_conn) {
		acts_bt_keys_link_key_clear_store(&link_key->addr);
		br_conn->br.link_key = acts_bt_keys_find_link_key_store(&link_key->addr);
		/* BR already connected, need update link key */
...
static void smp_pairing_br_complete(struct bt_smp_br *smp, uint8_t status)
{
	...
	if (status) {
		...
		if (le_auth && le_auth->pairing_failed) {
			le_auth->pairing_failed(smp->chan.chan.conn,
		...
		if (le_auth && le_auth->pairing_complete) {
			le_auth->pairing_complete(smp->chan.chan.conn,
						  bond_flag);
		}
	}
...
static void smp_br_derive_ltk(struct bt_smp_br *smp)
{
	...
	if (atomic_test_bit(smp->flags, SMP_FLAG_CT2)) {
		...
		if (smp_h7(salt, link_key->val, ilk)) {
			acts_bt_keys_link_key_clear(link_key);
			return;
		}
	} else {
		/* constants as specified in Core Spec Vol.3 Part H 2.4.2.5 */
...
#if defined(CONFIG_BT_STATIC_RANDOM_ADDR)
		memset(id_info->irk, 0, 16);
#else
...
static uint8_t smp_br_pairing_req(struct bt_smp_br *smp, struct net_buf *buf)
{
	...
	rsp->auth_req = BT_SMP_AUTH_CT2;	/* Support smp_h7 */
...
static void smp_pairing_complete(struct bt_smp *smp, uint8_t status)
{
	...
	if (!status) {
		...
		if (le_auth && le_auth->pairing_complete) {
			le_auth->pairing_complete(conn, bond_flag);
		}
	} else {
		...
		if (le_auth && le_auth->pairing_failed) {
			le_auth->pairing_failed(conn, security_err);
		}
	}
...
static uint8_t bt_smp_distribute_keys(struct bt_smp *smp)
{
	...
	if (smp->local_dist & BT_SMP_DIST_ID_KEY) {
		...
		id_addr_info = net_buf_add(buf, sizeof(*id_addr_info));
#if defined(CONFIG_BT_STATIC_RANDOM_ADDR)
		id_addr_info->addr.type = 0;
		bt_addr_copy(&id_addr_info->addr.a, &bt_dev.read_bd_addr);
#else
		bt_addr_le_copy(&id_addr_info->addr, &bt_dev.id_addr[conn->id]);
#endif
				    ...
				    struct bt_smp_pairing *pairing)
{
	...
	if (le_auth && le_auth->pairing_accept) {
		...
		return smp_err_get(le_auth->pairing_accept(conn, &feat));
	}
...
static uint8_t legacy_request_tk(struct bt_smp *smp)
{
	...
	switch (smp->method) {
	case LEGACY_OOB:
		if (le_auth && le_auth->oob_data_request) {
			struct bt_conn_oob_info info = {
				.type = BT_CONN_OOB_LE_LEGACY,
			};

			atomic_set_bit(smp->flags, SMP_FLAG_USER);
			le_auth->oob_data_request(smp->chan.chan.conn, &info);
		} else {
			return BT_SMP_ERR_OOB_NOT_AVAIL;
		}
	...
	case PASSKEY_DISPLAY:
		...
		if (le_auth && le_auth->passkey_display) {
			atomic_set_bit(smp->flags, SMP_FLAG_DISPLAY);
			le_auth->passkey_display(conn, passkey);
		}
...
static uint8_t legacy_pairing_req(struct bt_smp *smp)
{
	...
	if ((DISPLAY_FIXED(smp) || smp->method == JUST_WORKS) &&
	    !atomic_test_bit(smp->flags, SMP_FLAG_SEC_REQ) &&
	    le_auth && le_auth->pairing_confirm) {
		atomic_set_bit(smp->flags, SMP_FLAG_USER);
		le_auth->pairing_confirm(smp->chan.chan.conn);
		return 0;
	}
...
#if defined(CONFIG_BT_BREDR)
	auth |= BT_SMP_AUTH_CT2;		/* Support smp_h7 */
#endif
...
static bool sec_level_reachable(struct bt_conn *conn)
{
	...
	case BT_SECURITY_L3:
		return get_io_capa() != BT_SMP_IO_NO_INPUT_OUTPUT ||
		       (le_auth && le_auth->oob_data_request);
	case BT_SECURITY_L4:
		return (get_io_capa() != BT_SMP_IO_NO_INPUT_OUTPUT ||
			(le_auth && le_auth->oob_data_request)) && sc_supported;
	default:
		return false;
	}
}
...
static uint8_t smp_pairing_req_deal(struct bt_smp *smp, struct bt_smp_pairing *data)
{
	struct bt_conn *conn = smp->chan.chan.conn;
	struct bt_smp_pairing *req = data;
	struct bt_smp_pairing *rsp;
	uint8_t err;
	...
	if (pairing_wait) {
		printk("Pending smp pairing\n");
		memcpy(&smp_pairing_req_data, data, sizeof(struct bt_smp_pairing));
		smp_context = smp;
		return 0;
	}
...
static uint8_t smp_pairing_req(struct bt_smp *smp, struct net_buf *buf)
{
	...
	smp_clear_derive_br_linkkey();
    return smp_pairing_req_deal(smp, req);
}
#else
static uint8_t smp_pairing_req(struct bt_smp *smp, struct net_buf *buf)
{
...
void bt_set_pairing_mode(bool wait, bool reset)
{
	bool pairing_wait_last = pairing_wait;

	printk("smp pairing wait %d reset %d\n", wait, reset);
	if (reset) {
		pairing_wait = false;
		smp_context = NULL;
		return;
	...
	pairing_wait = wait;
	if((wait == false) && (pairing_wait_last)){
		if (smp_context) {
			smp_pairing_req_deal(smp_context, &smp_pairing_req_data);
			smp_context = NULL;
		}
	}
}
...
static uint8_t smp_pairing_random(struct bt_smp *smp, struct net_buf *buf)
{
	...
	if (smp->chan.chan.conn->role == BT_HCI_ROLE_MASTER) {
		...
		case PASSKEY_CONFIRM:
			...
			atomic_set_bit(smp->flags, SMP_FLAG_USER);
			atomic_set_bit(smp->flags, SMP_FLAG_DHKEY_SEND);
			le_auth->passkey_confirm(smp->chan.chan.conn, passkey);
			return 0;
		case JUST_WORKS:
			break;
		case LE_SC_OOB:
...
static uint8_t display_passkey(struct bt_smp *smp)
{
	...
	if (le_auth && le_auth->passkey_display) {
		atomic_set_bit(smp->flags, SMP_FLAG_DISPLAY);
		le_auth->passkey_display(smp->chan.chan.conn, smp->passkey);
	}
...
static uint8_t smp_public_key_slave(struct bt_smp *smp)
{
	...
	case PASSKEY_INPUT:
		atomic_set_bit(&smp->allowed_cmds, BT_SMP_CMD_PAIRING_CONFIRM);
		atomic_set_bit(smp->flags, SMP_FLAG_USER);
		le_auth->passkey_entry(smp->chan.chan.conn);
		break;
	case LE_SC_OOB:
		atomic_set_bit(&smp->allowed_cmds, BT_SMP_CMD_PAIRING_RANDOM);
		break;
	...
	if (IS_ENABLED(CONFIG_BT_CENTRAL) &&
		...
		case LE_SC_OOB:
			...
			if (le_auth && le_auth->oob_data_request) {
				...
				le_auth->oob_data_request(smp->chan.chan.conn,
							  &info);
			} else {
				return BT_SMP_ERR_OOB_NOT_AVAIL;
			}
...
int bt_smp_security_is_start(struct bt_conn *conn)
{
	struct bt_smp *smp;
	...
	return (atomic_get(smp->flags)) ? 1 : 0;
}
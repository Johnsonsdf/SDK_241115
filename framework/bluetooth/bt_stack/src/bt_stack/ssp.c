...
#if defined(CONFIG_BT_LINK_MISS_REJECT)
#define BT_SSP_LINK_MISS_REJECT		(1)
#else
#define BT_SSP_LINK_MISS_REJECT		(0)
#endif

#if defined(CONFIG_BT_DEVICE_ENCRYPT_FORCE)
#define BT_SSP_DEVICE_ENCRYPT_FORCE		(1)
#else
#define BT_SSP_DEVICE_ENCRYPT_FORCE		(0)
#endif
...
void hci_evt_link_key_notify(struct net_buf *buf)
{
	...
	if (!conn->br.link_key) {
		conn->br.link_key = acts_bt_keys_get_link_key(&evt->bdaddr);
	}
	if (!conn->br.link_key) {
	...
	acts_bt_keys_link_key_store(conn->br.link_key, valid_key, (!atomic_test_bit(conn->flags, BT_CONN_BR_NOBOND)));
	bt_conn_unref(conn);
}
...
void hci_evt_link_key_req(struct net_buf *buf)
{
	...
	if (!conn->br.link_key) {
		conn->br.link_key = acts_bt_keys_find_link_key_store(&evt->bdaddr);
	}
...
void hci_evt_io_capa_req(struct net_buf *buf)
{
	...
	BT_DBG("link_key %p %d %d %d.", 
		conn->br.link_key, conn->br.link_key->flags,conn->br.active_connect,conn->br.create_linkkey);

	if (( (conn->br.link_key == NULL) || (conn->br.link_key->flags & BT_LINK_KEY_REPLY)) &&
		(conn->br.create_linkkey == 0) && BT_SSP_LINK_MISS_REJECT && conn->br.active_connect) {
	...
	if (BT_SSP_DEVICE_ENCRYPT_FORCE) {
		cp->authentication = (auth == BT_HCI_NO_BONDING) ? BT_HCI_GENERAL_BONDING_MITM : auth;
	} else {
		cp->authentication = (auth == BT_HCI_NO_BONDING) ? BT_HCI_GENERAL_BONDING : auth;
	}
	cp->oob_data = 0U;
...
void hci_evt_auth_complete(struct net_buf *buf)
{
	...
	if (evt->status) {
		...
		if (evt->status == BT_HCI_ERR_AUTH_FAIL) {
			bt_addr_copy(&bt_addr, &conn->br.dst);
			acts_bt_keys_link_key_clear_store(&bt_addr);
		}

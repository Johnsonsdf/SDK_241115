...
#include <acts_bluetooth/services/ots.h>
...
static void bt_gatt_load_hash(void);
static void bt_gatt_load_sc(uint8_t id, const bt_addr_le_t *addr);
static void bt_gatt_load_cf(uint8_t id, const bt_addr_le_t *addr);
static void bt_gatt_load_ccc(uint8_t id, const bt_addr_le_t *addr);
#endif
/* Actions add end */
...
static void sc_store(struct gatt_sc_cfg *cfg)
{
	...
	if (cfg->id) {
		...
		bt_property_encode_key(key, sizeof(key), CFG_BT_SC,
				       &cfg->peer, id_str);
	} else {
		bt_property_encode_key(key, sizeof(key), CFG_BT_SC,
				       &cfg->peer, NULL);
	...
	err = bt_property_set(key, (char *)&cfg->data, sizeof(cfg->data));
	if (err) {
...
static void db_hash_store(void)
{
	...
	err = bt_property_set(CFG_BT_HASH, db_hash, sizeof(db_hash));
	if (err) {
...
static void db_hash_gen(bool store)
{
	...
	if (IS_ENABLED(CONFIG_BT_PROPERTY) && store) {
		db_hash_store();
	}
}
...
u16_t gatt_start_handle_rec;
u16_t gatt_end_handle_rec;
u16_t gobr_svc_uuid;

static void bt_gatt_update_start_end_handle(void)
{
	struct bt_gatt_service *svc;
	u16_t start_handle = 0xFFFF;
	u16_t end_handle = 0x0;
			...
			for (i = 0; i < svc->attr_count; i++) {
				...
				if (0 == i) {
					...
					if (uuid->type == BT_UUID_TYPE_16) {
						gobr_svc_uuid = sys_cpu_to_le16(BT_UUID_16(uuid)->val);
					} 
				}
	...
	if (start_handle != 0xFFFF && end_handle != 0) {
		gatt_start_handle_rec = start_handle;
		gatt_end_handle_rec = end_handle;
	} else {
		gatt_start_handle_rec = 0;
		gatt_end_handle_rec = 0;
	}
	
	BT_INFO("gatt_start_handle_rec 0x%x gatt_end_handle_rec 0x%x, gobr_svc_uuid 0x%x.",
...
#ifdef CONFIG_GATT_OVER_BREDR
	bt_gatt_update_start_end_handle();
#endif
	return 0;
}
#endif /* CONFIG_BT_GATT_DYNAMIC_DB */
...
#if defined(CONFIG_BT_PROPERTY_CCC_STORE_ON_WRITE)
...
#if defined(CONFIG_BT_PROPERTY)
	/* Load hash after sc work init, because we should submit sc work
	   when db hash not matched */
	bt_gatt_load_hash();
#endif
}

#if defined(CONFIG_BT_GATT_DYNAMIC_DB) || \
    (defined(CONFIG_BT_GATT_CACHING) && defined(CONFIG_BT_PROPERTY))
...
#endif /* BT_GATT_DYNAMIC_DB || (BT_GATT_CACHING && BT_PROPERTY) */
...
static int ccc_set(const char *name, size_t len_rd, property_read_cb read_cb,
		   char *cb_arg)
{
	if (IS_ENABLED(CONFIG_BT_PROPERTY)) {
		...
		bt_property_name_next(name, &next);
		...
		err = bt_property_decode_key(name, &addr);
		if (err) {
		...
		if (len_rd) {
			len = read_cb(cb_arg, (char *)ccc_store, sizeof(ccc_store));
...
static void bt_gatt_load_ccc(uint8_t id, const bt_addr_le_t *addr)
{
	...
	if (id) {
		...
		bt_property_encode_key(key, sizeof(key), CFG_BT_CCC,
					    addr, id_str);
	} else {
		bt_property_encode_key(key, sizeof(key), CFG_BT_CCC,
					    addr, NULL);
	...
	name = key + strlen(CFG_BT_CCC) + 1;
	ccc_set(name, strlen(name), bt_property_get, key);
}
...
#if defined(CONFIG_BT_PROPERTY)
#if defined(CONFIG_BT_GATT_SERVICE_CHANGED)
	bt_gatt_load_sc(conn->id, &conn->le.dst);
#endif

#if defined(CONFIG_BT_GATT_CACHING)
	bt_gatt_load_cf(conn->id, &conn->le.dst);
#endif

	/* Load CCC settings TODO: should we load ccc if not bonded*/
	bt_gatt_load_ccc(conn->id, &conn->le.dst);
#endif

	/* Load CCC settings from backend if bonded */
	if (IS_ENABLED(CONFIG_BT_PROPERTY_CCC_LAZY_LOADING) &&
	    bt_addr_le_is_bonded(conn->id, &conn->le.dst)) {
#if defined(CONFIG_BT_SETTINGS)
...
#if 0		/* Update ccc at conn_auto_initiate end */
...
void bt_gatt_connected_update_ccc(struct bt_conn *conn)
{
	struct conn_data data;
...
static int bt_gatt_store_cf(struct bt_conn *conn)
{
	...
	if (!cfg) {
		...
		if (conn->id) {
			...
			u8_to_dec(id_str, sizeof(id_str), conn->id);
			bt_property_encode_key(key, sizeof(key), CFG_BT_CF,
					       &conn->le.dst, id_str);
		}
	}
	...
	err = bt_property_set(key, str, len);
	if (err) {
...
int bt_gatt_store_ccc(uint8_t id, const bt_addr_le_t *addr)
{
	...
	if (id) {
		...
		u8_to_dec(id_str, sizeof(id_str), id);
		bt_property_encode_key(key, sizeof(key), CFG_BT_CCC, addr, id_str);
	} else {
		bt_property_encode_key(key, sizeof(key), CFG_BT_CCC, addr, NULL);
	}
...
#if defined(CONFIG_BT_GATT_SERVICE_CHANGED)
static int sc_set(const char *name, size_t len_rd, property_read_cb read_cb,
		  char *cb_arg)
{
	struct gatt_sc_cfg *cfg;
	uint8_t id;
	...
	if (len_rd) {
		len = read_cb(cb_arg, (char *)&cfg->data, sizeof(cfg->data));
		if (len < 0) {
			BT_DBG("Property %s not exist", cb_arg);
			return len;
		}
...
static void bt_gatt_load_sc(uint8_t id, const bt_addr_le_t *addr)
{
	...
	if (id) {
		...
		u8_to_dec(id_str, sizeof(id_str), id);
		bt_property_encode_key(key, sizeof(key), CFG_BT_SC, addr, id_str);
	} else {
		bt_property_encode_key(key, sizeof(key), CFG_BT_SC, addr, NULL);
	}

	name = key + strlen(CFG_BT_SC) + 1;
	sc_set(name, strlen(name), bt_property_get, key);
...
#if defined(CONFIG_BT_GATT_CACHING)
static int cf_set(const char *name, size_t len_rd, property_read_cb read_cb,
		  char *cb_arg)
{
	struct gatt_cf_cfg *cfg;
	bt_addr_le_t addr;
...
static void bt_gatt_load_cf(uint8_t id, const bt_addr_le_t *addr)
{
	...
	if (id) {
		...
		u8_to_dec(id_str, sizeof(id_str), id);
		bt_property_encode_key(key, sizeof(key), CFG_BT_CF, addr, id_str);
	} else {
		bt_property_encode_key(key, sizeof(key), CFG_BT_CF, addr, NULL);
	}

	name = key + strlen(CFG_BT_CF) + 1;
	cf_set(name, strlen(name), bt_property_get, key);
}

static uint8_t stored_hash[16];

static int db_hash_set(property_read_cb read_cb, char *cb_arg)
{
	ssize_t len;
...
static void bt_gatt_load_hash(void)
{
	db_hash_set(bt_property_get, CFG_BT_HASH);
	db_hash_commit();
}
#endif /*CONFIG_BT_GATT_CACHING */

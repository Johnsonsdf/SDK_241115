...
#define LOG_MODULE_NAME bt_att_ext
...
K_MEM_SLAB_DEFINE(att_slab, sizeof(struct bt_att),
		  (CONFIG_BT_MAX_CONN - CONFIG_BT_MAX_BR_CONN), __alignof__(struct bt_att));
K_MEM_SLAB_DEFINE(chan_slab, sizeof(struct bt_att_chan),
		   (CONFIG_BT_MAX_CONN - CONFIG_BT_MAX_BR_CONN) * ATT_CHAN_MAX,
		     ...
		     bt_att_chan_sent_t cb)
{
	...
	err = bt_l2cap_send_cb(chan->att->conn, cid,
...
static int process_queue(struct bt_att_chan *chan, struct k_fifo *queue)
{
	...
	if (chan->att->conn->state != BT_CONN_CONNECTED) {
		BT_ERR("att not connected!");
		return -ENOTCONN;
	}
...
static int chan_req_send(struct bt_att_chan *chan, struct bt_att_req *req)
{
	...
	if (chan->att->conn->type == BT_CONN_TYPE_LE) {
		if (chan->chan.tx.mtu < net_buf_frags_len(req->buf)) {
			return -EMSGSIZE;
		}
	} else {
		if (chan->br_chan.tx.mtu < net_buf_frags_len(req->buf)) {
			return -EMSGSIZE;
		}
	}
				       ...
				       size_t len)
{
	...
	if (chan->att->conn->type == BT_CONN_TYPE_LE) {
		...
		if (len + sizeof(op) > chan->br_chan.tx.mtu) {
			BT_WARN("ATT MTU exceeded, max %u, wanted %zu",
				chan->br_chan.tx.mtu, len + sizeof(op));
			return NULL;
		}
	}
...
static uint8_t att_mtu_req(struct bt_att_chan *chan, struct net_buf *buf)
{
	...
	if (conn->type == BT_CONN_TYPE_BR) {
		mtu_server = chan->br_chan.rx.mtu;
	}
	...
	if (conn->type == BT_CONN_TYPE_BR) {
		chan->br_chan.rx.mtu = MIN(mtu_client, mtu_server);
		chan->br_chan.tx.mtu = chan->br_chan.rx.mtu;

		BT_DBG("Negotiated MTU %u", chan->br_chan.rx.mtu);
	}
...
static uint8_t att_mtu_rsp(struct bt_att_chan *chan, struct net_buf *buf)
{
	...
	if (conn->type == BT_CONN_TYPE_BR) {
		chan->br_chan.rx.mtu = MIN(mtu, chan->br_chan.rx.mtu);
			    ...
			    void *user_data)
{
	...
	case BT_ATT_INFO_16:
		...
		if (conn->type == BT_CONN_TYPE_LE) {
			...
			if (chan->br_chan.tx.mtu - data->buf->len >
				sizeof(*data->info16)) {
				return BT_GATT_ITER_CONTINUE;
			}
		}
			    ...
			    void *user_data)
{
	...
	if (conn->type == BT_CONN_TYPE_LE) {
		...
		mtu = chan->br_chan.tx.mtu;
	}

	/* stop if there is no space left */
	if (mtu - net_buf_frags_len(data->buf) < sizeof(*data->group)) {
		return BT_GATT_ITER_STOP;
	}

	frag = net_buf_frag_last(data->buf);

	len = MIN(mtu - net_buf_frags_len(data->buf),
			     ...
			     attr_read_cb cb, void *user_data)
{
	 ...
	 */
	do {
		len = MIN(mtu - net_buf_frags_len(buf),
			  net_buf_tailroom(frag));
		if (!len) {
		...
		total += read;
		offset += read;
	} while (mtu > net_buf_frags_len(buf) && read == len);
			    ...
			    void *user_data)
{
	...
	return mtu - net_buf_frags_len(data->buf) >
...
#if 1 // def CONFIG_GATT_OVER_BREDR
	if (conn->type == BT_CONN_TYPE_LE) {
		chan = bt_l2cap_le_lookup_rx_cid(conn, BT_L2CAP_CID_ATT);
	} else {
		chan = bt_att_lookup_br_chan(conn);
	}
#else
	chan = bt_l2cap_le_lookup_rx_cid(conn, BT_L2CAP_CID_ATT);
#endif
...
static void bt_att_connected(struct bt_l2cap_chan *chan)
{
	...
	if (chan->conn->type == BT_CONN_TYPE_BR) {
		//att_chan = bt_att_chan_lookup_br_chan(chan->conn);
		att_chan = ATT_CHAN(chan);
	}
	...
	if (!att) {
		BT_DBG("Ignore disconnected ATT chan");
		return;
	}
	...
	if (!atomic_test_bit(att_chan->flags, ATT_ENHANCED)) {
		if (chan->conn->type == BT_CONN_TYPE_LE) {
	...
	BT_DBG("att %p att_chan %p", att, att_chan);
	//sys_slist_append(&att->chans, &att_chan->node);
}
...
struct bt_att_chan *att_chan_new(struct bt_att *att, atomic_val_t flags)
{
	...
	BT_DBG("att->chans %p", &att->chans);

	SYS_SLIST_FOR_EACH_CONTAINER(&att->chans, chan, node) {
		BT_DBG("chan %p chan->att %p att %p", chan,chan->att,att);
		if (chan->att == att) {
			quota++;
		}
...
uint16_t bt_att_get_mtu(struct bt_conn *conn)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&att->chans, chan, tmp, node) {
		if (conn->type == BT_CONN_TYPE_LE) {
			BT_DBG("chan %p tx.mtu %d mtu %d.",chan, chan->chan.tx.mtu, mtu);
			if (chan->chan.tx.mtu > mtu) {
				mtu = chan->chan.tx.mtu;
			}
		} else {
			BT_DBG("chan %p tx.mtu %d mtu %d.",chan, chan->br_chan.tx.mtu, mtu);
			if (chan->br_chan.tx.mtu > mtu) {
				mtu = chan->br_chan.tx.mtu;
			}
		}
	}
		...
		void *user_data)
{
	...
	if (cb) {
		if (conn->type == BT_CONN_TYPE_LE) {
			...
			SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&att->chans, chan, tmp, node) {
				return bt_l2cap_send_cb(conn, chan->br_chan.tx.cid, buf, cb,
							user_data);
			}
		}
	}

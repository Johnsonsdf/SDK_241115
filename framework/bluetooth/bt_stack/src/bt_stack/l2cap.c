...
void bt_l2cap_profile_init(void);
 ...
 */
BT_BUF_POOL_DEFINE(disc_pool, 1,
			  BT_L2CAP_BUF_SIZE(
				sizeof(struct bt_l2cap_sig_hdr) +
				sizeof(struct bt_l2cap_disconn_req)), 4, NULL, &host_tx_pool);
...
static struct net_buf *bt_l2cap_create_conn_pdu_len(struct net_buf_pool *pool, size_t reserve, uint16_t len)
{
	struct net_buf *buf;
	uint16_t malloc_len;
...
struct net_buf *bt_l2cap_create_pdu_len(struct net_buf_pool *pool, size_t reserve, uint16_t len)
{
	uint16_t malloc_len = (uint16_t)(len + sizeof(struct bt_l2cap_hdr) + reserve);

	return bt_l2cap_create_conn_pdu_len(pool, sizeof(struct bt_l2cap_hdr) + reserve, malloc_len);
}

int bt_l2cap_chan_send_with_cb(struct bt_l2cap_chan *chan, struct net_buf *buf, void(*cb)(struct bt_conn *, void *))
{
	if (!buf) {
		return -EINVAL;
	}
	...
	if (IS_ENABLED(CONFIG_BT_BREDR) &&
	    chan->conn->type == BT_CONN_TYPE_BR) {
		return bt_l2cap_br_chan_send_with_cb(chan, buf, cb);
	}

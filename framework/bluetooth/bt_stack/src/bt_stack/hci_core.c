...
#define BT_HCI_SEND_CMD_DEBUG      1
static bool btreset = 0;
static bool bt_hci_wait = 0;
...
static uint16_t bt_device_id[4] = {BT_DID_ACTIONS_COMPANY_ID, BT_DID_DEFAULT_COMPANY_ID, BT_DID_PRODUCT_ID, BT_DID_VERSION};
...
#define BT_DATA_HRTIMER_TIME		(1000*1000*10)		/* 10s */
static struct hrtimer bt_data_hrtimer;
struct k_delayed_work reset_work;
...
struct net_buf *bt_hci_cmd_create(uint16_t opcode, uint8_t param_len)
{
	...
	buf = net_buf_alloc(&hci_cmd_pool, K_SECONDS(10));
	__ASSERT_NO_MSG(buf);

	BT_DBG("buf %p", buf);
	if (!buf) {
		BT_ERR("alloc fail, buf %p", buf);
		if (0 == btreset) {
			btreset = 1;
			ready_cb(-EINTR);
			printk("%d hci cmd exit.\n",__LINE__);
		}
			 ...
			 struct net_buf **rsp)
{
	...
	bt_hci_wait = 0;
#ifdef BT_HCI_SEND_CMD_DEBUG
	if (err != 0) {
		BT_ERR("PMU_DET: %x\n",sys_read32(0x40004010));
		BT_ERR("PMUGATE_DIG: %x\n",sys_read32(0x40004030));
		BT_ERR("HOSC_CTL: %x\n",sys_read32(0x40000100));
		BT_ERR("LOSC_CTL: %x\n",sys_read32(0x40000110));
		BT_ERR("RC4M_CTL: %x\n",sys_read32(0x40000114));
		BT_ERR("opcode 0x%04x err: %d\n", opcode,err);
...
static inline bool rpa_is_new(void)
{
	 ...
	 * timeout was started.
	 */
	return k_ticks_to_ms_floor32(k_work_delayable_remaining_get(&bt_dev.rpa_update.work)) >
	       (RPA_TIMEOUT_MS - 500);
#else
	return false;
#endif
...
{
	if (hrtimer_is_running(&bt_data_hrtimer)) {
		hrtimer_stop(&bt_data_hrtimer);
	}
}
...
static void hci_num_completed_packets(struct net_buf *buf)
{
	...
	for (i = 0; i < evt->num_handles; i++) {
		uint16_t handle, count, count_bak;
		struct bt_conn *conn;
		...
		if (!conn) {
			BT_ERR("No connection for handle 0x%x", handle);
			continue;
		}
		...
		if (conn->type == BT_CONN_TYPE_SCO) {
			...
			if (count_bak) {
				bt_conn_tx_complete_notify(conn);
			}
		}
...
int bt_le_create_conn_cancel(void)
{
	...
	BT_ASSERT(buf);
	if (!buf) {
		BT_ERR("HCI cmd create fail, crash!");
		return -EFAULT;
	}
...
static void conn_auto_initiate(struct bt_conn *conn)
{
	...
	if (IS_ENABLED(CONFIG_BT_AUTO_DATA_LEN_UPDATE) &&
	    BT_FEAT_LE_DLE(bt_dev.le.features) && BT_FEAT_LE_DLE(conn->le.features)) {
	...
	/* Is beter do bt_gatt_connected_update_ccc after DLE finish,
	 * but avoid DLE event continually, just do after DLE.
	 */
	bt_gatt_connected_update_ccc(conn);
}
...
static bool bt_le_check_can_accept_connect(void)
{
	if (bt_conn_connected_le_cnt() >= bt_inner_value.le_max_conn) {
		return false;
	} else {
		return true;
	}
...
static void enh_conn_complete(struct bt_hci_evt_le_enh_conn_complete *evt)
{
	...
	if (IS_ENABLED(CONFIG_BT_PERIPHERAL) &&
		evt->role == BT_HCI_ROLE_SLAVE &&
		!bt_le_check_can_accept_connect()) {
		BT_ERR("No more le conn resources %d", bt_conn_connected_le_cnt());
		bt_hci_disconnect(handle, BT_HCI_ERR_INSUFFICIENT_RESOURCES);
		bt_conn_unref(conn);		/* unref conn ref by find_pending_connect */
		bt_conn_unref(conn);		/* unref conn for tirgger bt_le_adv_resume */
		return;
	}
...
static void le_data_len_change(struct net_buf *buf)
{
	...
	BT_DBG("max. tx: %u (%uus), max. rx: %u (%uus)", sys_le16_to_cpu(evt->max_tx_octets),
	       sys_le16_to_cpu(evt->max_tx_time), sys_le16_to_cpu(evt->max_rx_octets), sys_le16_to_cpu(evt->max_rx_time));
	...
	conn->le.data_len.tx_max_len = sys_le16_to_cpu(evt->max_tx_octets);
	conn->le.data_len.tx_max_time = sys_le16_to_cpu(evt->max_tx_time);
	conn->le.data_len.rx_max_len = sys_le16_to_cpu(evt->max_rx_octets);
	conn->le.data_len.rx_max_time = sys_le16_to_cpu(evt->max_rx_time);
...
static void le_conn_update_complete(struct net_buf *buf)
{
	...
	notify_le_param_updated(conn,evt->status);
...
static void unpair(uint8_t id, const bt_addr_le_t *addr)
{
	...
	if (IS_ENABLED(CONFIG_BT_BREDR)) {
		...
		if (addr->type == BT_ADDR_LE_PUBLIC) {
			acts_bt_le_unpair_clear_public_addr(&addr->a);
		}
	}
...
/* For build lib, alway set, bt_trans.lib used, bt_trans.lib can process */
...
static void conn_complete(struct net_buf *buf)
{
	...
	bt_hci_cmd_send(BT_HCI_OP_READ_REMOTE_FEATURES, buf);
}
...
static void hci_encrypt_change(struct net_buf *buf)
{
	...
	if (conn->type == BT_CONN_TYPE_BR) {
		...
		if (conn->encrypt) {
			conn->br.link_key->flags &= ~BT_LINK_KEY_REPLY;
		}
...
static void hci_cmd_complete(struct net_buf *buf)
{
	...
	if (ncmd) {
		...
		if (atomic_test_and_clear_bit(bt_dev.flags, BT_DEV_WAIT_NCMD_SEM)) {
			k_poll_signal_raise(&bt_dev.ncmd_signal, 0);
		}
	}
}
			...
			struct net_buf *buf, uint8_t len)
{
	...
	memcpy(&scan_info, info, sizeof(struct bt_le_scan_recv_info));
	BT_DBG("%s event %u, len %u, rssi %d dBm", bt_addr_le_str(addr),
	       scan_info.adv_type, len, scan_info.rssi);

	if (!IS_ENABLED(CONFIG_BT_PRIVACY) &&
	    !IS_ENABLED(CONFIG_BT_SCAN_WITH_IDENTITY) &&
	    atomic_test_bit(bt_dev.flags, BT_DEV_EXPLICIT_SCAN) &&
	    (scan_info.adv_props & BT_HCI_LE_ADV_PROP_DIRECT)) {
	...
	scan_info.addr = &id_addr;

	if (scan_dev_found_cb) {
		net_buf_simple_save(&buf->b, &state);

		buf->len = len;
		scan_dev_found_cb(&id_addr, scan_info.rssi, scan_info.adv_type,
				  &buf->b);
...
static void send_cmd(void)
{
	...
	BT_ASSERT(buf);
	if (!buf) {
		BT_ERR("cmd_tx_queue get fail, crash!");
		return;
	}
	...
	if ((timestamp - buf->timestamp) > NET_BUF_TIMESTAMP_CHECK_TIME) {
		BT_WARN("Tx thread cmd time %d ms", (timestamp - buf->timestamp));
	}
#endif
...
static void hci_tx_thread(void *p1, void *p2, void *p3)
{
	...
	while (1) {
		...
		if (atomic_test_bit(bt_dev.flags, BT_DEV_WAIT_NCMD_SEM)) {
			k_poll_signal_reset(&bt_dev.ncmd_signal);
...
static void vs_le_write_local_features(void)
{
	struct bt_hci_cp_vs_le_write_local_features *cp;
	struct net_buf *buf;
	uint8_t *fea;
	...
	bt_dev.le.features[(BT_LE_FEAT_BIT_CONN_PARAM_REQ >> 3)] &= (~(BIT((BT_LE_FEAT_BIT_CONN_PARAM_REQ) & 7)));
	buf = bt_hci_cmd_create(BT_HCI_OP_VS_LE_WRITE_LOCAL_FEATURES, sizeof(*cp));
	if (!buf) {
		return;
	}

	fea = net_buf_add(buf, sizeof(*cp));
	memcpy(fea, bt_dev.le.features, sizeof(bt_dev.le.features));
	bt_hci_cmd_send_sync(BT_HCI_OP_VS_LE_WRITE_LOCAL_FEATURES, buf, NULL);
}
...
static int le_init(void)
{
	...
	vs_le_write_local_features();

	/* Read Low Energy Supported Features again  */
	err = bt_hci_cmd_send_sync(BT_HCI_OP_LE_READ_LOCAL_FEATURES, NULL,
				   &rsp);
	if (err) {
		return err;
...
static uint8_t bt_read_public_addr(bt_addr_le_t *addr)
{
	...
	bt_addr_copy(&bt_dev.read_bd_addr, &rp->bdaddr);
	net_buf_unref(rsp);
	return 1U;
}
...
static int hci_init(void)
{
	...
	hci_vs_init();
#endif
	if (!IS_ENABLED(CONFIG_BT_PROPERTY) && !bt_dev.id_count) {
...
int bt_send(struct net_buf *buf)
{
	...
	if (BT_BUF_ACL_OUT == bt_buf_get_type(buf)) {
		if (!btreset && ready_cb && 
			(!hrtimer_is_running(&bt_data_hrtimer))) {
			hrtimer_start(&bt_data_hrtimer, BT_DATA_HRTIMER_TIME, 0);
		}
	}
...
static void hci_rx_thread(void)
{
	...
	while (1) {
		...
		BT_ASSERT(buf);
		if (!buf) {
			BT_ERR("rx_queue get fail, crash!");
			return;
		}
#if defined(CONFIG_NET_BUF_POOL_USAGE)
		timestamp = k_uptime_get_32();
		...
		if (atomic_test_bit(bt_dev.flags, BT_DEV_RX_THREAD_EXIT)) {
			/* unref current buf before exit */
			net_buf_unref(buf);
			atomic_clear_bit(bt_dev.flags, BT_DEV_RX_THREAD_EXIT);
			k_thread_priority_set(k_current_get(), -1);
			printk("hci_rx_thread exit.\n");
			return;
		}
...
extern void libbtstack_config_check(uint32_t cfg);

static void hci_btreset_work(struct k_work *work)
{
	if (!ready_cb) {
		return;
	}
	...
	printk("%d Try to send, read btcore.\n" ,__LINE__);
#if defined(CONFIG_BT_HCI) || defined(CONFIG_BT_HCI_ACTS)
	#define CHECK_READ_REG_ADDR		0xc0176100
	int bt_stack_vs_read_bb_reg(uint32_t addr, uint8_t size);

	bt_stack_vs_read_bb_reg(CHECK_READ_REG_ADDR, 1);
#endif
}

static void bt_data_hrtimer_callback(struct hrtimer *timer, void *expiry_fn_arg)
{
	if (!ready_cb) {
		return;
	}
	...
	k_delayed_work_submit(&reset_work, K_NO_WAIT);
}
...
int bt_enable(bt_ready_cb_t cb)
{
	...
	libbtstack_config_check(STACK_CHECK_CFG_VALUE);
	...
	hrtimer_init(&bt_data_hrtimer, bt_data_hrtimer_callback, NULL);
	k_delayed_work_init(&reset_work, hci_btreset_work);
	ready_cb = cb;
...
void bt_setup_public_id_addr(void)
{
	...
	if (bti_le_addr_type() == BT_ADDR_LE_PUBLIC) {
		/* Nothing todo */
	} else if (bti_le_addr_type() == BT_ADDR_LE_RANDOM) {
		addr.a.val[5] = ~(addr.a.val[5]);
		...
		if (!bt_smp_irk_get(ir, ir_irk)) {
			irk = ir_irk;
		} else if (IS_ENABLED(CONFIG_BT_PROPERTY)) {
...
int bt_setup_random_id_addr(void)
{
	...
	if (IS_ENABLED(CONFIG_BT_PRIVACY) && IS_ENABLED(CONFIG_BT_PROPERTY)) {
		atomic_set_bit(bt_dev.flags, BT_DEV_STORE_ID);
	}
...
int bt_le_get_adv_status(uint8_t *status)
{
	struct bt_le_ext_adv *adv = bt_adv_lookup_legacy();
...
int bt_le_adv_reset(void)
{
	...
	atomic_set(adv->flags, 0);
	memset(&bt_dev.random_addr.a,0,sizeof(bt_addr_t));
...
static int br_ext_cmd_init(void)
{
	...
	struct bt_hci_write_sync_flow_ctrl *flow_ctrl_cp;
	...
	buf = bt_hci_cmd_create(BT_HCI_OP_WRITE_SYNC_FLOW_CTRL, sizeof(*flow_ctrl_cp));
	if (!buf) {
		return -ENOBUFS;
	}

	flow_ctrl_cp = net_buf_add(buf, sizeof(*flow_ctrl_cp));
	flow_ctrl_cp->enable = 0x01;
	err = bt_hci_cmd_send_sync(BT_HCI_OP_WRITE_SYNC_FLOW_CTRL, buf, NULL);
	if (err) {
		return err;
	}
...
int bt_set_class_of_device(u32_t classofdevice)
{
	struct net_buf *buf;
	struct bt_hci_write_clase_of_device *class_cp;
	u32_t set_value;
	...
	if (classofdevice) {
		set_value = classofdevice;
	} else {
		set_value = bt_class_of_device;
	}

	class_cp = net_buf_add(buf, sizeof(*class_cp));
	class_cp->class_of_device[0] = (u8_t)set_value;
	class_cp->class_of_device[1] = (u8_t)(set_value >> 8);
	class_cp->class_of_device[2] = (u8_t)(set_value >> 16);
...
static void bt_thread_exit(void)
{
	...
	while ((buf = net_buf_get(&bt_dev.cmd_tx_queue, K_NO_WAIT)) != NULL) {
		BT_ERR("Unable to send to driver.");
		if (cmd(buf)->sync) {
			printk("%d bt_thread_exit.\n",__LINE__);
			k_sem_give(cmd(buf)->sync);
		}
...
int bt_disable(void)
{
	...
	k_delayed_work_cancel(&reset_work);
	if (hrtimer_is_running(&bt_data_hrtimer)) {
		hrtimer_stop(&bt_data_hrtimer);
	}
	...
	bt_conn_deinit();
	k_sem_give(&bt_dev.ncmd_sem);
	atomic_clear_bit(bt_dev.flags, BT_DEV_WAIT_NCMD_SEM);
	atomic_clear_bit(bt_dev.flags, BT_DEV_ENABLE);

	printk("bt_disable exit\n");
	return 0;
}

int bt_br_enable_pincode(bool enable)
{
    pincode_enable = enable;
 	BT_INFO("pincode %d\n",enable);
    return 0;
}
...
int bt_stack_vs_write_bb_reg(uint32_t addr, uint32_t val)
{
	struct net_buf *buf;
...
int bt_stack_vs_write_rf_reg(uint16_t addr, uint16_t val)
{
	struct net_buf *buf;
...
int bt_stack_vs_read_rf_reg(uint16_t addr, uint8_t size)
{
	struct net_buf *buf;
...
int bt_vs_set_tws_int_clock(struct bt_conn *conn, uint32_t bt_clk, uint16_t bt_intraslot)
{
	...
	return bt_hci_cmd_send(BT_HCI_OP_ACTS_VS_SET_TWS_INT_CLOCK, buf);
}
...
int bt_vs_enable_tws_int(uint8_t enable)
{
	...
	return bt_hci_cmd_send(BT_HCI_OP_ACTS_VS_ENABLE_TWS_INT, buf);
}
	...
	uint32_t clk, uint16_t slot, uint32_t per_clk, uint16_t per_slot)
{
	...
	return bt_hci_cmd_send(BT_HCI_OP_ACTS_VS_SET_TWS_INT_CLOCK_NEW, buf);
}
...
int bt_vs_enable_tws_int_new(uint8_t enable, uint8_t index)
{
	...
	return bt_hci_cmd_send(BT_HCI_OP_ACTS_VS_ENABLE_TWS_INT_NEW, buf);
}

...
/* Can't used default MTU(672), for default mtu,
 * send channel config will without MTU option,
 * some phone can't process sdp respone without MTU option.
 */
...
static struct bt_sdp bt_sdp_pool[CONFIG_BT_MAX_BR_CONN];
static K_MUTEX_DEFINE(sdp_lock);

/* Pool for outgoing SDP packets */
extern struct net_buf_pool_continue host_tx_pool;
BT_BUF_POOL_DEFINE(sdp_pool, CONFIG_BT_MAX_CONN,
			  BT_L2CAP_BUF_SIZE(SDP_MTU), 4, NULL, &host_tx_pool);

#define SDP_CLIENT_CHAN(_ch) CONTAINER_OF(_ch, struct bt_sdp_client, chan.chan)

//#define SDP_CLIENT_MTU 64
#define SDP_CLIENT_MTU (SDP_DATA_MTU + sizeof(struct bt_sdp_hdr))
...
static struct bt_sdp_client bt_sdp_client_pool[CONFIG_BT_MAX_BR_CONN];
			 ...
			 uint16_t tid)
{
	...
	BT_ASSERT(buf);
	if (!buf) {
		BT_ERR("err_rsp sdp create pdu, crash!");
		return;
	}
...
static struct bt_sdp_record *bt_sdp_foreach_svc(bt_sdp_svc_func_t func,
						void *user_data)
{
	k_mutex_lock(&sdp_lock, K_FOREVER);
				...
				uint16_t tid)
{
	...
	memset(matching_recs, 0, BT_SDP_MAX_SERVICES * sizeof(void *));
	...
	BT_ASSERT(resp_buf);
	if (!resp_buf) {
		BT_ERR("sdp pool alloc fail!");
		return BT_SDP_INVALID_PDU_SIZE;
	}
			    ...
			    struct net_buf *buf, uint8_t nest_level)
{
	...
	if ((elem->type & BT_SDP_TYPE_DESC_MASK) == BT_SDP_SEQ_UNSPEC ||
		...
		if ((elem->type & BT_SDP_TYPE_DESC_MASK) != BT_SDP_TEXT_STR_UNSPEC &&
			(elem->type & BT_SDP_TYPE_DESC_MASK) != BT_SDP_BOOL) {
			BT_INFO("Need convert2 type 0x%x, %d", (elem->type & BT_SDP_TYPE_DESC_MASK), seq_size);
		}
		net_buf_add_mem(buf, elem->data, seq_size);
	}
			     ...
			     uint16_t tid)
{
	...
	BT_ASSERT(rsp_buf);
	if (!rsp_buf) {
		BT_ERR("sdp create pdu fail, crash!");
		return BT_SDP_INVALID_PDU_SIZE;
	}
...
int bt_sdp_register_service(struct bt_sdp_record *service)
{
	...
	if (db) {
		...
		do {
			if (rec == service) {
				BT_ERR("registered 0x%x.",service);
				k_mutex_unlock(&sdp_lock);
				return -EBUSY;
			}
	...
	BT_DBG("Service registered at 0x%p 0x%x %d", service, handle,num_services);
	k_mutex_unlock(&sdp_lock);
...
{
	struct bt_sdp_record *rec_sdp = db;
	struct bt_sdp_record *prev_sdp = NULL;
	struct bt_sdp_record *last_sdp = NULL;
	int count = BT_SDP_MAX_SERVICES;
	...
	k_mutex_lock(&sdp_lock, K_FOREVER);
	if (num_services > BT_SDP_MAX_SERVICES) {
		BT_ERR("Reached max allowed registrations %d.",num_services);
		k_mutex_unlock(&sdp_lock);
		return -ENOMEM;
	}
	...
	while (rec_sdp && (count > 0)) {
		BT_DBG("db 0x%p rec_sdp 0x%p 0x%p 0x%p.", 
						db, rec_sdp, rec_sdp->next, rec_sdp->prev);
		if ((service == rec_sdp) &&
			(service->index == rec_sdp->index)) {
			last_sdp = rec_sdp->next;
			prev_sdp = rec_sdp->prev;

			if (last_sdp) {
				last_sdp->prev = prev_sdp;
			}

			if (prev_sdp) {
				prev_sdp->next = last_sdp;
			}

			if (!last_sdp && !prev_sdp) {
				db = NULL;
			}

			if ((rec_sdp == db) && last_sdp) {
				db = last_sdp;
			}

			rec_sdp->next = NULL;
			rec_sdp->prev = NULL;
			BT_INFO("Delete Completed.");
			break;
		}

		rec_sdp = rec_sdp->next;
		count --;
		if (!rec_sdp) {
			BT_ERR("not sdp handle.");
			k_mutex_unlock(&sdp_lock);
			return -ESRCH;
		}
	}
...
bool bt_sdp_service_exist(struct bt_sdp_record *service)
{
	k_mutex_lock(&sdp_lock, K_FOREVER);
	struct bt_sdp_record *rec = db;
	int count = BT_SDP_MAX_SERVICES;
...
static int sdp_client_receive(struct bt_l2cap_chan *chan, struct net_buf *buf)
{
	...
	case BT_SDP_SVC_SEARCH_ATTR_RSP:
		...
		if (total > net_buf_tailroom(session->rec_buf)) {
			...
			BT_INFO("total %d, rec_buf %d", total, (int)net_buf_tailroom(session->rec_buf));
			sdp_client_notify_result(session, UUID_NOT_RESOLVED);
			goto iterate;
		}
...
static struct net_buf *sdp_client_alloc_buf(struct bt_l2cap_chan *chan)
{
	...
	buf = net_buf_alloc(session->param->pool, K_SECONDS(10));
	__ASSERT_NO_MSG(buf);
...
static void sdp_client_disconnected(struct bt_l2cap_chan *chan)
{
	...
	do {
		...
		if (param) {
			if (param->func) {
				result.resp_buf = NULL;
				param->func(session->chan.chan.conn, &result);
			}
			sys_slist_remove(&session->reqs, NULL, &param->_node);
		}
	} while (param);
	...
	if (session->rec_buf) {
		/* Start sdp connect, but still not connected, rec_buf still not alloc */
...
static ssize_t sdp_get_seq_len_item(uint8_t **data, size_t len)
{
	...
	BT_ASSERT(data);
	if (!data) {
		BT_ERR("sdp item pointer, crash!");
		return 0;
	}

	BT_ASSERT(*data);
	if (!(*data)) {
		BT_ERR("sdp item null, crash!");
		return 0;
	}

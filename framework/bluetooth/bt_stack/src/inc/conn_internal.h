...
/* For build lib, alway keep member, bt_trans.lib used */
//#ifdef CONFIG_BT_A2DP_TRS
	uint8_t 		direction:1;
//#endif
	uint8_t         active_connect:1;		/* Local create br connect */
	uint8_t         create_linkkey:1;		/* Create linkkey when remote without linkkey */
};
...
//#endif
	...
	bt_tx_pending_cb tx_pending_cb;
	/* Actions add end */
...
struct net_buf *bt_conn_create_pdu_len_timeout_debug(struct net_buf_pool *pool,
						 ...
						 k_timeout_t timeout,
						 const char *func, int line);
#define bt_conn_create_pdu_len_timeout(_pool, _reserve, len, _timeout) \
	bt_conn_create_pdu_len_timeout_debug(_pool, _reserve, len, _timeout, \
					 __func__, __LINE__)

#define bt_conn_create_pdu_len(_pool, _reserve, len) \
	bt_conn_create_pdu_len_timeout_debug(_pool, _reserve, len, K_FOREVER, \
					 __func__, __LINE__)
#else
struct net_buf *bt_conn_create_pdu_len_timeout(struct net_buf_pool *pool,
					   size_t reserve, size_t len, k_timeout_t timeout);

#define bt_conn_create_pdu_len(_pool, len, _reserve) \
	bt_conn_create_pdu_len_timeout(_pool, _reserve, len, K_FOREVER)
#endif
...
#define bt_conn_create_frag_len_timeout(_reserve, len, _timeout) \
	bt_conn_create_frag_len_timeout_debug(_reserve, len, _timeout, \
					  __func__, __LINE__)

#define bt_conn_create_frag_len(_reserve, len) \
	bt_conn_create_frag_len_timeout_debug(_reserve, len, K_FOREVER, \
					  __func__, __LINE__)
#else
struct net_buf *bt_conn_create_frag_len_timeout(size_t reserve, size_t len,
					    k_timeout_t timeout);

#define bt_conn_create_frag_len(_reserve, len) \
	bt_conn_create_frag_len_timeout(_reserve, len, K_FOREVER)
#endif
...
/** @brief Get connected le device number.
 *
 *  @return  le connected number.
 */

...
sensor_res_t sensor_test_res = {0};
static void hr_hb_handler(uint8_t hb_val, uint8_t hb_lvl_val, uint16_t rr_val)
{
	// sensor_raw_t sdata;
	...
	// // call algo
	// sensor_algo_process(sdata.id);
	printk("hr_hb_handler hb_val=%d, hb_lvl_val=%d, rr_val=%d\n", hb_val, hb_lvl_val, rr_val);
	sensor_test_res.heart_rate = hb_val;

	sensor_service_callback(IN_HEARTRATE, &sensor_test_res);

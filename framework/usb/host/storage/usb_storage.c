...
LOG_MODULE_REGISTER(usb_storage);
...
static inline int usb_stor_read_capacity(struct us_data *us, u8_t lun)
{
	...
	do {
		...
		if (ret == USB_STOR_XFER_GOOD) {
			...
			LOG_INF("lba: 0x%x, blksz: 0x%x", us->lba, us->blksz);
			break;
		} else if (ret == USB_STOR_XFER_FAILED) {
			;	/* retry */
		} else {

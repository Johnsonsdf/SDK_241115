...
#define LOG_LEVEL CONFIG_SYS_LOG_USB_HID_LEVEL
	...
	if (REQTYPE_GET_DIR(setup->bmRequestType) == REQTYPE_DIR_TO_HOST) {
		...
		case HID_SET_REPORT:
			if (hid_device.ops->set_report == NULL) {
				LOG_ERR("set report not implemented");
				return -EINVAL;
			}

...
static void _cst816s_read_touch(const struct device *i2c_dev, struct input_value *val);
...
static void _cst816s_poweron(const struct device *dev, bool is_on)
{
...
static void _tpkey_irq_callback(const struct device *port, struct gpio_callback *cb, uint32_t pins)
{
	...
	if(tpkey->inited) {
		_cst816s_read_touch(tpkey->i2c_dev, &val);
	}
...
static void _cst816s_irq_config(const struct device *dev, bool is_on)
{
...
static void _cst816s_reset(const struct device *dev)
{
...
static void _cst816s_enter_low_power_mode(const struct device *dev, bool low_power)
{
	uint8_t cmd[6] = {0};
...
static void _cst816s_set_report_mode(const struct device *dev, int mode)
{
	uint8_t cmd[6] = {0};
...
#if 0
static void _cst816s_turn_off_auto_reset()
{
	uint8_t cmd[6] = {0};
...
static void _tpkey_init_work(struct k_work *work)
{
	...
	_cst816s_poweron(tpkey->this_dev, true);

	_cst816s_reset(tpkey->this_dev);

	k_msleep(50);

	_cst816s_irq_config(tpkey->this_dev, true);

	//_cst816s_enter_low_power_mode(tpkey->this_dev,false);
	_cst816s_set_report_mode(tpkey->this_dev,POINT_REPORT_MODE);
...
void tpkey_acts_dump(void)
{
	...
	if (low_power_mode) {
		...
		low_power_mode = true;
		_cst816s_enter_low_power_mode(tpkey->this_dev,true);
	}
...
#ifdef CONFIG_PM_DEVICE
static void _cst816s_suspend(const struct device *dev)
{
	struct acts_tpkey_data *tpkey = (struct acts_tpkey_data *)dev->data;

	_cst816s_irq_config(tpkey->this_dev, false);
	_cst816s_poweron(tpkey->this_dev, false);

	printk("ok\n");
}
static void _cst816s_resume(const struct device *dev)
{
	struct acts_tpkey_data *tpkey = (struct acts_tpkey_data *)dev->data;
...
static int _cst816s_pm_control(const struct device *dev,  enum pm_device_action action)
{
	int ret = 0;
...
static int _cst816s_pm_control(const struct device *dev, uint32_t ctrl_command,
...
DEVICE_DEFINE(tpkey, CONFIG_TPKEY_DEV_NAME, _tpkey_acts_init,
			_cst816s_pm_control, &tpkey_acts_ddata, &_tpkey_acts_cdata, POST_KERNEL,

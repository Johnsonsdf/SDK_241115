...
int display_buffer_fill_color(const display_buffer_t *buffer, display_color_t color, int pad_byte)
{
	...
	uint16_t bytes_per_line = buffer->desc.pitch;
	uint16_t bytes_to_fill = (buffer->desc.width * bits_per_pixel + 7) / 8;
...
int display_buffer_fill_image(const display_buffer_t *buffer, const void *data, uint16_t data_stride_bytes, int pad_byte)
{
	uint8_t bits_per_pixel = display_format_get_bits_per_pixel(buffer->desc.pixel_format);
	uint16_t bytes_per_line = buffer->desc.pitch;
	uint16_t bytes_to_copy = (buffer->desc.width * bits_per_pixel + 7) / 8;
	uint8_t *buf8 = (uint8_t *)buffer->addr;

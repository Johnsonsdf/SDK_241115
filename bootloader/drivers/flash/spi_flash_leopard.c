...
__ramfunc static unsigned int spi0_cache_enter_dtr_mode(struct spinor_info *sni)
{
	...
	spi0_dtr_set_clk(MHZ(CONFIG_SPI_FLASH_FREQ_MHZ) * 2);
...
static int spi_flash_acts_read(const struct device *dev, off_t offset, void *data, size_t len)
{
	...
	xip_start = CONFIG_SPI_XIP_VADDR + offset;
	pbrom_libc_api->p_memcpy(data, (void *)xip_start, len);
	//memcpy(data, (void *)xip_start, len);
	return 0;
}
...
__ramfunc int spi_flash_acts_init(const struct device *dev)
{
	...
	xspi_setup_delaychain(sni, sni->spi.delay_chain);
		...
		if(sni->spi.bus_width == 4){
			//while(loop);
			sni->chipid = spi0_cache_enter_dtr_mode(sni);

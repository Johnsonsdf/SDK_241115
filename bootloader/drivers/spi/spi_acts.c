...
#define SPI_CTL_DMS_MASK		(0x0 << 25)
...
#define SPI_CTL_IO_MODE_MASK			(0x0 << 10)
              ...
              struct k_poll_signal *signal)
{
    ...
    if (!tx_bufs->count && !rx_bufs->count) {
        return 0;
    }
...
#define SPI_ACTS_DEVICE_INIT(n)						\
    ...
    DEVICE_DEFINE(spi_acts_##n,				\
                ...
                SPI_ACTS_DEFINE_CONFIG(1)
#endif
#if IS_ENABLED(CONFIG_SPI_2)
                SPI_ACTS_DEFINE_CONFIG(2)
#endif
#if IS_ENABLED(CONFIG_SPI_3)
                SPI_ACTS_DEFINE_CONFIG(3)
#endif

...
#define IRQ_VETOR_MAXLEN 	0x140
static volatile uint32_t Z_GENERIC_SECTION(.srm_irq_vector) sram_irq_vector[IRQ_VETOR_MAXLEN/4] __aligned(1024);
#endif
...
{
#ifdef CONFIG_IRQ_VECTOR_IN_SRAM
	memcpy((void *)sram_irq_vector, (const void *)VECTOR_ADDRESS, IRQ_VETOR_MAXLEN);
	SCB->VTOR = (unsigned int)sram_irq_vector ;
#else
